...
TEXT_DELAY_SECONDS = int(os.environ.get('TEXT_DELAY_SECONDS', '30'))  # Wait 30 seconds after text before calling
...
def send_twilio_alert(message, to=None):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(f"✅ Twilio alert sent!")
            return {'success': True}
        else:
            print(f"❌ Twilio alert failed: {response.text}")
...
def verify_retell_webhook(handler, body_bytes):
    ...
    # Check for Retell signature header
    signature = handler.headers.get('x-retell-signature', '')
    
    # If no signature provided, allow through (Retell may not always send it)
    if not signature:
        print(f"⚠️ Retell webhook from {client_ip} - no signature (allowing)")
        return True, None
    
    # If we have a webhook secret configured, verify it
    ...
    if webhook_secret:
        try:
            ...
            if not hmac.compare_digest(signature, expected_sig):
                print(f"🚫 Invalid Retell webhook signature from {client_ip}")
                return False, "Invalid webhook signature"
        except Exception as e:
            print(f"⚠️ Signature verification error: {e}")
    else:
        # No webhook secret configured - log and allow
        print(f"✅ Retell webhook from {client_ip} (no secret configured, allowing)")
...
def send_security_alert(reason, details=""):
    """Send SMS alert to owner about suspicious activity - via Retell"""
    try:
        message = f"VoiceLab Security Alert\n\n{reason}\n\n{details}\n\nCheck: voicelab.live/api/security-status"
        
        # Use Retell SMS (same number as calls!)
        if OWNER_PHONE:
            print(f"🚨 Sending security alert to {OWNER_PHONE}")
            
            # Use correct Retell SMS Chat API
            response = requests.post(
                "https://api.retellai.com/create-sms-chat",
                headers={
                    "Authorization": f"Bearer {RETELL_API_KEY}",
                ...
                json={
                    "from_number": HAILEY_PHONE_NUMBER,
                    "to_number": format_phone(OWNER_PHONE),
                    "retell_llm_dynamic_variables": {
                        "alert_message": message,
                        "alert_type": "security"
                    },
                    "metadata": {
                        "source": "voicelab_security",
                        "reason": reason
                    }
                },
                timeout=15
...
def init_db():
    ...
    # 🔥 SCHEDULED TEXTS - For SMS sequence that never gives up!
    c.execute('''CREATE TABLE IF NOT EXISTS scheduled_texts (
        id INTEGER PRIMARY KEY,
        sequence_id INTEGER,
        phone TEXT,
        first_name TEXT,
    ...
    # 🔥 SMS Follow-up Sequences - Text every 10 minutes until they respond!
    c.execute('''CREATE TABLE IF NOT EXISTS sms_sequences (
        id INTEGER PRIMARY KEY,
        phone TEXT NOT NULL,
        ghl_contact_id TEXT,
        first_name TEXT,
        ...
        -- Sequence status
        status TEXT DEFAULT 'active',  -- active, responded, appointment_set, stopped, max_reached
        texts_sent INTEGER DEFAULT 1,
        max_texts INTEGER DEFAULT 20,
        
        -- Timing
        last_text_at TIMESTAMP,
        next_text_at TIMESTAMP,
        minutes_between_texts INTEGER DEFAULT 10,
        
        -- Response tracking
        customer_responded INTEGER DEFAULT 0,
        appointment_booked INTEGER DEFAULT 0,
...
# Every text ends with 2 time options
SMS_FOLLOWUP_TEMPLATES = [
    # Follow-up 1 (10 min) - Reference the ad
    "{first_name} - so that $400 bill thing... that hit close to home or nah? 😂 I got 2pm or 5pm today - which works?",
    
    # Follow-up 2 (20 min) - Sarcastic + relatable
    "Hey {first_name} - just checking if the electric company got to your paycheck before I did lol. Today at 5pm or tomorrow at 10am?",
    
    # Follow-up 3 (30 min) - Self-aware
    "{first_name} - I know I'm texting again. Blame the electric company, not me 🤷 Morning or afternoon work better?",
    
    # Follow-up 4 (40 min) - Funny + helpful
    "Look {first_name} - either the AC is killing your wallet or I'm wrong about {address}. 15 min to find out - 10am or 2pm tomorrow?",
    
    # Follow-up 5 (50 min) - Real talk
    "{first_name} - not gonna lie, your area is getting crushed. Figured you'd wanna know before summer hits 🥵 Today 5pm or tomorrow 10am?",
    
    # Follow-up 6 (60 min) - Pattern interrupt
    "Ok {first_name} - quick 👍 or 👎? If it's a 👍, I got 2pm or 6pm open tomorrow.",
    
    # Follow-up 7 (70 min) - Sarcastic guilt
    "{first_name} - your neighbors already figured this out. Just saying 🏠 You want the morning slot or afternoon?",
    
    # Follow-up 8 (80 min) - Funny take-away  
    "Alright {first_name} - last shot before I leave you alone 😅 Tomorrow 10am or 5pm? Either one works.",
    
    # Follow-up 9 (90 min) - Last shot, keep it light
    "Final text {first_name} - 10am or 2pm tomorrow and I'll show you what's up with {address}. Or just ignore me, no hard feelings 👊",
    
    # Follow-up 10+ - Rotate these casual check-ins
    "{first_name} - still thinking about {address}? I got morning or afternoon open - lmk.",
    "Hey {first_name} 👋 rates went up again. 15 min could save you thousands. Today or tomorrow?",
    "{first_name} - random but someone just cancelled. 2pm or 5pm slot is yours if you want it.",
    "Quick check {first_name} - you figure out that bill situation yet? 😂 Got 10am or 6pm if you're ready.",
]

# 🔥 SMS SEQUENCE CONFIG - Text every 10 minutes until they respond!
SMS_SEQUENCE_CONFIG = {
    'minutes_between_texts': 10,     # Text every 10 minutes
    'max_texts': 15,                 # Max texts before giving up (15 texts = 2.5 hours of trying)
    'max_days': 3,                   # Keep trying for 3 days
    'blocked_hours_start': 21,       # 9pm - no texts after
    'blocked_hours_end': 9,          # 9am - no texts before
}
...
def is_in_call_window():
    ...
    if BYPASS_CALLING_HOURS:
        print(f"🔧 BYPASS_CALLING_HOURS=true - Allowing call anytime!")
...
def schedule_next_call(sequence_id):
    """
    Schedule the next call in a sequence.
    
    🔥 CALLS EVERY 2 HOURS UNTIL THEY PICK UP!
    - Respects calling hours (9am-8pm Pacific)
    - If outside hours, schedules for next available time
    - Stops when: appointment set, max days reached, or they answer
    """
    try:
        ...
        # Check if sequence is complete (max days reached)
        if seq.get('current_day', 1) > seq.get('max_days', 7):
            c.execute('UPDATE call_sequences SET status = ? WHERE id = ?', ('max_days_reached', sequence_id))
            conn.commit()
            conn.close()
            print(f"📊 Sequence completed: Day {seq.get('current_day')} of {seq.get('max_days', 7)}")
            return {"success": True, "status": "sequence_completed", "reason": "max_days_reached"}
        
        # Check if max calls reached
        if seq.get('calls_made', 0) >= seq.get('max_calls', 42):
            c.execute('UPDATE call_sequences SET status = ? WHERE id = ?', ('max_calls_reached', sequence_id))
            conn.commit()
            conn.close()
            print(f"📊 Max calls reached: {seq.get('calls_made')} calls made")
            return {"success": True, "status": "max_calls_reached"}
            ...
            try:
                ...
                if contact.get('contact', {}).get('tags', []):
                    ...
                    if 'appointment set' in tags or 'appointment-set' in tags or 'ai - appointment set' in tags:
                        ...
                        conn.commit()
                        conn.close()
                        print(f"✅ Appointment set! Stopping sequence.")
                        return {"success": True, "status": "appointment_set"}
            except:
                pass
        
        # 🔥 SIMPLE 2-HOUR INTERVAL SCHEDULING
        hours_between = CALL_SEQUENCE_CONFIG.get('hours_between_calls', 2)
        blocked_start = CALL_SEQUENCE_CONFIG.get('blocked_hours_start', 20)  # 8pm
        blocked_end = CALL_SEQUENCE_CONFIG.get('blocked_hours_end', 9)       # 9am
        
        # Calculate next call time: now + 2 hours
        next_time = now + timedelta(hours=hours_between)
        
        # Check if next_time is within calling hours
        next_hour = next_time.hour
        
        # If outside calling hours, adjust to next available window
        if next_hour >= blocked_start or next_hour < blocked_end:
            if next_hour >= blocked_start:
                # After 8pm - schedule for 9am tomorrow
                next_time = now + timedelta(days=1)
                next_time = next_time.replace(hour=blocked_end, minute=0, second=0, microsecond=0)
            else:
                # Before 9am - schedule for 9am today
                next_time = now.replace(hour=blocked_end, minute=0, second=0, microsecond=0)
            
            # Skip Sunday
            if next_time.weekday() == 6:
                next_time = next_time + timedelta(days=1)
        
        window_name = f"{hours_between}hr_interval"
        ...
        c.execute('UPDATE call_sequences SET calls_today = calls_today + 1 WHERE id = ?', (sequence_id,))
        
        conn.commit()
        conn.close()
        
        print(f"📅 Next call in {hours_between} hours: {next_time.strftime('%Y-%m-%d %I:%M %p')} Pacific")
        return {"success": True, "scheduled_time": next_time.isoformat(), "window": window_name, "hours_until": hours_between}
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔥 SMS SEQUENCE - NEVER GIVE UP TEXTING!
# Texts every 10 minutes until they respond or book
# ═══════════════════════════════════════════════════════════════════════════════

def schedule_next_text(sequence_id, phone, first_name, address, ghl_contact_id, message_number=1):
    """
    Schedule the next text in the sequence.
    Texts every 10 minutes until they respond or book!
    """
    try:
        conn = sqlite3.connect(DB_PATH, timeout=10)
        ...
        c.execute('SELECT status FROM call_sequences WHERE id = ?', (sequence_id,))
        seq = c.fetchone()
        if seq and seq[0] in ['appointment_set', 'stopped', 'booked']:
            conn.close()
            print(f"📱 SMS Sequence stopped - appointment already set")
            return {"success": True, "status": "appointment_set"}
        
        # Check if max texts reached
        if message_number > SMS_SEQUENCE_CONFIG.get('max_texts', 15):
            conn.close()
            print(f"📱 SMS Sequence complete - max texts reached ({message_number})")
            return {"success": True, "status": "max_texts_reached"}
        
        # Calculate next text time
        now = get_pacific_time()
        minutes_between = SMS_SEQUENCE_CONFIG.get('minutes_between_texts', 10)
        next_time = now + timedelta(minutes=minutes_between)
        
        # Check if outside texting hours
        blocked_start = SMS_SEQUENCE_CONFIG.get('blocked_hours_start', 21)  # 9pm
        blocked_end = SMS_SEQUENCE_CONFIG.get('blocked_hours_end', 9)       # 9am
        
        next_hour = next_time.hour
        if next_hour >= blocked_start or next_hour < blocked_end:
            # Too late/early - schedule for tomorrow morning
            if next_hour >= blocked_start:
                next_time = now + timedelta(days=1)
            next_time = next_time.replace(hour=blocked_end, minute=0, second=0, microsecond=0)
        
        # Schedule the text
        c.execute('''INSERT INTO scheduled_texts 
            (sequence_id, phone, first_name, address, ghl_contact_id, scheduled_time, message_number, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (sequence_id, phone, first_name, address, ghl_contact_id, next_time.isoformat(), message_number, 'pending', datetime.now().isoformat()))
        
        conn.commit()
        conn.close()
        
        print(f"📱 Text #{message_number} scheduled for {next_time.strftime('%I:%M %p')} Pacific")
        return {"success": True, "scheduled_time": next_time.isoformat(), "message_number": message_number}
    
    except Exception as e:
        print(f"❌ Error scheduling text: {e}")
        return {"success": False, "error": str(e)}

def process_scheduled_texts():
    """
    Process any texts that are due.
    Run this every minute via the scheduler.
    """
    try:
        conn = sqlite3.connect(DB_PATH, timeout=10)
        conn.row_factory = sqlite3.Row
        ...
        # Get pending texts that are due
        c.execute('''SELECT * FROM scheduled_texts 
                    WHERE status = 'pending' AND scheduled_time <= ?
                    ORDER BY scheduled_time ASC
                    LIMIT 5''', (now.isoformat(),))
        ...
        for txt in texts:
            txt = dict(txt)
            txt_id = txt['id']
            sequence_id = txt['sequence_id']
            phone = txt['phone']
            first_name = txt['first_name']
            address = txt['address']
            ghl_contact_id = txt['ghl_contact_id']
            message_number = txt.get('message_number', 1)
            
            print(f"📱 Processing scheduled text #{message_number} to {phone}")
            
            # Check if appointment was set (skip if so)
            if sequence_id:
                c.execute('SELECT status FROM call_sequences WHERE id = ?', (sequence_id,))
                seq = c.fetchone()
                if seq and seq[0] in ['appointment_set', 'stopped', 'booked']:
                    c.execute('UPDATE scheduled_texts SET status = ?, executed_at = ? WHERE id = ?',
                             ('skipped_appointment_set', datetime.now().isoformat(), txt_id))
                    print(f"📱 Skipping text - appointment already set!")
                    continue
            
            # Get the message template
            template_index = min(message_number - 1, len(SMS_FOLLOWUP_TEMPLATES) - 1)
            # For messages beyond our templates, rotate through the last few
            if message_number > len(SMS_FOLLOWUP_TEMPLATES):
                template_index = len(SMS_FOLLOWUP_TEMPLATES) - 4 + ((message_number - len(SMS_FOLLOWUP_TEMPLATES)) % 4)
            
            message_template = SMS_FOLLOWUP_TEMPLATES[template_index]
            message = message_template.format(first_name=first_name, address=address)
            
            # Send the text via Retell
            try:
                sms_result = send_retell_sms(phone, message=message, name=first_name, address=address)
                
                if sms_result.get('success'):
                    c.execute('UPDATE scheduled_texts SET status = ?, message_sent = ?, executed_at = ? WHERE id = ?',
                             ('sent', message, datetime.now().isoformat(), txt_id))
                    print(f"✅ Text #{message_number} sent: {message[:50]}...")
                    
                    # Schedule the next text
                    schedule_next_text(sequence_id, phone, first_name, address, ghl_contact_id, message_number + 1)
                    processed += 1
                else:
                    c.execute('UPDATE scheduled_texts SET status = ?, error = ?, executed_at = ? WHERE id = ?',
                             ('failed', str(sms_result.get('error')), datetime.now().isoformat(), txt_id))
                    print(f"❌ Text failed: {sms_result.get('error')}")
            
            except Exception as send_err:
                c.execute('UPDATE scheduled_texts SET status = ?, error = ?, executed_at = ? WHERE id = ?',
                         ('error', str(send_err), datetime.now().isoformat(), txt_id))
                print(f"❌ Text send error: {send_err}")
        ...
        print(f"❌ Error processing scheduled texts: {e}")
        return {"success": False, "error": str(e)}

def stop_sms_sequence(phone=None, sequence_id=None, reason="manual"):
    """Stop SMS sequence for a phone number or sequence ID"""
    try:
        conn = sqlite3.connect(DB_PATH, timeout=10)
        c = conn.cursor()
        
        if sequence_id:
            c.execute('UPDATE scheduled_texts SET status = ? WHERE sequence_id = ? AND status = ?',
                     (f'stopped_{reason}', sequence_id, 'pending'))
        elif phone:
            c.execute('UPDATE scheduled_texts SET status = ? WHERE phone = ? AND status = ?',
                     (f'stopped_{reason}', phone, 'pending'))
        
        affected = c.rowcount
        conn.commit()
        conn.close()
        
        print(f"📱 Stopped {affected} pending texts ({reason})")
        return {"success": True, "stopped": affected}
...
def process_sms_followups():
    """
    🔥 Process SMS follow-ups - Text every 10 minutes until they respond!
    Jeremy Miner style - NEVER GIVE UP!
    """
    try:
        conn = sqlite3.connect(DB_PATH, timeout=10)
        conn.row_factory = sqlite3.Row
        ...
        # Get SMS sequences that need follow-up
        c.execute('''SELECT * FROM sms_sequences 
            WHERE status = 'active' 
            AND next_text_at <= ? 
            AND texts_sent < max_texts
            ORDER BY next_text_at ASC
            LIMIT 5''', (now.isoformat(),))
        ...
        for seq in sequences:
            ...
            address = seq.get('address', 'your home')
            texts_sent = seq.get('texts_sent', 1)
            minutes_between = seq.get('minutes_between_texts', 10)
            
            # Get the right follow-up template
            template_index = min(texts_sent - 1, len(SMS_FOLLOWUP_TEMPLATES) - 1)
            
            # For later follow-ups, rotate through the last few templates
            if texts_sent > len(SMS_FOLLOWUP_TEMPLATES):
                # Rotate through last 4 templates
                template_index = len(SMS_FOLLOWUP_TEMPLATES) - 4 + ((texts_sent - len(SMS_FOLLOWUP_TEMPLATES)) % 4)
            
            template = SMS_FOLLOWUP_TEMPLATES[template_index]
            message = template.format(first_name=first_name, address=address)
            
            print(f"📱 SMS Follow-up #{texts_sent + 1} to {phone}: {message[:50]}...")
            
            # Send the follow-up via Retell SMS
            try:
                sms_result = send_retell_sms(phone, message=message, name=first_name, address=address)
                
                if sms_result.get('success'):
                    # Update sequence - schedule next follow-up
                    next_text_time = now + timedelta(minutes=minutes_between)
                    c.execute('''UPDATE sms_sequences SET 
                        texts_sent = texts_sent + 1,
                        last_text_at = ?,
                        next_text_at = ?
                        WHERE id = ?''',
                        (now.isoformat(), next_text_time.isoformat(), seq['id']))
                    
                    processed += 1
                    print(f"✅ Follow-up sent! Next in {minutes_between} min")
                else:
                    print(f"⚠️ Follow-up failed: {sms_result.get('error')}")
                    
            except Exception as send_err:
                print(f"❌ SMS send error: {send_err}")
        
        # Check for max reached
        c.execute('''UPDATE sms_sequences SET status = 'max_reached' 
            WHERE status = 'active' AND texts_sent >= max_texts''')
        ...
        print(f"❌ Error processing SMS follow-ups: {e}")
        import traceback
        traceback.print_exc()
        return {"processed": 0, "error": str(e)}


def stop_sms_sequence(phone):
    """Stop SMS sequence for a phone number (they responded or booked)"""
    try:
        conn = sqlite3.connect(DB_PATH, timeout=10)
        c = conn.cursor()
        c.execute('''UPDATE sms_sequences SET status = 'responded', customer_responded = 1 
            WHERE phone = ? AND status = 'active' ''', (phone,))
        conn.commit()
        conn.close()
        print(f"📱 SMS sequence stopped for {phone} - they responded!")
        return True
    except Exception as e:
        print(f"⚠️ Error stopping SMS sequence: {e}")
        return False
...
def send_sms(phone, message, message_type="general", name="", address=""):
    ...
    print(f"📱 SEND SMS via Retell")
    print(f"   From: {HAILEY_PHONE_NUMBER}")
    print(f"   To: {phone}")
    ...
    try:
        # Retell SMS Chat API - correct endpoint
        response = requests.post(
            "https://api.retellai.com/create-sms-chat",
            headers={
                "Authorization": f"Bearer {RETELL_API_KEY}",
            ...
            json={
                "from_number": HAILEY_PHONE_NUMBER,
                "to_number": phone,
                # Pass dynamic variables for the chat agent to use in messages
                "retell_llm_dynamic_variables": {
                    "first_name": first_name,
                    "customer_name": first_name,
                    "name": name,
                    "address": address or "your home",
        ...
        if response.status_code in [200, 201]:
            data = response.json()
            print(f"✅ Retell SMS Chat started successfully!")
            ...
            try:
                ...
                c.execute('INSERT INTO sms_log (phone, message, message_type, status, twilio_sid) VALUES (?, ?, ?, ?, ?)',
                          (phone, message, message_type, 'sent', data.get('chat_id', 'retell')))
                conn.commit()
                conn.close()
                log_cost('sms', COST_PER_SMS, f'{message_type} SMS to {phone}')
            except Exception as db_err:
                print(f"⚠️ DB log error: {db_err}")
            
            return {"success": True, "from": HAILEY_PHONE_NUMBER, "chat_id": data.get('chat_id')}
        else:
            print(f"❌ Retell SMS failed: {response.status_code}")
            print(f"   Response: {response.text}")
            return {"success": False, "error": response.text}
            
    except Exception as e:
        print(f"❌ Retell SMS error: {e}")
        return {"success": False, "error": str(e)}
...
def send_retell_sms(to_number, message="", from_number=None, name="", address=""):
    ...
    Uses Retell's create-sms-chat endpoint.
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            data = response.json()
            print(f"✅ Retell SMS Chat started!")
            print(f"   Chat ID: {data.get('chat_id', 'N/A')}")
            return {'success': True, 'from': from_number, 'chat_id': data.get('chat_id'), 'data': data}
        else:
            print(f"❌ Retell SMS failed: {response.status_code}")
            print(f"   Response: {response.text}")
            return {'success': False, 'error': response.text}
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None, address="", text_first=None):
    """
    Make an AI call. For solar leads, sends text first by default (NEPQ sequence).
    
    text_first options:
    - None (default): Auto-detect based on agent_type (True for solar)
    - True: Always send text before call
    - False: Skip text, call immediately
    """
    phone = format_phone(phone)
    ...
    # 🔥 NEPQ TEXT-FIRST SEQUENCE
    # For solar leads, send text before calling (unless explicitly disabled)
    if text_first is None:
        text_first = TEXT_BEFORE_CALL and agent_type == 'solar'
    
    if text_first and agent_type == 'solar':
        print(f"")
        print(f"📱🔥 NEPQ TEXT-FIRST SEQUENCE ACTIVATED!")
        print(f"📱 Sending intro text from {HAILEY_PHONE_NUMBER}...")
        ...
        if sms_result.get('success'):
            ...
            print(f"⚠️ Text failed, calling anyway: {sms_result.get('error')}")
        
        # Small delay to let text arrive (2 seconds)
        import time
        time.sleep(2)
    
    # 🔥 USE VERIFIED CALLER ID BRIDGE for outbound calls (disabled - using Retell direct)
    if USE_VERIFIED_CALLER_ID and agent_type != 'inbound' and not agent_type.startswith('inbound_'):
        print(f"🔥 Using Verified Caller ID Bridge!")
        return make_call_with_verified_caller_id(phone, name, agent_type, ghl_contact_id, address)
    ...
    if is_inbound:
        ...
        company_name = "Bulldog Roofing"
        # Begin message lives in the Retell agent: "Hey, is this {{first_name}}?"
        opening = f"Hey, is this {name}?"
        ...
        # 🔥 v3 OPENING: Reference text + Lead with bill guess (CLEANER VERSION)
        if address:
            opening = f"Hey {name} - Hailey here, how's it going? ... So you saw my text about {address}? What's your bill running you, like $200, $300 a month?"
        else:
            opening = f"Hey {name} - Hailey here, how's it going? ... So you saw my text? What's your electric bill running you these days?"
...
# NOTE: send_retell_sms function is defined at end of file with correct Retell API endpoint
# Uses POST https://api.retellai.com/create-sms-chat
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 🔥 TwiML endpoint for Twilio→Retell bridge
        if path == '/twiml/retell-stream':
            twiml = handle_twiml_retell_stream(q)
            self.send_response(200)
...
<script>setTimeout(()=>location.reload(), 60000);</script>
    ...
    def do_POST(self):
        ...
        # Demo call from landing page
            ...
            text_first = d.get('text_first', None)  # None = auto-detect, True = always text first
            print(f"📞 DEMO CALL: agent_type={agent_type}, phone={phone}, text_first={text_first}")
            if not phone:
                self.send_json({"success": False, "error": "Phone number required"})
            else:
                # Make the call via Retell's create-phone-call API
                result = make_call(phone, name="there", agent_type=agent_type, is_test=False, text_first=text_first)
                self.send_json(result)
        
        # 🔥 NEPQ TEXT-THEN-CALL ENDPOINT
        elif path == '/api/text-then-call':
            phone = d.get('phone', '')
            name = d.get('name', 'there')
            agent_type = d.get('agent_type', 'solar')
            address = d.get('address', '')
            delay = int(d.get('delay', TEXT_DELAY_SECONDS))
            
            print(f"📱🔥 TEXT-THEN-CALL REQUEST")
            print(f"   Phone: {phone}")
            print(f"   Name: {name}")
            ...
            if not phone:
                ...
                result = text_then_call(phone, name=name, agent_type=agent_type, address=address, delay_seconds=delay)
                self.send_json(result)
        
        # 📱 SEND SMS VIA RETELL (same number as calls!)
        elif path == '/api/send-sms':
            phone = d.get('phone', d.get('to', ''))
            message = d.get('message', '')
            from_number = d.get('from', HAILEY_PHONE_NUMBER)
            
            print(f"📱 SEND SMS REQUEST")
            print(f"   To: {phone}")
            print(f"   From: {from_number}")
            ...
            if not phone:
                ...
                self.send_json({"success": False, "error": "Message required"})
            else:
                result = send_retell_sms(phone, message, from_number)
                self.send_json(result)
        
        # 🔥 TEST NEPQ SEQUENCE (your phone)
        elif path == '/api/test-nepq':
            phone = d.get('phone', '')
            name = d.get('name', 'Boss')
            
            print(f"🔥 TEST NEPQ SEQUENCE to {phone}")
            
            if not phone:
                self.send_json({"success": False, "error": "Phone number required"})
            else:
                # Send text, wait, then call
                result = make_call(phone, name=name, agent_type='solar', text_first=True)
                self.send_json(result)
        
        elif path == '/api/test-verified-caller':
            # 🔥 TEST ENDPOINT: Test the verified caller ID bridge
            phone = d.get('phone', '')
            name = d.get('name', 'there')
            agent_type = d.get('agent_type', 'solar')
            address = d.get('address', '')
            
            print(f"🔥 TEST VERIFIED CALLER ID BRIDGE")
            print(f"   Phone: {phone}")
            print(f"   From: {VERIFIED_CALLER_ID}")
            
            if not phone:
                self.send_json({"success": False, "error": "Phone number required"})
            else:
                result = make_call_with_verified_caller_id(phone, name, agent_type, None, address)
                self.send_json(result)
...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                # GHL nests the real routing fields under customData - read them from there.
                ...
                try:
                    ...
                    print(f"⚠️ DB log error (continuing anyway): {db_err}")
                    conn = sqlite3.connect(DB_PATH, timeout=10)
                    c = conn.cursor()
                ...
                if action == 'sequence' or action == 'start_sequence':
                    ...
                    # 🔥 Send NEPQ intro text via Retell (same number as calls!)
                    # Uses name + address for personalization - matches Facebook ad energy
                    ...
                    if sms_result.get('success'):
                        ...
                        # 🔥 START SMS FOLLOW-UP SEQUENCE - Text every 10 min until they respond!
                        try:
                            schedule_next_text(
                                sequence_id=sequence_id,
                                phone=phone,
                                ...
                                message_number=1  # First follow-up
                            )
                            print(f"📱 SMS follow-up sequence started - next text in 10 minutes")
                        except Exception as sms_seq_err:
                            print(f"⚠️ SMS sequence creation error: {sms_seq_err}")
                    else:
                        print(f"⚠️ Retell SMS failed: {sms_result.get('error')}")
                        # Fallback: try old method
                        sms_msg = f"Hey {first_name}! This is Hailey. Got your info for {full_address}. Calling you shortly! 📞"
                        send_sms(phone, sms_msg, "sequence_start", name=first_name, address=full_address)
                    
                    # Update GHL
                    if contact_id and GHL_API_KEY:
                        ghl_add_tag(contact_id, 'AI Sequence Started')
                        ghl_create_note(contact_id, f"🚀 AI Sequence Started\n• Calls: Every 2 hours until answer (9am-8pm)\n• Texts: Every 10 min until response\n• Never gives up for 7 days!\n• Agent: {agent_type}")
                    
                    # 🔧 If BYPASS_CALLING_HOURS is enabled, make call immediately for testing
                    if BYPASS_CALLING_HOURS:
                        print(f"🔧 BYPASS_CALLING_HOURS enabled - Making immediate call!")
                        import time
                        time.sleep(2)  # Let the SMS arrive first
                        call_result = make_call(phone, name=first_name, agent_type=agent_type, 
                                               ghl_contact_id=contact_id, address=full_address, text_first=False)
                        print(f"📞 Immediate call result: {call_result}")
                        
                        self.send_json({
                            "success": True, 
                            "sequence_id": sequence_id,
                            "lead_id": lead_id,
                            "message": "BYPASS MODE - SMS sent + immediate call made!",
        ...
        elif path == '/twiml/retell-stream':
            # 🔥 TwiML for Twilio→Retell bridge (POST handler)
        ...
        elif path == '/webhook/twilio-status':
            # 🔥 Twilio status callback for verified caller ID bridge
            try:
                q = parse_qs(urlparse(self.path).query)
                call_id = q.get('call_id', [''])[0]
                status = d.get('CallStatus', 'unknown')
                twilio_sid = d.get('CallSid', '')
                
                print(f"📞 Twilio status: {status} for call_id={call_id}, sid={twilio_sid}")
                ...
                if call_id and status:
                    try:
                        conn = sqlite3.connect(DB_PATH)
                        c = conn.cursor()
                        c.execute('UPDATE call_log SET status = ? WHERE call_id = ?', (status, call_id))
                        conn.commit()
                        conn.close()
                    except Exception as e:
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
        # 📱 RETELL SMS WEBHOOK - Captures appointments booked via text!
        # ═══════════════════════════════════════════════════════════════════════════════
        elif path == '/webhook/retell-sms':
            try:
                print(f"")
                print(f"📱" + "=" * 50)
                print(f"📱 RETELL SMS WEBHOOK RECEIVED")
                print(f"📱" + "=" * 50)
                print(f"📱 Data: {json.dumps(d, indent=2)[:1000]}")
                
                event = d.get('event', d.get('type', ''))
                chat_data = d.get('chat', d)
                ...
                # Get dynamic variables (contains name, address, phone)
                dynamic_vars = chat_data.get('retell_llm_dynamic_variables', {})
                metadata = chat_data.get('metadata', {})
                
                customer_name = dynamic_vars.get('first_name', dynamic_vars.get('name', ''))
                customer_phone = dynamic_vars.get('customer_phone', metadata.get('phone', ''))
                address = dynamic_vars.get('address', metadata.get('address', ''))
                
                # Get post-chat analysis (appointment data!)
                chat_analysis = chat_data.get('chat_analysis', {})
                custom_data = chat_analysis.get('custom_analysis_data', {})
                
                # Check for appointment booking
                appointment_booked = custom_data.get('appointment_booked', False)
                appointment_date = custom_data.get('appointment_date', '')
                appointment_time = custom_data.get('appointment_time', '')
                ...
                print(f"📱 Address: {address}")
                print(f"📱 Appointment Booked: {appointment_booked}")
                print(f"📱 Date: {appointment_date}")
                print(f"📱 Time: {appointment_time}")
                
                # If appointment was booked via text, create it in GHL!
                if appointment_booked or 'locked in' in transcript.lower() or 'see you' in transcript.lower():
                    print(f"")
                    print(f"🎉" + "=" * 50)
                    print(f"🎉 APPOINTMENT BOOKED VIA TEXT!")
                    print(f"🎉 {customer_name} at {address}")
                    print(f"🎉 {appointment_date} {appointment_time}")
                    print(f"🎉" + "=" * 50)
                    
                    # Try to find or create GHL contact
                    if customer_phone:
                        # Search for contact in GHL
                        contact = ghl_search_contact(customer_phone)
                        
                        if contact:
                            contact_id = contact.get('id', '')
                            
                            # Add tags
                            ghl_add_tag(contact_id, ['SMS Appointment', 'Hailey Text Booking', 'Hot Lead'])
                            
                            # Add note with appointment details
                            note = f"""🎉 APPOINTMENT BOOKED VIA TEXT!
                            
📅 Date: {appointment_date or 'See transcript'}
⏰ Time: {appointment_time or 'See transcript'}
📍 Address: {address}
📱 Booked by: Hailey SMS Agent
💬 Chat ID: {chat_id}

📝 Transcript:
{transcript[:500]}"""
                            
                            ghl_create_note(contact_id, note)
                            
                            # Move to appointment stage
                            try:
                                ghl_move_pipeline_stage(contact_id, 'appointment_set')
                            except:
                                pass
                            
                            # Send confirmation text (from same number!)
                            try:
                                confirm_msg = f"✅ You're all set! {appointment_date} at {appointment_time} at {address}. See you then! 👊"
                                # Don't send duplicate - the chat agent already confirmed
                            except:
                                pass
                            
                            print(f"✅ GHL updated for {customer_name}")
                        else:
                            print(f"⚠️ Could not find GHL contact for {customer_phone}")
                    
                    # 🔥 STOP SMS SEQUENCE - They booked!
                    if customer_phone:
                        stop_sms_sequence(phone=customer_phone, reason="booked")
                        print(f"📱 ✅ SMS sequence STOPPED - appointment booked!")
                    
                    # Alert owner about text booking
                    try:
                        owner_msg = f"🎉 TEXT BOOKING!\n{customer_name}\n{customer_phone}\n{address}\n{appointment_date} {appointment_time}"
                        send_owner_alert(owner_msg)
                        print(f"📱 ✅ OWNER ALERT SENT!")
                    except Exception as alert_err:
                        print(f"📱 ⚠️ Owner alert error: {alert_err}")
                
                # 🔥 STOP SMS SEQUENCE IF THEY RESPONDED (chat has transcript)
                elif transcript and len(transcript) > 10 and customer_phone:
                    # They responded to our text! Stop the sequence, let the chat agent handle it
                    stop_sms_sequence(phone=customer_phone, reason="responded")
                    print(f"📱 ✅ SMS sequence STOPPED - customer responded!")
                
                # Log to database
                try:
                    conn = sqlite3.connect(DB_PATH)
                    c = conn.cursor()
                    c.execute('''INSERT INTO sms_log (phone, message, message_type, status, twilio_sid) 
                                 VALUES (?, ?, ?, ?, ?)''',
                              (customer_phone, f"SMS Chat: {transcript[:200]}", 'retell_sms_chat', 
                               'appointment' if appointment_booked else chat_status, chat_id))
                    conn.commit()
                    conn.close()
                except Exception as db_err:
                ...
                print(f"❌ Retell SMS webhook error: {e}")
                import traceback
                traceback.print_exc()
                self.send_json({"success": False, "error": str(e)})
            ...
            try:
                ...
                try:
                    ...
                    conn.commit()
                    conn.close()
                    # Note: Google Sheets logging is done at the end after outcome is determined
                ...
                # Log final call data to Google Sheets with outcome
                if GOOGLE_SHEETS_ENABLED and 'outcome' in dir():
                    try:
                        log_call_to_sheets({
                            'call_id': call_id,
                            'lead_id': ghl_contact_id or '',
                            'name': customer_name if 'customer_name' in dir() else '',
                            'phone': to_number if 'to_number' in dir() else '',
                            'agent_type': agent_type if 'agent_type' in dir() else 'solar',
                            'status': friendly_status if 'friendly_status' in dir() else call_status,
                            'duration': duration_sec if 'duration_sec' in dir() else 0,
                            'outcome': outcome,
                            'sentiment': 'positive' if outcome == 'appointment_set' else ('negative' if outcome in ['not_interested', 'bad_credit'] else 'neutral'),
                            'recording_url': recording_url if 'recording_url' in dir() else '',
                            'answered': 1 if (duration_sec if 'duration_sec' in dir() else 0) > 30 else 0,
                            'notes': transcript[:200] if 'transcript' in dir() and transcript else ''
                        })
                        print(f"📊 Call logged to Sheets with outcome: {outcome}")
                    except Exception as sheet_err:
                        print(f"⚠️ Final Sheets log error: {sheet_err}")
...
def run_call_scheduler():
    """Background thread that processes scheduled calls AND SMS follow-ups every 60 seconds"""
    print("🕐 Call scheduler started - checking every 60 seconds")
    print("📱 SMS follow-up processor enabled - texting every 10 minutes until response!")
    while True:
        try:
            ...
            # 🔥 Process SMS follow-ups (old method)
            sms_result = process_sms_followups()
            if sms_result.get('processed', 0) > 0:
                print(f"📱 Scheduler sent {sms_result['processed']} SMS follow-ups")
            
            # 🔥 Process scheduled texts (new method)
            txt_result = process_scheduled_texts()
            if txt_result.get('processed', 0) > 0:
                print(f"📱 Scheduler sent {txt_result['processed']} scheduled texts")
...
def retell_webhook():
    ...
    try:
        ...
        # ----- STOP SEQUENCE IF BOOKED OR ANSWERED -----
        if metadata.get('sequence_id') and (booked or outcome == "Appointment Set"):
            try:
                conn = sqlite3.connect(DB_PATH, timeout=10)
                c = conn.cursor()
                c.execute('UPDATE call_sequences SET status = ? WHERE id = ?', 
                         ('appointment_set', metadata.get('sequence_id')))
                # Also delete any pending scheduled calls
                c.execute('DELETE FROM scheduled_calls WHERE sequence_id = ? AND status = ?',
                         (metadata.get('sequence_id'), 'pending'))
                conn.commit()
                conn.close()
                print(f"[RETELL WEBHOOK] ✅ Sequence STOPPED - Appointment booked!")
            except Exception as seq_err:
                print(f"[RETELL WEBHOOK] ⚠️ Could not stop sequence: {seq_err}")
        ...
        # ----- SCHEDULE NEXT CALL IF NO ANSWER -----
        if outcome in ["No Answer", "Voicemail", "Busy"] and metadata.get('sequence_id'):
            try:
                # Schedule next call in 2 hours
                result = schedule_next_call(metadata.get('sequence_id'))
                if result.get('success'):
                    print(f"[RETELL WEBHOOK] 📅 Next call scheduled: {result.get('scheduled_time')}")
                else:
                    print(f"[RETELL WEBHOOK] ⚠️ Could not schedule next call: {result.get('error')}")
            except Exception as sched_err:
                print(f"[RETELL WEBHOOK] ⚠️ Schedule error: {sched_err}")
...
def make_call_with_verified_caller_id(phone, name="there", agent_type="solar", ghl_contact_id=None, address=""):
    """
    🔥 BULLETPROOF SAME-NUMBER SOLUTION
    ...
    if US_ONLY_CALLS and not is_us_number(phone):
        print(f"🚫 BLOCKED: Non-US number {phone}")
        return {'success': False, 'error': 'Only US numbers allowed'}
    
    print(f"")
    print(f"🔥" + "=" * 50)
    print(f"🔥 VERIFIED CALLER ID BRIDGE")
    print(f"🔥 From: {VERIFIED_CALLER_ID} (Same as GHL SMS!)")
    print(f"🔥 To: {phone}")
    print(f"🔥 Agent: {agent_type}")
    print(f"🔥" + "=" * 50)
    
    # Get the right Retell agent
    ...
    company_name = "Bulldog Roofing" if agent_type == 'roofing' else ("All Access" if agent_type == 'solar' else "VoiceLab")
    if agent_type == 'roofing':
        opening = f"Hey, is this {name}?"
    elif address:
    ...
    try:
        ...
        print(f"📡 Step 1: Registering call with Retell...")
        ...
        if register_response.status_code not in [200, 201]:
            print(f"❌ Retell register failed: {register_response.status_code}")
            print(f"❌ Response: {register_response.text}")
            return {'success': False, 'error': f'Retell register failed: {register_response.text}'}
        
        register_data = register_response.json()
        call_id = register_data.get('call_id')
        
        print(f"✅ Retell call registered: {call_id}")
        
        # Step 2: Make outbound call via Twilio with verified caller ID
        print(f"📞 Step 2: Twilio calling {phone} from {VERIFIED_CALLER_ID}...")
        
        # TwiML that streams audio to Retell
        # We need to host this TwiML - use a webhook on our server
        twiml_url = f"https://www.voicelab.live/twiml/retell-stream?call_id={call_id}"
        
        twilio_response = requests.post(
            f"https://api.twilio.com/2010-04-01/Accounts/{YOUR_TWILIO_SID}/Calls.json",
            auth=(YOUR_TWILIO_SID, YOUR_TWILIO_AUTH),
            data={
                "From": VERIFIED_CALLER_ID,
                "To": phone,
                "Url": twiml_url,
                "StatusCallback": f"https://www.voicelab.live/webhook/twilio-status?call_id={call_id}",
                "StatusCallbackEvent": ["initiated", "ringing", "answered", "completed"]
            },
            timeout=15
        )
        
        if twilio_response.status_code not in [200, 201]:
            print(f"❌ Twilio call failed: {twilio_response.status_code}")
            print(f"❌ Response: {twilio_response.text}")
            return {'success': False, 'error': f'Twilio call failed: {twilio_response.text}'}
        
        twilio_data = twilio_response.json()
        twilio_call_sid = twilio_data.get('sid')
        
        print(f"✅ Twilio call initiated: {twilio_call_sid}")
        print(f"🔥 Customer will see: {VERIFIED_CALLER_ID}")
        
        # Log to database
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute('''INSERT INTO call_log (phone, status, call_id, agent_type, is_test_call, is_inbound, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?)''',
                      (phone, 'initiated', call_id, agent_type, 0, 0, datetime.now().isoformat()))
            conn.commit()
            conn.close()
        except Exception as e:
        ...
        return {
            ...
            'from_number': VERIFIED_CALLER_ID,
            'to_number': phone,
            'message': f'Call initiated from {VERIFIED_CALLER_ID} (same as SMS!)'
        }
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔊 TwiML ENDPOINT FOR RETELL STREAMING
# Twilio calls this URL, we return TwiML that streams audio to Retell
# ═══════════════════════════════════════════════════════════════════════════════

def handle_twiml_retell_stream(query_params):
    """
    Returns TwiML that tells Twilio to stream audio to Retell's WebSocket
    """
    call_id = query_params.get('call_id', [''])[0]
    
    print(f"🔊 TwiML request for Retell stream, call_id: {call_id}")
    
    # Retell's WebSocket URL for this call
    retell_ws_url = f"wss://api.retellai.com/audio-websocket/{call_id}"
    
    # TwiML that connects Twilio audio to Retell
    twiml = f'''<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Connect>
        <Stream url="{retell_ws_url}">
            <Parameter name="call_id" value="{call_id}"/>
        </Stream>
    </Connect>
</Response>'''
    
    print(f"🔊 Streaming to: {retell_ws_url}")
    
    return twiml


# ═══════════════════════════════════════════════════════════════════════════════
# 📱 RETELL SMS - Send texts from same number as calls!
# ═══════════════════════════════════════════════════════════════════════════════
...
def text_then_call(phone, name="there", agent_type="solar", ghl_contact_id=None, address="", delay_seconds=30):
    """
    🔥 NEPQ SEQUENCE: Text first, then call!
    1. Send intro text from Hailey (with name and address for personalization)
    ...
    print(f"")
    print(f"🔥" + "=" * 50)
    print(f"🔥 TEXT-THEN-CALL SEQUENCE (NEPQ)")
    print(f"🔥 Number: {HAILEY_PHONE_NUMBER} (same for text + call!)")
    print(f"🔥 To: {phone}")
    print(f"🔥 Name: {name}")
    print(f"🔥 Address: {address}")
    ...
    # Step 1: Send intro text with personalization (name, address)
    sms_result = send_retell_sms(phone, name=name, address=address)
    
    if not sms_result.get('success'):
        print(f"⚠️ SMS failed, calling anyway...")
    else:
        print(f"✅ Intro text sent! Waiting {delay_seconds} seconds before calling...")
    
    # Step 2: Schedule call after delay
    def make_delayed_call():
        time.sleep(delay_seconds)
        print(f"📞 Delay complete! Making call now...")
        make_call(phone, name=name, agent_type=agent_type, ghl_contact_id=ghl_contact_id, address=address)
    
    # Run in background thread so we don't block
    call_thread = threading.Thread(target=make_delayed_call)
    call_thread.start()
    ...
    return {
        ...
        'phone': phone,
        'from_number': HAILEY_PHONE_NUMBER,
        'message': f'Text sent, call scheduled in {delay_seconds} seconds'
    }


def text_then_call_immediate(phone, name="there", agent_type="solar", ghl_contact_id=None, address=""):
    """
    🔥 TEXT THEN IMMEDIATE CALL - No delay version
    Send text, immediately call (while they're reading the text!)
    """
    phone = format_phone(phone)
    
    print(f"🔥 TEXT + IMMEDIATE CALL to {phone}")
    print(f"   Name: {name}, Address: {address}")
    
    # Send quick intro text with personalization
    sms_result = send_retell_sms(phone, name=name, address=address)
    
    # Immediately make call
    call_result = make_call(phone, name=name, agent_type=agent_type, ghl_contact_id=ghl_contact_id, address=address)
