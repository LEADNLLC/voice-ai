...
VOICE — Always Answer
=====================
Your presence, even when you can't be.
VOICE makes sure opportunity never goes unanswered.

✨ PRESENCE - Every call answered, 24/7
🎯 CERTAINTY - No missed opportunities  
🔄 FOLLOW-UP - Relentless, professional persistence
📅 BOOKING - From conversation to calendar
👁️ VISIBILITY - Complete audit trail  
...
# ═══════════════════════════════════════════════════════════════════════════════
# UNIFIED PHONE NUMBER STRATEGY
# ═══════════════════════════════════════════════════════════════════════════════
# ONE number for EVERYTHING: Voice (via Retell) + SMS (via Twilio API)
#
# How it works:
# 1. Buy number in Twilio (+17208189512)
# 2. Create Elastic SIP Trunk in Twilio → points to Retell (sip:sip.retellai.com)
# 3. Import number into Retell dashboard as "Custom Telephony"
# 4. Voice calls route through Retell AI
# 5. SMS sends directly via Twilio API
# Same number. Unified experience. Text then call. Double tap.
# ═══════════════════════════════════════════════════════════════════════════════

# TWILIO - The source of truth for phone numbers
...
# THE ONE NUMBER - Used for BOTH voice (via Retell) AND SMS (via Twilio)
PRIMARY_PHONE = os.environ.get('PRIMARY_PHONE', '+17208189512')
TWILIO_PHONE = PRIMARY_PHONE  # Alias for SMS functions

# RETELL - AI Voice Engine (uses the same number via Custom Telephony/SIP)
RETELL_API_KEY = os.environ.get('RETELL_API_KEY', 'key_9128661dbc7b1c100bac91bbcf9b')

# Legacy configs (keeping for backwards compatibility during migration)
RETELL_PHONE_NUMBER = os.environ.get('RETELL_PHONE_NUMBER', PRIMARY_PHONE)
RETELL_INBOUND_NUMBER = os.environ.get('RETELL_INBOUND_NUMBER', PRIMARY_PHONE)
TWILIO_INBOUND_NUMBER = PRIMARY_PHONE
...
COMPANY_PHONE = os.environ.get('COMPANY_PHONE', PRIMARY_PHONE)
...
COST_PER_SMS = 0.0079  # Twilio SMS rate
FB_DAILY_BUDGET = 50.00
MONTHLY_BUDGET_GOAL = 2000.00
DAILY_BUDGET_GOAL = 100.00
DB_PATH = os.environ.get('DB_PATH', './voice.db')
...
# ═══════════════════════════════════════════════════════════════════════════════
# DATABASE HELPER - Prevents "database is locked" errors
# ═══════════════════════════════════════════════════════════════════════════════
...
def db_execute_with_retry(query, params=(), max_retries=3, fetch=False, fetchone=False):
    """Execute a database query with retry logic for lock errors"""
...
def init_db():
    ...
    # ═══════════════════════════════════════════════════════════════════
    # TCPA COMPLIANCE TABLES
    # ═══════════════════════════════════════════════════════════════════
    
    # Immutable consent ledger - append only, never delete
    c.execute("""CREATE TABLE IF NOT EXISTS consent_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT NOT NULL,
        lead_id INTEGER,
        consent_type TEXT DEFAULT 'opt_in',
        source TEXT,
        source_id TEXT,
        ip_address TEXT,
    ...
    # SMS Conversations
    c.execute("""CREATE TABLE IF NOT EXISTS sms_conversations (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT NOT NULL,
        lead_id INTEGER,
        contact_name TEXT,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS internal_dnc (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Phone number configuration (voice/SMS capabilities)
    c.execute("""CREATE TABLE IF NOT EXISTS phone_config (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        phone TEXT UNIQUE NOT NULL,
        name TEXT,
        ...
        sms_approved_at TIMESTAMP,
        notes TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )""")
    
    # Insert default phone configs if not exists
    c.execute("SELECT COUNT(*) FROM phone_config")
    if c.fetchone()[0] == 0:
        c.execute("""INSERT OR IGNORE INTO phone_config (phone, name, type, provider, voice_enabled, sms_enabled) VALUES
            ('+17208189512', 'Primary (Unified)', 'primary', 'twilio', 1, 1)
        """)
    
    # Contact attempt log (for frequency limiting)
    ...
    c.execute('''INSERT OR IGNORE INTO users (email, password_hash, salt, name, role, is_active, email_verified) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)''', 
              ('admin@voicelab.live', admin_hash, admin_salt, 'Admin', 'admin', 1, 1))
    
    # Create demo user with FIXED credentials (demo@demo.com / demo1234)
    # Using fixed salt so password always works
    demo_salt = 'demosalt1234567890abcdef'
    demo_hash = hashlib.sha256(('demo1234' + demo_salt).encode()).hexdigest()
    c.execute('''INSERT OR REPLACE INTO users (email, password_hash, salt, name, company, role, is_active, email_verified) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)''', 
              ('demo@demo.com', demo_hash, demo_salt, 'Demo User', 'VOICE Demo', 'admin', 1, 1))
    ...
    try:
        ...
        if result['expires_at']:
            try:
                ...
                pass  # If date parsing fails, consider valid
...
# ═══════════════════════════════════════════════════════════════════
# TCPA COMPLIANCE FUNCTIONS
# ═══════════════════════════════════════════════════════════════════

def log_consent(phone, lead_id=None, source='facebook', source_id=None, ip=None):
    """Log consent to immutable ledger"""
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("""INSERT INTO consent_log (phone, lead_id, source, source_id, ip_address, consent_text_hash)
                     VALUES (?, ?, ?, ?, ?, ?)""",
                  (phone_clean, lead_id, source, source_id, ip, 
                   hashlib.sha256(f"Consent to contact via {source}".encode()).hexdigest()[:16]))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def add_to_dnc(phone, reason='opt_out', source='sms_stop'):
    """Add phone to internal DNC"""
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO internal_dnc (phone, reason, source) VALUES (?, ?, ?)",
                  (phone_clean, reason, source))
        conn.commit()
        conn.close()
        print(f"Added to DNC: {phone_clean} ({reason})")
        return True
    except:
        return False

def is_on_dnc(phone):
    """Check if phone is on internal DNC"""
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT 1 FROM internal_dnc WHERE phone = ?", (phone_clean,))
...
def get_contact_count(phone, hours=24):
    """Get number of contact attempts in last N hours"""
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("""SELECT COUNT(*) FROM contact_log 
                     WHERE phone = ? AND created_at > datetime('now', ?)""",
                  (phone_clean, f'-{hours} hours'))
        count = c.fetchone()[0]
        conn.close()
        return count
    except:
...
def log_contact_attempt(phone, contact_type='call', status='initiated', agent_id=None):
    """Log a contact attempt"""
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("INSERT INTO contact_log (phone, contact_type, status, agent_id) VALUES (?, ?, ?, ?)",
                  (phone_clean, contact_type, status, agent_id))
        conn.commit()
        conn.close()
    except:
        pass

def get_timezone_from_phone(phone):
    """Get timezone offset from area code (returns hours offset from UTC)"""
    phone_clean = re.sub(r'[^0-9]', '', phone)
    if len(phone_clean) >= 10:
        area_code = phone_clean[-10:-7] if len(phone_clean) >= 10 else phone_clean[:3]
        # Pacific: UTC-8 (UTC-7 DST)
        pacific = ['206','209','213','253','310','323','341','408','415','424','442','458','503','509','510','530','541','559','562','619','626','628','650','657','661','669','707','714','747','760','775','805','818','831','858','909','916','925','949','951']
        # Mountain: UTC-7 (UTC-6 DST)
        mountain = ['303','307','385','435','480','505','520','602','623','719','720','801','928']
        # Central: UTC-6 (UTC-5 DST)
        central = ['210','214','254','281','312','314','316','318','319','325','346','361','402','405','409','417','430','432','469','501','504','507','512','515','539','563','573','580','601','605','608','612','615','618','620','630','636','641','651','660','662','682','701','708','712','713','715','716','731','737','763','769','785','806','816','817','830','832','847','850','870','901','903','913','915','918','920','936','940','956','972','979']
        
        if area_code in pacific:
            return -8  # Pacific
        elif area_code in mountain:
            return -7  # Mountain
        elif area_code in central:
            return -6  # Central
        else:
            return -5  # Eastern (default)
    return -5  # Default to Eastern

def is_valid_call_time(phone):
    """Check if current time is valid for calling (8am-9pm local)"""
    from datetime import datetime, timezone, timedelta
    try:
        utc_now = datetime.now(timezone.utc)
        offset_hours = get_timezone_from_phone(phone)
        local_time = utc_now + timedelta(hours=offset_hours)
        hour = local_time.hour
        return 8 <= hour < 21  # 8am to 9pm
    except:
        pass
    # Default: be conservative (9am-8pm Eastern)
    hour = datetime.now().hour
    return 9 <= hour < 20

def pre_flight_check(phone, contact_type='call'):
    """
    Run all compliance checks before making contact.
    Returns (allowed: bool, reason: str)
    """
    phone_clean = re.sub(r'[^0-9+]', '', phone)
    
    # 1. Check internal DNC
    if is_on_dnc(phone_clean):
        return False, "DNC: Phone is on Do Not Call list"
    
    # 2. Check time window
    if not is_valid_call_time(phone_clean):
        return False, "TIME: Outside calling hours (8am-9pm local)"
    
    # 3. Check daily frequency (max 3 per day)
    daily_count = get_contact_count(phone_clean, hours=24)
    if daily_count >= 3:
        return False, f"FREQ: Already contacted {daily_count}x in 24hrs (max 3)"
    
    # 4. Check weekly frequency (max 7 per week)
    weekly_count = get_contact_count(phone_clean, hours=168)
    if weekly_count >= 7:
        return False, f"FREQ: Already contacted {weekly_count}x in 7 days (max 7)"
    
    # All checks passed
    return True, "OK"

def process_sms_keywords(phone, message):
    """Process STOP/START/HELP keywords"""
    msg_upper = message.strip().upper()
    
    if msg_upper in ['STOP', 'UNSUBSCRIBE', 'CANCEL', 'END', 'QUIT']:
        add_to_dnc(phone, 'opt_out', 'sms_stop')
        return True, "You have been unsubscribed. Reply START to resubscribe."
    
    if msg_upper in ['START', 'SUBSCRIBE', 'YES']:
        # Remove from DNC
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("DELETE FROM internal_dnc WHERE phone = ?", (re.sub(r'[^0-9+]', '', phone),))
            conn.commit()
            conn.close()
        except:
            pass
        return True, "You have been resubscribed to messages."
    
    if msg_upper in ['HELP', 'INFO']:
        return True, f"Reply STOP to unsubscribe. For support call {COMPANY_PHONE}."
...
def get_sms_conversations():
    """Get all SMS conversations with enhanced data for Conversation Intelligence Hub"""
    try:
        conn = get_db_connection()
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        c.execute("""
            SELECT c.*, 
                   (SELECT message FROM sms_messages WHERE conversation_id = c.id ORDER BY sent_at DESC LIMIT 1) as last_message,
                   (SELECT COUNT(*) FROM sms_messages WHERE conversation_id = c.id AND direction = 'inbound') as inbound_count,
                   (SELECT COUNT(*) FROM sms_messages WHERE conversation_id = c.id AND direction = 'outbound') as outbound_count,
                   l.id as lead_db_id, l.first_name as lead_first_name, l.last_name as lead_last_name, 
                   l.status as lead_status, l.agent_type as lead_agent_type, l.city as lead_city, l.state as lead_state
            FROM sms_conversations c
            LEFT JOIN leads l ON c.lead_id = l.id OR c.phone = l.phone
        ...
        for row in c.fetchall():
            ...
            if convo.get('lead_db_id'):
                convo['lead'] = {
                    'id': convo.pop('lead_db_id'),
                    'first_name': convo.pop('lead_first_name', ''),
                    'last_name': convo.pop('lead_last_name', ''),
                    'status': convo.pop('lead_status', 'new'),
                    'agent_type': convo.pop('lead_agent_type', ''),
                    'city': convo.pop('lead_city', ''),
                    'state': convo.pop('lead_state', '')
                }
            else:
                # Remove the extra fields
                for k in ['lead_db_id', 'lead_first_name', 'lead_last_name', 'lead_status', 'lead_agent_type', 'lead_city', 'lead_state']:
                    convo.pop(k, None)
            convos.append(convo)
        conn.close()
        return convos
    except Exception as e:
        print(f"get_sms_conversations error: {e}")
        import traceback
        traceback.print_exc()
        return []

def get_sms_conversation(convo_id):
    """Get single conversation with messages and linked lead info"""
    try:
        conn = get_db_connection()
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        ...
        convo['inbound_count'] = len([m for m in convo['messages'] if m.get('direction') == 'inbound'])
        convo['outbound_count'] = len([m for m in convo['messages'] if m.get('direction') == 'outbound'])
        ...
        if not lead and convo.get('phone'):
            phone = convo['phone']
            phone_digits = ''.join(filter(str.isdigit, phone))[-10:]
            c.execute("SELECT * FROM leads WHERE phone LIKE ?", (f"%{phone_digits}%",))
            lead_row = c.fetchone()
            if lead_row:
                lead = dict(lead_row)
                # Auto-link
        ...
        print(f"get_sms_conversation error: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}

def create_sms_conversation(phone, contact_name=None, initial_message=None):
    """Create new SMS conversation and optionally send initial message"""
    if not phone:
    ...
    try:
        ...
        # Check existing
        c.execute("SELECT id FROM sms_conversations WHERE phone = ?", (phone_clean,))
        existing = c.fetchone()
        ...
        if existing:
            ...
            if contact_name:
                c.execute("UPDATE sms_conversations SET contact_name = ? WHERE id = ?", (contact_name, convo_id))
        else:
            # Try to find lead by phone to get name
            if not contact_name:
                phone_digits = ''.join(filter(str.isdigit, phone_clean))[-10:]
                c.execute("SELECT first_name, last_name, id FROM leads WHERE phone LIKE ?", (f"%{phone_digits}%",))
                lead_row = c.fetchone()
                if lead_row:
                    contact_name = f"{lead_row['first_name'] or ''} {lead_row['last_name'] or ''}".strip()
            
            c.execute("INSERT INTO sms_conversations (phone, contact_name) VALUES (?, ?)",
                      (phone_clean, contact_name or ''))
            convo_id = c.lastrowid
        
        conn.commit()
        conn.close()
        
        # Send initial message if provided - use the proper send function
        if initial_message:
            send_result = send_sms_to_conversation(convo_id, phone_clean, initial_message)
            return {"id": convo_id, "success": True, "message_sent": send_result.get('success', False)}
        
        return {"id": convo_id, "success": True}
    except Exception as e:
        print(f"create_sms_conversation error: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}

def send_sms_to_conversation(conversation_id, phone, message):
    """Send SMS to conversation via Twilio with delivery tracking"""
    if not message:
    ...
    try:
        ...
        if TWILIO_SID and TWILIO_TOKEN and TWILIO_PHONE:
            try:
                ...
                # In production, this should be your Railway URL
                ...
                if base_url:
                    twilio_data["StatusCallback"] = f"{base_url}/api/twilio/status"
                ...
                if response.status_code == 201:
                    ...
                    log_cost('sms', COST_PER_SMS, f'Conversation SMS to {phone}')
                    print(f"📱 SMS sent to {phone}: {twilio_sid}")
                else:
                    print(f"❌ Twilio error: {response.text}")
                    status = 'failed'
            except Exception as te:
                print(f"❌ Twilio send error: {te}")
                status = 'failed'
        else:
        ...
        c.execute("INSERT INTO sms_messages (conversation_id, direction, message, status, twilio_sid) VALUES (?, 'outbound', ?, ?, ?)",
                  (conversation_id, message, status, twilio_sid))
        message_id = c.lastrowid
        c.execute("UPDATE sms_conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", (conversation_id,))
        
        conn.commit()
        conn.close()
        
        # Log contact attempt for compliance
        log_contact_attempt(phone, 'sms', status)
        
        return {"success": status not in ['failed'], "message_id": message_id, "status": status, "twilio_sid": twilio_sid}
    except Exception as e:
...
def send_lead_welcome_sms(phone, name):
    """Send welcome SMS to new lead with booking link - sent BEFORE the AI call"""
    # Use first name only, default to "there" if empty
    first_name = name.split()[0] if name else "there"
    
    # Format the phone number for display
    display_phone = PRIMARY_PHONE
    if len(PRIMARY_PHONE) == 12:  # +1XXXXXXXXXX
        display_phone = f"({PRIMARY_PHONE[2:5]}) {PRIMARY_PHONE[5:8]}-{PRIMARY_PHONE[8:]}"
    
    msg = f"""Hi {first_name}, Hailey here locally — most people who check this out are just making sure their home and family are protected.

If it's easier, call/text {display_phone} or book here: {CALENDLY_LINK}"""
    
    if TWILIO_SID and TWILIO_TOKEN and phone:
        result = send_sms(phone, msg, "lead_welcome")
        if result.get('success'):
            print(f"✅ Welcome SMS sent to {first_name} at {phone}")
        return result
    return {"success": False, "error": "SMS not configured"}


def send_lead_intro_sms(phone, name, industry="roofing"):
    """Send intro SMS to new lead - personalized by industry"""
    first_name = name.split()[0] if name else "there"
    ...
    industry_messages = {
        'roofing': "most people who check this out are just making sure their home and family are protected",
        'solar': "a lot of homeowners are curious about cutting their electric bill in half",
        'hvac': "most folks reaching out want to make sure their home stays comfortable year-round",
        'medical': "we know your health matters, and we're here to help you get the care you need",
        'dental': "we're here to help you get that smile you've been wanting",
        'legal': "we understand legal matters can be stressful — we're here to help",
        'real_estate': "finding the right home is a big decision — we're here to guide you",
        'insurance': "protecting what matters most to you is our priority",
        'auto': "getting you into the right vehicle is what we do best"
    }
    
    context = industry_messages.get(industry.lower(), "most people who check this out are looking for a better solution")
    
    msg = f"""Hi {first_name}, Hailey here locally — {context}.

If it's easier, call/text {display_phone} or book here: {CALENDLY_LINK}"""
    
    if TWILIO_SID and TWILIO_TOKEN and phone:
        result = send_sms(phone, msg, "lead_intro")
        if result.get('success'):
            print(f"✅ Intro SMS sent to {first_name} at {phone}")
        return result
    return {"success": False, "error": "SMS not configured"}


def send_appointment_confirmation_sms(phone, name, appt_date, appt_time):
    """Send appointment confirmation SMS when someone books via Calendly"""
    first_name = name.split()[0] if name else "there"
    ...
    try:
        ...
        formatted_date = dt.strftime('%A, %B %d')  # e.g., "Monday, January 15"
    except:
        pass
...
If you need to reschedule, call/text {display_phone}

See you soon!
- Hailey"""
    
    if TWILIO_SID and TWILIO_TOKEN and phone:
        result = send_sms(phone, msg, "appointment_confirmation")
        if result.get('success'):
            print(f"   ✅ Confirmation SMS sent to {first_name} at {phone}")
        return result
    return {"success": False, "error": "SMS not configured"}


# ═══════════════════════════════════════════════════════════════════════════════
# COMPLIANCE TRACKING & DOUBLE-TAP FUNCTIONS
# ═══════════════════════════════════════════════════════════════════════════════

def update_compliance_step(lead_id, step):
    """Update compliance tracking for a lead"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        timestamp = datetime.now().isoformat()
        
        # Map step names to column updates
        step_columns = {
            'text_sent': 'text_sent_at',
            'call_1': 'call_1_at',
            'call_2': 'call_2_at',
            'call_1215': 'call_1215_at',
            'call_1215_double': 'call_1215_double_at',
            'call_530': 'call_530_at',
            'call_530_double': 'call_530_double_at',
            'appointment_booked': 'appointment_booked_at',
            'contacted': 'contacted_at'
        }
        
        col = step_columns.get(step)
        if col:
            # Check if column exists, if not add it
            c.execute(f"PRAGMA table_info(leads)")
            columns = [row[1] for row in c.fetchall()]
            if col not in columns:
                c.execute(f"ALTER TABLE leads ADD COLUMN {col} TEXT")
            
            c.execute(f"UPDATE leads SET {col} = ? WHERE id = ?", (timestamp, lead_id))
            conn.commit()
            print(f"   📋 Compliance updated: {step} at {timestamp}")
        
        conn.close()
    except Exception as e:
        print(f"   ⚠️ Compliance update error: {e}")


def store_pending_call(lead_id, phone, name, agent_type, call_id, attempt_num):
    """Store a pending call for double-tap tracking"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        
        # Create pending_calls table if not exists
        c.execute('''CREATE TABLE IF NOT EXISTS pending_calls (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            lead_id INTEGER,
            phone TEXT,
            name TEXT,
        ...
        c.execute('''INSERT OR REPLACE INTO pending_calls 
                     (lead_id, phone, name, agent_type, call_id, attempt_num, status)
                     VALUES (?, ?, ?, ?, ?, ?, 'pending')''',
                  (lead_id, phone, name, agent_type, call_id, attempt_num))
        conn.commit()
        conn.close()
        print(f"   📝 Stored pending call: {call_id} (attempt {attempt_num})")
    except Exception as e:
        print(f"   ⚠️ Store pending call error: {e}")


def handle_call_completion(call_id, outcome, duration=0):
    """Handle call completion - trigger double-tap if no answer"""
    try:
        conn = get_db_connection()
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        
        # ALWAYS update call_log with outcome and duration
        c.execute('''UPDATE call_log SET status = ?, outcome = ?, duration = ?, 
                     answered = ?, voicemail = ?, no_answer = ?
                     WHERE call_id = ?''', 
                  (outcome, outcome, int(duration),
                   1 if outcome in ['answered', 'completed', 'user_hangup', 'agent_hangup'] else 0,
                   1 if outcome in ['voicemail', 'voicemail_reached', 'machine'] else 0,
                   1 if outcome in ['no_answer', 'busy', 'failed'] else 0,
                   call_id))
        
        # Get the pending call info
        c.execute('SELECT * FROM pending_calls WHERE call_id = ?', (call_id,))
        row = c.fetchone()
        
        if not row:
            conn.commit()
            conn.close()
            print(f"   ⚠️ No pending call found for: {call_id} (call_log updated)")
            return
        ...
        attempt_num = pending['attempt_num']
        
        # Update the pending call status
        c.execute('''UPDATE pending_calls 
                     SET status = 'completed', completed_at = CURRENT_TIMESTAMP, call_outcome = ?
                     WHERE call_id = ?''', (outcome, call_id))
        conn.commit()
        ...
        # Check if we need to double-tap (call again)
        no_answer_outcomes = ['no_answer', 'voicemail', 'voicemail_reached', 'busy', 'failed', 'no-answer', 'machine']
        answered_outcomes = ['answered', 'completed', 'user_hangup', 'agent_hangup', 'call_ended']
        
        if outcome.lower() in no_answer_outcomes or (duration < 15 and outcome.lower() not in answered_outcomes):
            # NO ANSWER - DOUBLE TAP!
            if attempt_num < 2:
                print(f"   🔄 NO ANSWER - INITIATING DOUBLE TAP...")
                conn.close()  # Close connection before making call
                
                # Brief delay before double-tap
                import time
                time.sleep(3)
                
                # Make the second call immediately
                call_result = make_call(phone, name, agent_type=agent_type, is_test=False)
                
                if call_result.get('success'):
                    new_call_id = call_result.get('call_id')
                    print(f"   ✅ Double-tap call initiated: {new_call_id}")
                    
                    # Update compliance tracking
                    update_compliance_step(lead_id, 'call_2')
                    
                    # Store for tracking
                    store_pending_call(lead_id, phone, name, agent_type, new_call_id, 2)
                else:
                    print(f"   ❌ Double-tap failed: {call_result.get('error')}")
            else:
                print(f"   ⏹️ Already attempted {attempt_num} times, not double-tapping again")
                conn.close()
        else:
            # ANSWERED - Update lead status to contacted
            print(f"   ✅ CALL ANSWERED - Updating lead status")
            c.execute("UPDATE leads SET status = 'contacted', updated_at = CURRENT_TIMESTAMP WHERE id = ?", (lead_id,))
            update_compliance_step(lead_id, 'contacted')
            conn.commit()
            conn.close()
        
    except Exception as e:
        print(f"   ❌ Handle call completion error: {e}")
        import traceback
        traceback.print_exc()
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False):
    ...
    # BOTH inbound and outbound now use the SAME number via Custom Telephony
    if is_inbound:
        retell_agent_id = 'agent_862cd6cf87f7b4d68a6986b3e9'  # Paige INBOUND
        from_number = PRIMARY_PHONE  # +17208189512 (unified number)
        call_type = "inbound"
    ...
    try:
        ...
        # Include voicemail detection and auto-hangup settings
        call_config = {
            "agent_id": retell_agent_id,
            "from_number": from_number,  # Uses correct number based on inbound/outbound
            "to_number": phone,
            # ═══════════════════════════════════════════════════════════════
            # CALL SETTINGS - Voicemail detection & auto-hangup
            # ═══════════════════════════════════════════════════════════════
            "voicemail_detection": "true",  # Enable AMD (Answering Machine Detection)
            "voicemail_action": "hangup",   # Hang up if voicemail detected
            "max_call_duration_ms": 600000,  # 10 minute max call (600,000ms)
            "end_call_after_silence_ms": 10000,  # End call after 10s silence
            # Webhook to receive call completion events
            "webhook_url": f"https://voicelab.live/api/retell/call-completed",
            ...
            "retell_llm_dynamic_variables": {
                ...
                # Booking link for the agent to share
                "booking_link": CALENDLY_LINK,
                "phone_number": PRIMARY_PHONE,
                # Instructions for ending calls properly
                "call_ending_instructions": "When the conversation is complete, say a friendly goodbye like 'Thanks so much, have a great day! Goodbye.' and END THE CALL. Do not wait for the customer to hang up."
            }
        }
        ...
        if response.status_code in [200, 201]:
            ...
            # Log the call - use timeout-enabled connection
            conn = get_db_connection()
            c = conn.cursor()
            c.execute('INSERT INTO call_log (phone, status, call_id, agent_type, is_test_call, is_inbound, is_live) VALUES (?, ?, ?, ?, ?, ?, ?)',
                      (phone, 'initiated', call_id, agent_type, 1 if is_test else 0, 1 if is_inbound else 0, 0 if is_test else 1))
            conn.commit()
            conn.close()
...
def get_lead_pipeline():
    ...
    try:
        ...
        # Get all leads - LEFT JOIN so we get leads even without sequences
        c.execute('''
        ...
        # Organize by status - use lead.status as fallback
        pipeline = {
        ...
        for lead in leads:
            seq_status = lead.get('sequence_status')
            lead_status = lead.get('status', 'new')
            slot = lead.get('current_slot', 0)
            
            # Check lead status first, then sequence status
            if lead_status == 'appointment_set' or seq_status == 'appointment_set':
                pipeline['appointment'].append(lead)
            elif lead_status == 'contacted':
                pipeline['contacted'].append(lead)
            elif lead_status == 'not_interested' or seq_status == 'not_interested':
                ...
                # Default: new leads go to 'new' bucket
...
<title>VOICE — Always Answer</title>
<style>
...
<div class="admin-layout">
    <aside class="sidebar">
        ...
        <div class="sidebar-section">Presence</div>
...
def get_conversation_with_lead_info(convo_id):
    ...
    try:
        ...
        print(f"get_conversation_with_lead_info error: {e}")
        import traceback
        traceback.print_exc()
        return {"error": str(e)}
...
<meta http-equiv="Content-Security-Policy" content="default-src 'self' 'unsafe-inline' 'unsafe-eval' https: data: blob: wss:; script-src 'self' 'unsafe-inline' 'unsafe-eval' https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: https: blob:; connect-src 'self' https: wss:;">
<title>VOICE — Always Answer</title>
<meta name="description" content="VOICE makes sure opportunity never goes unanswered. Your presence, even when you can't be."
...
<section class="hero">
<div class="hero-badge"><span></span> Every Opportunity Answered</div>
<h1>Always<br>Answer.</h1>
<p class="hero-sub">VOICE is your presence when you can't be — answering, qualifying, and following up so opportunity never goes silent.</p>
...
<div class="stats">
<div class="stat"><div class="stat-value">60s</div><div class="stat-label">Response Time</div></div>
<div class="stat"><div class="stat-value">24/7</div><div class="stat-label">Always Present</div></div>
<div class="stat"><div class="stat-value">100%</div><div class="stat-label">Calls Answered</div></div>
<div class="stat"><div class="stat-value">0</div><div class="stat-label">Missed Opportunities</div></div>
</div>
</section>
...
<div class="section-label">Why VOICE</div>
<h2 class="section-title">Businesses don't lose deals because they're bad.<br>They lose because they weren't there.</h2>
<div class="features-grid">
<div class="feature">
<div class="feature-icon">⚡</div>
<h3>Instant Response</h3>
<p>Every inquiry answered within 60 seconds. No more "I'll call them back later." No more missed moments.</p>
</div>
<div class="feature">
<div class="feature-icon">🌙</div>
<h3>Always Present</h3>
<p>3am inquiry? Holiday weekend? VOICE answers. Your business never sleeps, never takes breaks, never goes silent.</p>
</div>
<div class="feature">
<div class="feature-icon">🔄</div>
<h3>Relentless Follow-Up</h3>
<p>The fortune is in the follow-up. VOICE reaches out until they answer — politely, persistently, professionally.</p>
</div>
<div class="feature">
<div class="feature-icon">📅</div>
<h3>Seamless Booking</h3>
<p>From conversation to calendar in one call. No friction, no delays, no dropped balls.</p>
</div>
<div class="feature">
<div class="feature-icon">👁️</div>
<h3>Complete Visibility</h3>
<p>Every call, every outcome, every opportunity — tracked and visible. Nothing falls through the cracks.</p>
</div>
<div class="feature">
<div class="feature-icon">🎯</div>
<h3>Every Industry</h3>
<p>Roofing to real estate, dental to legal. VOICE adapts to your business, speaks your language.</p>
</div>
</div>
</section>
...
<h2 class="section-title">Your Presence. Every Industry.</h2>
<p style="color:var(--gray-400);max-width:600px;margin:0 auto">VOICE adapts to speak your language — whether you're in roofing or real estate, dental or legal.</p>
...
<div class="section-label">Experience It</div>
<h2 class="section-title">Hear presence in action</h2>
<p class="demo-sub">Enter your phone. Pick your industry. Experience what it means to always answer.</p>
...
<h2 class="section-title">What happens when you never miss a call</h2>
<div class="reviews-stats">
<div class="reviews-stat"><div class="reviews-stat-value">4.9</div><div class="reviews-stat-label">Rating</div></div>
<div class="reviews-stat"><div class="reviews-stat-value">2,500+</div><div class="reviews-stat-label">Businesses</div></div>
<div class="reviews-stat"><div class="reviews-stat-value">1.2M+</div><div class="reviews-stat-label">Answered</div></div>
</div>
</div>
<div class="reviews-grid">
<div class="review-card">
<div class="review-stars">★★★★★</div>
<div class="review-text">"We stopped missing leads. That's it. That's the whole story. We went from 8 appointments a week to 47 just by being there when people called."</div>
...
<h2 class="section-title">What's the cost of being absent?</h2>

<div class="pricing-box">
<div class="pricing-headline">Presence at Scale</div>
<div class="pricing-sub">Every missed call is a missed opportunity. VOICE ensures you're always there — 24/7, 365 days a year.</div>

<div class="pricing-features-list">
<div class="pricing-feature-item">Always Present</div>
<div class="pricing-feature-item">Instant Response</div>
<div class="pricing-feature-item">Relentless Follow-Up</div>
<div class="pricing-feature-item">Seamless Booking</div>
<div class="pricing-feature-item">Complete Visibility</div>
<div class="pricing-feature-item">Industry Fluent</div>
<div class="pricing-feature-item">Dedicated Support</div>
<div class="pricing-feature-item">Peace of Mind</div>
</div>

<a href="#" onclick="openBookingModal(); return false;" class="pricing-cta-btn">Start Being Present</a>
<p class="pricing-note">Free demo • No commitment • Live in 24 hours</p>
</div>
</div>
</section>
...
<p>VOICE makes sure someone always answers.</p>
<a href="#" onclick="openBookingModal(); return false;" class="btn-primary">Experience VOICE →</a>
</section>
...
<a href="#features">Why VOICE</a>
<a href="#agents">Industries</a>
<a href="#pricing">Pricing</a>
<a href="/app">Dashboard</a>
</div>
<div class="footer-copy">© 2025 VOICE. Always Answer.</div>
</footer>
...
<div class="booking-title">Experience Presence</div>
<div class="booking-sub">See what it means to never miss another opportunity</div>
</div>
...
async function startHeroCall() {
    ...
    try {
        ...
        const response = await fetch('/api/demo-call', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ phone: formattedPhone, agent_type: agent, is_inbound: heroIsInbound })
        });
...
body.logged-out .sidebar,body.logged-out .main,body.logged-out #aria-btn{display:none !important}body.logged-out .modal-bg#login-modal{display:flex !important}
...
async function initAuth() {
    const token = localStorage.getItem('voice_session');
    if (token) {
        try {
            const r = await fetch('/api/me', {headers: {'Authorization': 'Bearer ' + token}}).then(r => r.json());
            if (r && r.id) {
                currentUser = r;
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',async function(){await initAuth();setupNav();init()})}
else{setupNav();init()}
function load(p){
...
else if(p==='compliance')loadCompliance().catch(function(e){console.error('loadCompliance failed:',e)});
...
loadAppts();
}catch(e){console.error('saveDispo error:',e);toast('Error saving disposition',true)}
}

// ═══════════════════════════════════════════════════════════════════════════════
// CONVERSATION INTELLIGENCE HUB - VOICE 2.0 (10× Better)
// "This is not SMS. This is Conversation Intelligence."
// ═══════════════════════════════════════════════════════════════════════════════
...
let selectedGoal = 'continue';
let searchQuery = '';

// NEPQ-Inspired Message Templates (Much Better)
const MESSAGE_TEMPLATES = {
    opener: "Hi {name}! 👋 This is Hailey with VOICE. Thanks for reaching out — I'd love to help. What's the best way I can assist you today?",
    followup: "Hey {name}! Just wanted to follow up on our conversation. I know things get busy — is there anything I can help clarify or answer for you?",
    booking: "Hi {name}! 📅 I'd love to schedule a quick call to see how we can help. Would you prefer morning or afternoon? I'll work around your schedule.",
    checkin: "Hey {name}! Hope your week is going well. Just checking in — no pressure at all. Is there anything on your mind I can help with?",
    reengage: "Hi {name}! It's been a bit since we last connected. Just wanted to reach out and see if your situation has changed at all. I'm here whenever you're ready! 🙂",
    price: "I totally understand wanting to know the investment. The best part? We can customize everything to fit your budget. Would a quick call help me understand your needs better?",
    spouse: "Of course — that's a big decision to make together. Would it help if I sent over some info you could review with them? No rush at all.",
    timing: "I completely understand timing is everything. When would be a better time for us to connect? I'm flexible with my schedule.",
    objection: "I hear you, and I appreciate you being honest with me. Can I ask — what's the main thing holding you back? I just want to make sure I'm not missing something that could help."
};

// Enhanced Intent Detection
const INTENT_PATTERNS = {
    price: ['price', 'cost', 'how much', 'expensive', 'afford', 'budget', 'quote', '$', 'money', 'fee', 'payment', 'financing'],
    timing: ['when', 'how long', 'schedule', 'available', 'soon', 'today', 'tomorrow', 'week', 'busy', 'time'],
    info: ['what', 'how does', 'tell me', 'explain', 'information', 'details', 'learn', 'more about', 'work'],
    objection: ['not sure', 'think about', 'spouse', 'husband', 'wife', 'partner', 'later', 'maybe', 'idk', 'dont know'],
    ready: ['yes', 'interested', 'sounds good', "let's do it", 'ready', 'sign up', 'book', 'set up', 'start', 'proceed', 'definitely']
};

// Emotion Detection Patterns
const EMOTION_PATTERNS = {
    curious: ['?', 'how', 'what', 'why', 'tell me', 'wondering', 'curious'],
    hesitant: ['maybe', 'not sure', 'idk', 'possibly', 'might', 'thinking'],
    urgent: ['asap', 'urgent', 'emergency', 'now', 'today', 'immediately', 'help'],
    positive: ['great', 'awesome', 'perfect', 'love', 'excited', 'yes', 'thanks', '!'],
    negative: ['no', "can't", "won't", 'disappointed', 'frustrated', 'annoyed', 'upset']
};

async function loadMessages() {
    console.log('🔄 Loading conversations...');
    try {
        const convos = await fetch('/api/sms/conversations').then(r => r.json()).catch(e => { 
            console.error('fetch error:', e); 
            return []; 
        });
        console.log('✅ Loaded', convos.length, 'conversations');
        
        // Analyze and sort by priority (revenue impact)
        smsConvos = convos.map(c => analyzeConversation(c)).sort((a, b) => {
            // Priority: unread > hot > follow-up > replied > new > stale > booked
            const priority = {hot: 6, unread: 5, 'follow-up': 4, replied: 3, new: 2, booked: 1, stale: 0};
            const aPri = (a.unread_count > 0 ? 5 : 0) + (priority[a.status] || 0);
            const bPri = (b.unread_count > 0 ? 5 : 0) + (priority[b.status] || 0);
            return bPri - aPri;
        });
        
        renderConvoList(filterConvosByStatus(smsConvos, convoFilter));
    } catch(e) { 
        console.error('❌ loadMessages error:', e); 
    }
}

function analyzeConversation(convo) {
    const lastMsg = (convo.last_message || '').toLowerCase();
    const hoursSinceUpdate = convo.updated_at ? (Date.now() - new Date(convo.updated_at)) / 3600000 : 999;
    const hasReplied = (convo.inbound_count || 0) > 0;
    ...
    for (const [key, patterns] of Object.entries(INTENT_PATTERNS)) {
        for (const pattern of patterns) {
            if (lastMsg.includes(pattern)) {
                intent = key;
                intentScore = key === 'ready' ? 0.95 : key === 'price' ? 0.8 : key === 'objection' ? 0.4 : 0.7;
                break;
            }
        }
    ...
    // Emotion Analysis
    let emotion = 'neutral';
    for (const [key, patterns] of Object.entries(EMOTION_PATTERNS)) {
        for (const pattern of patterns) {
            if (lastMsg.includes(pattern)) {
                emotion = key;
                break;
    ...
    let status = 'new';
    if (convo.lead?.status === 'appointment_set') {
        status = 'booked';
    } else if (intent === 'ready' || intentScore > 0.85) {
        status = 'hot';
    } else if (hoursSinceUpdate > 48) {
        status = 'stale';
    } else if (hasReplied && hoursSinceUpdate > 4) {
        status = 'follow-up';
    } else if (hasReplied) {
        status = 'replied';
    }
    ...
    // SLA Calculation
    let slaStatus = 'ok';
    let slaMinutes = Math.round(hoursSinceUpdate * 60);
    if (hasReplied && hoursSinceUpdate > 1) slaStatus = 'warning';
    if (hasReplied && hoursSinceUpdate > 4) slaStatus = 'urgent';
    
    // Lead Score (0-100)
    let score = 50;
    if (intent === 'ready') score = 95;
    else if (intent === 'price') score = 75;
    else if (intent === 'info') score = 65;
    else if (intent === 'timing') score = 60;
    else if (intent === 'objection') score = 40;
    if (hasReplied) score += 15;
    if (hoursSinceUpdate < 1) score += 10;
    ...
    return {
        ...
        slaMinutes,
        score,
        scoreLevel: score >= 70 ? 'high' : score >= 45 ? 'medium' : 'low'
    };
}

function filterConvosByStatus(convos, filter) {
    let filtered = convos;
    if (filter === 'unread') filtered = convos.filter(c => (c.unread_count || 0) > 0);
    else if (filter === 'hot') filtered = convos.filter(c => c.status === 'hot' || c.score >= 75);
    else if (filter !== 'all') filtered = convos.filter(c => c.status === filter);
    
    // Also apply search query
    if (searchQuery && searchQuery.length >= 2) {
        const q = searchQuery.toLowerCase();
        filtered = filtered.filter(c => {
            const name = (c.contact_name || c.lead?.first_name || '').toLowerCase();
            const phone = (c.phone || '').replace(/\D/g, '');
            const msg = (c.last_message || '').toLowerCase();
            return name.includes(q) || phone.includes(q.replace(/\D/g, '')) || msg.includes(q);
        });
    }
    return filtered;
}

function setConvoFilter(filter) {
    convoFilter = filter;
    document.querySelectorAll('.convo-filter').forEach(btn => {
        btn.classList.toggle('active', btn.getAttribute('data-filter') === filter);
    });
    renderConvoList(filterConvosByStatus(smsConvos, filter));
}
...
function renderConvoList(convos) {
    ...
    if (!convos || convos.length === 0) {
        list.innerHTML = '<div class="convo-list-empty"><div class="convo-list-empty-icon">💬</div><div class="convo-list-empty-text">No conversations found</div><div class="convo-list-empty-sub">Try a different filter or start a new conversation</div></div>';
        return;
    }
    
    list.innerHTML = convos.map(c => {
        const name = c.contact_name || c.lead?.first_name || formatPhoneDisplay(c.phone);
        const isSelected = currentConvo && currentConvo.id === c.id;
        const hasUnread = (c.unread_count || 0) > 0;
        const source = c.lead?.source || 'direct';
        const timeAgo = formatTimeAgo(c.updated_at);
        const intentEmoji = {price: '💰', timing: '⏰', info: 'ℹ️', objection: '🤔', ready: '🔥', general: '💬'}[c.intent] || '💬';
        
        return '<div class="convo-item' + (isSelected ? ' active' : '') + (hasUnread ? ' unread' : '') + '" onclick="openConvo(' + c.id + ')">' +
            '<div class="convo-item-top">' +
            '<div class="convo-item-name">' + escapeHtml(name) + 
            (source !== 'direct' ? ' <span class="convo-item-source">' + source + '</span>' : '') + '</div>' +
            '<div class="convo-item-time">' + timeAgo + '</div>' +
            '</div>' +
            '<div class="convo-item-preview">' + escapeHtml(c.last_message || 'No messages yet') + '</div>' +
            '<div class="convo-item-meta">' +
            '<span class="convo-status ' + c.status + '">' + c.status.replace('-', ' ') + '</span>' +
            '<span class="convo-intent">' + intentEmoji + ' ' + c.intent + '</span>' +
            '<span class="convo-score ' + c.scoreLevel + '">' + c.score + '%</span>' +
            (hasUnread ? '<span class="convo-unread-badge">' + c.unread_count + '</span>' : '') +
            (c.slaStatus !== 'ok' ? '<span class="convo-sla ' + c.slaStatus + '">⏱ ' + formatSLA(c.slaMinutes) + '</span>' : '') +
            '</div>' +
            '</div>';
    }).join('');
}
...
async function openConvo(id) {
    ...
    try {
        const convo = await fetch('/api/sms/conversations/' + id).then(r => r.json()).catch(e => { 
            console.error('openConvo fetch error:', e); 
            return null; 
        });
        
        if (!convo || convo.error) {
            toast('Failed to load conversation', true);
            return;
        }
        
        currentConvo = analyzeConversation(convo);
        renderConvoThread(currentConvo);
        updateAriaCopilot(currentConvo);
        
        // Update active state in list
        document.querySelectorAll('.convo-item').forEach(el => el.classList.remove('active'));
        const activeItem = document.querySelector('.convo-item[onclick="openConvo(' + id + ')"]');
        if (activeItem) activeItem.classList.add('active');
        
        // Mark as read
        if (currentConvo.unread_count > 0) {
            fetch('/api/sms/conversations/' + id + '/read', {method: 'POST'}).catch(() => {});
        }
    } catch(e) {
        console.error('❌ openConvo error:', e);
        toast('Error loading conversation', true);
    }
}

function renderConvoThread(convo) {
    const area = $('convo-thread-area');
    if (!area) return;
    
    const name = convo.contact_name || convo.lead?.first_name || 'Unknown';
    const initials = name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0,2) || '?';
    const lead = convo.lead || {};
    const location = [lead.city, lead.state].filter(Boolean).join(', ');
    const isHot = convo.score >= 75;
    const intentLabels = {price: 'Asking about pricing', timing: 'Interested in timing', info: 'Seeking information', objection: 'Has concerns', ready: 'Ready to proceed', general: 'General inquiry'};
    
    let html = '<div class="convo-thread-header">' +
        '<div class="convo-thread-contact">' +
        '<div class="convo-thread-avatar' + (isHot ? ' hot' : '') + '">' + initials + '</div>' +
        '<div class="convo-thread-info">' +
        '<h3>' + escapeHtml(name) + (lead.id ? ' <span class="lead-badge">Lead #' + lead.id + '</span>' : '') + '</h3>' +
        '<div class="convo-thread-meta">' +
        '<span>📞 ' + formatPhoneDisplay(convo.phone) + '</span>' +
        (location ? '<span>📍 ' + location + '</span>' : '') +
        (lead.agent_type ? '<span>🏷️ ' + lead.agent_type + '</span>' : '') +
        '</div>' +
        '</div>' +
        '<div class="convo-thread-actions">' +
        '<button class="convo-thread-btn" onclick="triggerVoiceCall(\'' + convo.phone + '\', \'' + escapeHtml(name) + '\')" title="AI Call">📞</button>' +
        '<button class="convo-thread-btn" onclick="openLeadFromConvo(' + (lead.id || 0) + ', \'' + convo.phone + '\', \'' + escapeHtml(name) + '\')" title="View/Create Lead">👤</button>' +
        '<button class="convo-thread-btn primary" onclick="$(\'convo-input\').focus()" title="Reply">✍️</button>' +
        '</div>' +
        '</div>' +
        '</div>';
    
    // Lead Context Bar
    html += '<div class="convo-lead-bar">' +
        '<div class="convo-lead-info">' +
        '<span class="convo-lead-tag intent-' + convo.intent + '">' + (intentLabels[convo.intent] || 'General') + '</span>' +
        '<span style="color:rgba(255,255,255,0.5)">•</span>' +
        '<span style="font-size:12px">Emotion: <strong>' + convo.emotion + '</strong></span>' +
        '</div>' +
        '<div class="convo-lead-score">' +
        '<span style="font-size:12px;color:rgba(255,255,255,0.5)">Lead Score</span>' +
        '<div class="convo-lead-score-bar"><div class="convo-lead-score-fill" style="width:' + convo.score + '%"></div></div>' +
        '<span style="font-size:13px;font-weight:700;color:#14b8a6">' + convo.score + '%</span>' +
        '</div>' +
        '</div>';
    
    // Messages
    html += '<div class="convo-messages" id="convo-messages">';
    
    const messages = convo.messages || [];
    if (messages.length === 0) {
        html += '<div style="text-align:center;color:rgba(255,255,255,0.4);padding:40px">No messages yet. Start the conversation below!</div>';
    } else {
        messages.forEach(msg => {
            const isOutbound = msg.direction === 'outbound';
            const time = msg.sent_at ? new Date(msg.sent_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '';
            const statusIcon = msg.status === 'delivered' ? '✓✓' : msg.status === 'sent' ? '✓' : msg.status === 'failed' ? '✗' : '○';
            const statusClass = msg.status === 'delivered' ? 'delivered' : msg.status === 'sent' ? 'sent' : msg.status === 'failed' ? 'failed' : '';
            
            html += '<div class="convo-msg ' + (isOutbound ? 'outbound' : 'inbound') + '">' +
                '<div class="convo-msg-content">' +
                '<div class="convo-msg-bubble">' + escapeHtml(msg.message) + '</div>' +
                '<div class="convo-msg-meta">' +
                '<span>' + time + '</span>' +
                (isOutbound ? '<span class="convo-msg-status ' + statusClass + '">' + statusIcon + '</span>' : '') +
                '</div>' +
                '</div>' +
                '</div>';
        });
    ...
    // Smart Composer
    html += '<div class="convo-composer">' +
        '<div class="convo-composer-top">' +
        '<span class="convo-composer-label">Tone:</span>' +
        '<div class="convo-tone-btns">' +
        '<button class="convo-tone-btn' + (selectedTone === 'calm' ? ' active' : '') + '" onclick="setTone(\'calm\')">😌 Calm</button>' +
        '<button class="convo-tone-btn' + (selectedTone === 'friendly' ? ' active' : '') + '" onclick="setTone(\'friendly\')">😊 Friendly</button>' +
        '<button class="convo-tone-btn' + (selectedTone === 'direct' ? ' active' : '') + '" onclick="setTone(\'direct\')">🎯 Direct</button>' +
        '<button class="convo-tone-btn' + (selectedTone === 'curious' ? ' active' : '') + '" onclick="setTone(\'curious\')">🤔 Curious</button>' +
        '</div>' +
        '<div class="convo-goal-btns">' +
        '<button class="convo-goal-btn' + (selectedGoal === 'continue' ? ' active' : '') + '" onclick="setGoal(\'continue\')" title="Continue conversation">💬</button>' +
        '<button class="convo-goal-btn' + (selectedGoal === 'book' ? ' active' : '') + '" onclick="setGoal(\'book\')" title="Book appointment">📅</button>' +
        '<button class="convo-goal-btn' + (selectedGoal === 'reengage' ? ' active' : '') + '" onclick="setGoal(\'reengage\')" title="Re-engage">🔥</button>' +
        '</div>' +
        '</div>' +
        '<div class="convo-input-wrap">' +
        '<textarea class="convo-input" id="convo-input" placeholder="Type your message... (Enter to send)" rows="1" onkeydown="if(event.key===\'Enter\' && !event.shiftKey){event.preventDefault();sendConvoMsg()}"></textarea>' +
        '<button class="convo-send-btn" onclick="sendConvoMsg()">➤</button>' +
        '</div>' +
        '<div class="convo-composer-preview good" id="composer-preview">✓ Message looks good</div>' +
        '</div>';
...
function setTone(tone) {
    selectedTone = tone;
    document.querySelectorAll('.convo-tone-btn').forEach(btn => {
        btn.classList.toggle('active', btn.textContent.toLowerCase().includes(tone));
    });
}

function setGoal(goal) {
    selectedGoal = goal;
    document.querySelectorAll('.convo-goal-btn').forEach(btn => {
        btn.classList.toggle('active', btn.getAttribute('onclick').includes("'" + goal + "'"));
    });
}

function openLeadFromConvo(leadId, phone, name) {
    if (leadId) {
        // Navigate to lead detail
        navTo('leads');
        setTimeout(() => openLeadDetail(leadId), 300);
    } else {
        // Create new lead from conversation
        if (confirm('Create a new lead from this conversation?')) {
            fetch('/api/sms/convert-to-lead', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                ...
                if (result.success) {
                    toast('✅ Lead created!');
                    if (currentConvo) openConvo(currentConvo.id);
                } else {
    ...
    messages.forEach((m, i) => {
        const msgDate = m.sent_at ? new Date(m.sent_at).toDateString() : '';
        
        // Date separator
        if (msgDate && msgDate !== lastDate) {
            const dateLabel = msgDate === new Date().toDateString() ? 'Today' : 
                             msgDate === new Date(Date.now() - 86400000).toDateString() ? 'Yesterday' :
                             new Date(m.sent_at).toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' });
            messagesHtml += '<div style="text-align:center;padding:16px 0 8px"><span style="background:rgba(255,255,255,0.08);padding:4px 14px;border-radius:12px;font-size:11px;color:rgba(255,255,255,0.5)">' + dateLabel + '</span></div>';
            lastDate = msgDate;
        }
        
        const isOutbound = m.direction === 'outbound';
        const time = formatMsgTime(m.sent_at);
        const emotion = detectEmotion(m.message);
        
        messagesHtml += '<div class="convo-msg ' + (isOutbound ? 'outbound' : 'inbound') + '">' +
            '<div>' +
            '<div class="convo-msg-bubble">' + (m.message || '').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>' +
            '<div class="convo-msg-meta">' +
            '<span>' + time + '</span>' +
            (isOutbound ? getStatusIndicator(m) : '') +
            (emotion && !isOutbound ? '<span class="convo-msg-emotion">' + emotion + '</span>' : '') +
            '</div>' +
            '</div>' +
            '</div>';
    });
    ...
    if (!messages.length) {
        messagesHtml = '<div class="convo-empty"><div class="convo-empty-icon">💬</div><div class="convo-empty-title">Start the conversation</div><div class="convo-empty-subtitle">Send your first message below</div></div>';
    }
    ...
    if (convo.lead) {
        const lead = convo.lead;
        const statusColors = {new:'#3b82f6',contacted:'#f59e0b',qualified:'#8b5cf6',appointment_set:'#14b8a6',sold:'#22c55e',lost:'#ef4444'};
        leadBarHtml = '<div class="convo-lead-bar">' +
            '<div class="convo-lead-info">' +
            '<span>👤 Linked Lead:</span>' +
            '<strong>' + (lead.first_name || '') + ' ' + (lead.last_name || '') + '</strong>' +
            '<span class="convo-lead-tag" style="background:' + (statusColors[lead.status] || '#6b7280') + ';color:#fff">' + (lead.status || 'new').replace('_',' ').toUpperCase() + '</span>' +
            '</div>' +
        ...
        leadBarHtml = '<div class="convo-lead-bar" style="background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.1)">' +
            '<div class="convo-lead-info" style="color:rgba(255,255,255,0.5)">' +
            '<span>Not linked to any lead</span>' +
            '</div>' +
    ...
    area.innerHTML = 
        ...
        '<div class="convo-thread-contact">' +
        '<div class="convo-thread-avatar">' + ((convo.contact_name || 'U')[0].toUpperCase()) + '</div>' +
        '<div class="convo-thread-info">' +
        '<h3>' + (convo.contact_name || formatPhoneDisplay(convo.phone)) + '</h3>' +
        '<p>' + convo.phone + ' • ' + (convo.lead?.city || 'Unknown location') + '</p>' +
        '</div>' +
        '<div class="convo-thread-actions">' +
        '<button class="convo-thread-btn" onclick="window.open(\'tel:' + convo.phone + '\')" title="Call">📞</button>' +
        '<button class="convo-thread-btn" onclick="triggerVoiceCall(\'' + convo.phone + '\', \'' + (convo.contact_name || 'there') + '\')" title="AI Call">🤖</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        leadBarHtml +
        '<div class="convo-messages" id="convo-messages">' + messagesHtml + '</div>' +
        '<div class="convo-composer">' +
        '<div class="convo-composer-tools">' +
        '<button class="convo-tone-btn' + (selectedTone==='calm'?' active':'') + '" onclick="setTone(\'calm\')">😌 Calm</button>' +
        '<button class="convo-tone-btn' + (selectedTone==='friendly'?' active':'') + '" onclick="setTone(\'friendly\')">😊 Friendly</button>' +
        '<button class="convo-tone-btn' + (selectedTone==='professional'?' active':'') + '" onclick="setTone(\'professional\')">👔 Professional</button>' +
        '<button class="convo-tone-btn' + (selectedTone==='urgent'?' active':'') + '" onclick="setTone(\'urgent\')">⚡ Urgent</button>' +
        '</div>' +
        '<div class="convo-input-wrap">' +
        '<textarea class="convo-input" id="convo-input" placeholder="Type a message..." onkeypress="if(event.key===\'Enter\'&&!event.shiftKey){event.preventDefault();sendConvoMsg()}" rows="1"></textarea>' +
        '<button class="convo-send-btn" onclick="sendConvoMsg()">➤</button>' +
        '</div>' +
        '</div>';
...
function getStatusIndicator(msg) {
    ...
    let icon = '✓', color = 'rgba(255,255,255,0.4)';
    if (status === 'delivered') { icon = '✓✓'; color = '#22c55e'; }
    else if (status === 'read') { icon = '✓✓'; color = '#3b82f6'; }
    else if (status === 'failed') { icon = '✗'; color = '#ef4444'; }
    return '<span style="color:' + color + ';margin-left:6px">' + icon + '</span>';
}

function detectEmotion(message) {
    if (!message) return null;
    const msg = message.toLowerCase();
    if (msg.includes('!') && (msg.includes('great') || msg.includes('awesome') || msg.includes('perfect'))) return '😊 Excited';
    if (msg.includes('?') && msg.length < 50) return '🤔 Curious';
    if (msg.includes('not sure') || msg.includes('maybe') || msg.includes('think about')) return '😐 Hesitant';
    if (msg.includes('yes') || msg.includes('sounds good') || msg.includes('interested')) return '👍 Positive';
    return null;
}
...
function updateAriaCopilot(convo) {
    const panel = $('aria-copilot-content');
    if (!panel) return;
    
    const lead = convo.lead || {};
    const name = convo.contact_name || lead.first_name || 'there';
    const messages = convo.messages || [];
    const lastInbound = messages.filter(m => m.direction === 'inbound').pop();
    
    // Generate intelligent AI insight based on deep analysis
    let insight = '';
    let analysisTitle = 'Conversation Active';
    let analysisIcon = '💬';
    let suggestedReplies = [];
    let tags = [];
    
    // Detect key conversation signals
    const hasSpouseMention = messages.some(m => /spouse|husband|wife|partner|we need to/i.test(m.message));
    const hasPricingQ = messages.some(m => /price|cost|how much|\$|expensive|budget/i.test(m.message));
    const hasUrgency = messages.some(m => /asap|urgent|today|immediately|emergency/i.test(m.message));
    const hasPositive = lastInbound && /yes|great|awesome|perfect|interested|sounds good/i.test(lastInbound.message);
    const hasNegative = lastInbound && /no|can't|not interested|stop|remove/i.test(lastInbound.message);
    
    if (hasSpouseMention) tags.push({text: '👫 Decision Maker', type: 'info'});
    if (hasPricingQ) tags.push({text: '💰 Price Sensitive', type: 'warn'});
    if (hasUrgency) tags.push({text: '⚡ Urgent', type: 'hot'});
    if (hasPositive) tags.push({text: '✅ Positive', type: 'good'});
    
    if (convo.intent === 'objection' || hasSpouseMention) {
        analysisIcon = '🤔';
        analysisTitle = 'Handling Objection';
        insight = 'They mentioned needing to discuss with someone else. <strong>Don\'t push.</strong> Use permission-based questions and offer to send info they can review together.';
        suggestedReplies = [
            { text: 'Totally understand! What information would be most helpful to share with them?', type: 'NEPQ', style: 'nepq' },
            { text: 'No rush at all! Would it help if I sent some details you could review together?', type: 'Soft', style: 'soft' },
            { text: MESSAGE_TEMPLATES.spouse.replace('{name}', name), type: 'Template', style: 'direct' }
        ];
    } else if (convo.intent === 'price' || hasPricingQ) {
        analysisIcon = '💰';
        analysisTitle = 'Price Discussion';
        insight = 'They\'re asking about pricing. <strong>Focus on value first.</strong> Understand their needs before discussing numbers. This qualifies their interest!';
        suggestedReplies = [
            { text: 'Great question! To give you accurate pricing, can I ask what you\'re hoping to accomplish?', type: 'Discovery', style: 'nepq' },
            { text: MESSAGE_TEMPLATES.price.replace('{name}', name), type: 'Value-First', style: 'soft' },
            { text: 'Pricing varies based on your needs. Would a quick 5-min call help me give you an exact quote?', type: 'Qualifying', style: 'direct' }
        ];
    } else if (convo.intent === 'ready' || hasPositive) {
        analysisIcon = '🔥';
        analysisTitle = 'HIGH INTENT - Strike Now!';
        insight = 'This lead is <strong>ready to move forward!</strong> Book them immediately. Don\'t let this opportunity cool down.';
        suggestedReplies = [
            { text: 'Awesome! Let\'s get you scheduled. Does tomorrow at 10am or 2pm work better?', type: 'Booking', style: 'direct' },
            { text: 'Perfect! I have a spot available today if that works for you?', type: 'Urgent', style: 'nepq' },
            { text: MESSAGE_TEMPLATES.booking.replace('{name}', name), type: 'Template', style: 'soft' }
        ];
    } else if (convo.status === 'stale' || convo.slaMinutes > 2880) {
        analysisIcon = '⏰';
        analysisTitle = 'Re-engagement Needed';
        insight = 'It\'s been a while since they responded. Send a <strong>gentle, no-pressure</strong> message. They might just be busy!';
        suggestedReplies = [
            { text: 'Hey ' + name + '! Just checking in — no pressure at all. Here whenever you\'re ready! 🙂', type: 'Soft', style: 'soft' },
            { text: MESSAGE_TEMPLATES.reengage.replace('{name}', name), type: 'Re-engage', style: 'nepq' },
            { text: 'Hi ' + name + '! Wanted to touch base. Has anything changed on your end?', type: 'Check-in', style: 'direct' }
        ];
    } else if (convo.intent === 'timing') {
        analysisIcon = '📅';
        analysisTitle = 'Timing Interest';
        insight = 'They\'re asking about scheduling/timing. Great sign! <strong>Be flexible</strong> and offer specific times.';
        suggestedReplies = [
            { text: MESSAGE_TEMPLATES.timing.replace('{name}', name), type: 'Flexible', style: 'soft' },
            { text: 'I\'m pretty flexible! What time works best with your schedule?', type: 'Accommodating', style: 'nepq' },
            { text: 'How about tomorrow afternoon? I could do 2pm or 4pm.', type: 'Specific', style: 'direct' }
        ];
    } else if (hasNegative) {
        analysisIcon = '⚠️';
        analysisTitle = 'Negative Response';
        insight = 'They might not be interested right now. <strong>Respect their decision</strong> but leave the door open for the future.';
        suggestedReplies = [
            { text: 'No problem at all! I\'ll make a note. Feel free to reach out if anything changes. Have a great day!', type: 'Graceful Exit', style: 'soft' },
            { text: 'Totally understand! Is there anything specific that didn\'t work for you? Just want to make sure I\'m not missing something.', type: 'Feedback', style: 'nepq' }
        ];
    } else {
        analysisIcon = '💬';
        analysisTitle = 'Keep Building Rapport';
        insight = 'Conversation is flowing well. Keep asking <strong>open-ended questions</strong> to understand their needs better.';
        suggestedReplies = [
            { text: 'That\'s really helpful to know! What would be the ideal outcome for you?', type: 'Discovery', style: 'nepq' },
            { text: 'Thanks for sharing! Can I ask what prompted you to reach out today?', type: 'Qualifying', style: 'soft' },
            { text: 'Interesting! Tell me more about what you\'re looking for.', type: 'Open-ended', style: 'direct' }
        ];
    }
    
    panel.innerHTML = 
        '<div class="aria-section">' +
        '<div class="aria-section-title"><span>' + analysisIcon + '</span> ' + analysisTitle + '</div>' +
        '<div class="aria-analysis">' +
        '<div class="aria-analysis-text">' + insight + '</div>' +
        (tags.length > 0 ? '<div class="aria-analysis-tags">' + tags.map(t => '<span class="aria-analysis-tag">' + t.text + '</span>').join('') + '</div>' : '') +
        '</div>' +
        (lead.id ? '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px;font-size:12px">' +
        '<div style="background:rgba(255,255,255,0.03);padding:10px;border-radius:8px"><span style="color:rgba(255,255,255,0.4)">Type</span><br><strong>' + (lead.agent_type || 'General') + '</strong></div>' +
        '<div style="background:rgba(255,255,255,0.03);padding:10px;border-radius:8px"><span style="color:rgba(255,255,255,0.4)">Status</span><br><strong>' + (lead.status || 'New') + '</strong></div>' +
        '</div>' : '') +
        '</div>' +
        
        '<div class="aria-section">' +
        '<div class="aria-section-title"><span>💡</span> Suggested Replies</div>' +
        suggestedReplies.map(r => 
            '<div class="aria-reply" onclick="useSuggestedReply(\'' + r.text.replace(/'/g, "\\'").replace(/"/g, '&quot;') + '\')">' +
            '<div class="aria-reply-text">' + r.text + '</div>' +
            '<div class="aria-reply-meta">' +
            '<span class="aria-reply-type ' + r.style + '">' + r.type + '</span>' +
            '<span class="aria-reply-use">Use this →</span>' +
            '</div>' +
            '</div>'
        ).join('') +
        '</div>' +
        ...
        '<div class="aria-section">' +
        '<div class="aria-section-title"><span>⚡</span> Actions</div>' +
        '<div class="aria-actions-grid">' +
        '<div class="aria-action" onclick="triggerVoiceCall(\'' + convo.phone + '\', \'' + name.replace(/'/g, "\\'") + '\')">' +
        '<div class="aria-action-icon">📞</div>' +
        '<div class="aria-action-text">AI Call</div>' +
        '</div>' +
        '<div class="aria-action" onclick="useTemplate(\'booking\')">' +
        '<div class="aria-action-icon">📅</div>' +
        '<div class="aria-action-text">Book Appt</div>' +
        '</div>' +
        '<div class="aria-action" onclick="scheduleFollowUp(\'' + convo.phone + '\')">' +
        '<div class="aria-action-icon">⏰</div>' +
        '<div class="aria-action-text">Follow-up</div>' +
        '</div>' +
        '<div class="aria-action" onclick="openLeadFromConvo(' + (lead.id || 0) + ', \'' + convo.phone + '\', \'' + name.replace(/'/g, "\\'") + '\')">' +
        '<div class="aria-action-icon">👤</div>' +
        '<div class="aria-action-text">' + (lead.id ? 'View Lead' : 'Create Lead') + '</div>' +
        '</div>' +
        '</div>' +
        '</div>' +
        
        '<div class="aria-section">' +
        '<div class="aria-section-title"><span>📝</span> Quick Templates</div>' +
        '<div class="aria-templates">' +
        '<button class="aria-template" onclick="useTemplate(\'opener\')">👋 Opener</button>' +
        '<button class="aria-template" onclick="useTemplate(\'followup\')">🔄 Follow-up</button>' +
        '<button class="aria-template" onclick="useTemplate(\'booking\')">📅 Book</button>' +
        '<button class="aria-template" onclick="useTemplate(\'checkin\')">💬 Check-in</button>' +
        '<button class="aria-template" onclick="useTemplate(\'reengage\')">🔥 Re-engage</button>' +
        '</div>' +
        '</div>';
}
...
function useTemplate(templateKey) {
    if (!currentConvo) { toast('Select a conversation first', true); return; }
    
    const template = MESSAGE_TEMPLATES[templateKey];
    if (!template) return;
    
    const name = currentConvo.contact_name || currentConvo.lead?.first_name || 'there';
    const company = 'our team'; // Could be dynamic
...
function triggerVoiceCall(phone, name) {
    if (confirm('Start an AI voice call to ' + name + '?')) {
        fetch('/api/call-now', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({phone: phone, name: name, agent_type: 'roofing'})
        }).then(r => r.json()).then(result => {
            if (result.success) {
                toast('✅ AI calling ' + name + '...');
            } else {
                toast('❌ ' + (result.error || 'Call failed'), true);
            }
        });
    }
}

function scheduleFollowUp(phone) {
    toast('Follow-up scheduled for 24 hours from now');
    // Could implement actual scheduling here
}
...
async function sendConvoMsg() {
    ...
    if (msgContainer) {
        ...
        tempMsg.className = 'convo-msg outbound';
        tempMsg.innerHTML = '<div><div class="convo-msg-bubble">' + msg.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div><div class="convo-msg-meta"><span>Sending...</span></div></div>';
        msgContainer.appendChild(tempMsg);
    ...
    const result = await fetch('/api/sms/send', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({conversation_id: currentConvo.id, phone: currentConvo.phone, message: msg})
    }).then(r => r.json()).catch(() => ({error: 'Failed to send'}));
    
    setTimeout(() => openConvo(currentConvo.id), 500);
...
// Auto-refresh messages every 10 seconds when viewing a conversation
setInterval(() => {
    if (currentConvo && document.getElementById('page-messages').classList.contains('active')) {
        // Silently refresh conversation list
        fetch('/api/sms/conversations').then(r => r.json()).then(convos => {
            smsConvos = convos;
...
// Phone Number SMS Status Functions
// Now using unified number for both inbound and outbound
const PRIMARY_PHONE_NUMBER = '+17208189512';

async function markSmsEnabled(type) {
    try {
        await fetch('/api/phone-config', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({phone: PRIMARY_PHONE_NUMBER, sms_enabled: true})
        });
        
        const statusEl = $(type + '-sms-status');
        if (statusEl) {
            statusEl.innerHTML = '<span style="font-size:18px">💬</span><span style="font-size:10px;color:#22c55e;font-weight:600">SMS ✓</span>';
        }
        toast('✅ SMS marked as enabled!');
    } catch(e) {
...
async function loadSmsStatus() {
    try {
        const phones = await fetch('/api/phone-config').then(r => r.json());
        phones.forEach(p => {
            if (p.sms_enabled && p.phone === PRIMARY_PHONE_NUMBER) {
                // Update both outbound and inbound status since unified number
        ...
        // Load setup checklist status
        ['sip', 'retell', 'a2p'].forEach(item => {
            if (localStorage.getItem('setup_' + item) === 'done') {
                markSetupVisual(item);
            }
        });
    } catch(e) {
        console.log('Could not load phone config');
    }
}

async function testSmsNumber() {
    const phone = prompt('Enter phone number to send test SMS:', '+1');
    if (!phone || phone.length < 10) return;
    
    toast('📱 Sending test SMS...');
    try {
        const convoResult = await fetch('/api/sms/conversations', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                phone: phone, 
                contact_name: 'Test', 
                message: '🧪 Test from VOICE - Same number for calls AND texts! Reply to confirm.'
            })
        }).then(r => r.json());
        
        if (convoResult.id || convoResult.success) {
            toast('✅ Test SMS sent! Check your phone.');
            // Switch to messages page
            setTimeout(() => { navTo('messages'); loadMessages(); }, 1000);
        } else {
            toast('❌ Failed: ' + (convoResult.error || 'Unknown error'), true);
        }
    } catch(e) {
...
async function testVoiceCall() {
    const phone = prompt('Enter phone number to call:', '+1');
    if (!phone || phone.length < 10) return;
    
    toast('📞 Initiating test call...');
    try {
        const result = await fetch('/api/test-call', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({phone: phone, agent: 'receptionist'})
        }).then(r => r.json());
        
        if (result.success || result.call_id) {
            toast('✅ Call initiated! Your phone should ring.');
        } else {
            toast('❌ Failed: ' + (result.error || 'Unknown error'), true);
        }
    } catch(e) {
...
async function doubleTap() {
    const phone = prompt('Enter phone number for Double Tap (SMS then Call):', '+1');
    if (!phone || phone.length < 10) return;
    
    toast('⚡ Double Tap: Sending SMS...');
    
    try {
        // Step 1: Send SMS
        const smsResult = await fetch('/api/sms/conversations', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                phone: phone, 
                contact_name: 'Double Tap Lead', 
                message: 'Hey! Just tried calling - I\\'ll try again in just a sec. Talk soon!'
            })
        }).then(r => r.json());
        
        if (smsResult.error) {
            toast('❌ SMS failed: ' + smsResult.error, true);
            return;
        }
        
        toast('📱 SMS sent! Calling in 5 seconds...');
        
        // Step 2: Wait 5 seconds then call
        setTimeout(async () => {
            try {
                const callResult = await fetch('/api/test-call', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({phone: phone, agent: 'receptionist'})
                }).then(r => r.json());
                
                if (callResult.success || callResult.call_id) {
                    toast('✅ Double Tap complete! Call initiated.');
                } else {
                    toast('📱 SMS sent, but call failed: ' + (callResult.error || 'Unknown'), true);
                }
            } catch(e) {
                toast('📱 SMS sent, but call error: ' + e.message, true);
            }
        }, 5000);
...
function markSetupComplete(item) {
    localStorage.setItem('setup_' + item, 'done');
    markSetupVisual(item);
    toast('✅ ' + item.toUpperCase() + ' marked as complete!');
}

function markSetupVisual(item) {
    const statusMap = {
        'sip': 'sip-trunk-status',
        'retell': 'retell-import-status',
        'a2p': 'a2p-status'
    };
    const el = $(statusMap[item]);
    if (el) {
        el.style.background = 'rgba(34,197,94,0.1)';
        el.querySelector('span').textContent = '✓';
        el.querySelector('span').style.color = '#22c55e';
    }
}

// Load SMS status on page load
setTimeout(loadSmsStatus, 500);

async function startNewConvo() {
    const phone = $('new-sms-phone').value.trim();
    const name = $('new-sms-name').value.trim();
    const msg = $('new-sms-msg').value.trim();
    if (!phone) { toast('Enter phone number', true); return; }
    const r = await fetch('/api/sms/conversations', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({phone: phone, contact_name: name, message: msg})
    }).then(r => r.json());
    if (r.id) {
        closeModal('new-sms-modal');
        loadMessages();
        if (msg) openConvo(r.id);
    }
}
...
async function loadCompliance() {
    try {
        const filter = $('comp-filter') ? $('comp-filter').value : 'all';
        const data = await fetch('/api/compliance?filter=' + filter).then(r => r.json()).catch(() => ({leads: [], stats: {}}));
        
        const stats = data.stats || {};
        if($('comp-total')) $('comp-total').textContent = stats.total || 0;
        if($('comp-contacted')) $('comp-contacted').textContent = stats.contacted || 0;
        if($('comp-pending')) $('comp-pending').textContent = stats.pending || 0;
        if($('comp-complete')) $('comp-complete').textContent = stats.complete || 0;
        ...
        if (!leads.length) {
            container.innerHTML = '<div style="text-align:center;padding:60px;color:var(--gray-500)"><div style="font-size:48px;margin-bottom:12px">📋</div><div style="font-weight:500">No leads to show</div><div style="font-size:13px;margin-top:8px">Leads from Facebook will appear here with their compliance timeline</div></div>';
            return;
        }
        
        container.innerHTML = leads.map(lead => renderComplianceCard(lead)).join('');
    } catch(e) {
        console.error('loadCompliance error:', e);
    }
}

function renderComplianceCard(lead) {
    const steps = [
        { key: 'fb_submitted', label: '📥 Lead Submitted', time: lead.created_at },
        { key: 'text_sent', label: '💬 Welcome Text Sent', time: lead.text_sent_at },
        { key: 'call_1', label: '📞 First Call', time: lead.call_1_at },
        { key: 'call_2', label: '📞 Double Tap (if no answer)', time: lead.call_2_at },
        { key: 'call_1215', label: '🕐 12:15 PM Call', time: lead.call_1215_at },
        { key: 'call_1215_double', label: '📞 12:15 Double Tap', time: lead.call_1215_double_at },
        { key: 'call_530', label: '🌅 5:30 PM Call', time: lead.call_530_at },
        { key: 'call_530_double', label: '📞 5:30 Double Tap', time: lead.call_530_double_at },
        { key: 'contacted', label: '✅ Contacted / Answered', time: lead.contacted_at },
        { key: 'appointment_booked', label: '📅 Appointment Booked', time: lead.appointment_booked_at }
    ];
    
    const dayNum = lead.sequence_day || 1;
    const answered = lead.status === 'contacted' || lead.status === 'appointment_set' || lead.status === 'sold';
    
    let stepsHtml = steps.map(step => {
        const done = !!step.time;
        const timeStr = step.time ? new Date(step.time).toLocaleString('en-US', {month:'short', day:'numeric', hour:'numeric', minute:'2-digit'}) : '--';
        return '<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05)">' +
            '<div style="width:28px;height:28px;border-radius:50%;background:' + (done ? '#22c55e' : 'rgba(255,255,255,0.1)') + ';display:flex;align-items:center;justify-content:center;font-size:14px">' + (done ? '✓' : '○') + '</div>' +
            '<div style="flex:1">' +
            '<div style="font-size:13px;color:' + (done ? '#fff' : 'var(--gray-500)') + '">' + step.label + '</div>' +
            '</div>' +
            '<div style="font-size:12px;color:' + (done ? '#22c55e' : 'var(--gray-500)') + ';font-family:monospace">' + timeStr + '</div>' +
            '</div>';
    }).join('');
    ...
    return '<div style="background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:12px;margin-bottom:16px;overflow:hidden">' +
        // Header
        '<div style="padding:16px 20px;background:rgba(0,0,0,0.2);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">' +
        '<div style="display:flex;align-items:center;gap:12px">' +
        '<div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#14b8a6,#0d9488);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:600;color:#fff">' + (lead.first_name || 'L').charAt(0).toUpperCase() + '</div>' +
        '<div>' +
        '<div style="font-weight:600;font-size:16px">' + (lead.first_name || 'Unknown') + ' ' + (lead.last_name || '') + '</div>' +
        '<div style="font-size:13px;color:var(--gray-400)">' + (lead.phone || 'No phone') + '</div>' +
        '</div>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:12px">' +
        '<span style="background:' + statusColor + ';color:#fff;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600;text-transform:uppercase">' + (lead.status || 'new').replace('_', ' ') + '</span>' +
        '<span style="background:rgba(255,255,255,0.1);color:var(--gray-300);padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600">Day ' + dayNum + '/7</span>' +
        (answered ? '<span style="background:#22c55e;color:#fff;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:600">✓ ANSWERED</span>' : '') +
        '</div>' +
        '</div>' +
        // Timeline
        '<div style="padding:12px 20px">' + stepsHtml + '</div>' +
        // Footer
        '<div style="padding:12px 20px;background:rgba(0,0,0,0.15);display:flex;gap:8px;flex-wrap:wrap">' +
        '<button onclick="manualCall(' + lead.id + ')" class="btn btn-sm btn-primary">📞 Call Now</button>' +
        '<button onclick="markContacted(' + lead.id + ')" class="btn btn-sm btn-secondary">✓ Mark Contacted</button>' +
        '<button onclick="viewLeadDetail(' + lead.id + ')" class="btn btn-sm btn-secondary">📋 Details</button>' +
        '</div>' +
        '</div>';
}

async function manualCall(leadId) {
    toast('📞 Calling lead...');
    await fetch('/api/compliance/call', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function markContacted(leadId) {
    await fetch('/api/compliance/mark-contacted', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({lead_id: leadId})
    });
...
function viewLeadDetail(leadId) {
    // Could open a modal with full lead details
    toast('Opening lead ' + leadId);
}

// Submit Test Lead - goes through full compliance flow
async function submitTestLead() {
    const phone = $('test-lead-phone') ? $('test-lead-phone').value.trim() : '';
    const firstName = $('test-lead-fname') ? $('test-lead-fname').value.trim() : 'Test';
    const lastName = $('test-lead-lname') ? $('test-lead-lname').value.trim() : 'Lead';
    const email = $('test-lead-email') ? $('test-lead-email').value.trim() : '';
    const agentType = $('test-lead-agent') ? $('test-lead-agent').value : 'roofing';
    const shouldCall = $('test-lead-call') ? $('test-lead-call').checked : true;
    
    if (!phone) {
        toast('📱 Phone number required', true);
        return;
    }
    ...
    toast('🔄 Adding test lead...');
    
    try {
        // Use the same endpoint as Facebook/Zapier leads
        const result = await fetch('/api/compliance/test-lead', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                phone: cleanPhone,
                ...
                auto_call: shouldCall,
                source: 'test'
            })
        }).then(r => r.json());
        
        if (result.success) {
            toast('✅ Test lead added!' + (shouldCall ? ' AI is calling now...' : ''));
            closeModal('test-lead-modal');
            
            // Clear form
            if($('test-lead-phone')) $('test-lead-phone').value = '';
            if($('test-lead-fname')) $('test-lead-fname').value = '';
            if($('test-lead-lname')) $('test-lead-lname').value = '';
            if($('test-lead-email')) $('test-lead-email').value = '';
            
            // Refresh compliance list after short delay
            setTimeout(loadCompliance, 1500);
        } else {
            toast('❌ ' + (result.error || 'Failed to add lead'), true);
        }
    } catch (e) {
        toast('❌ Error: ' + e.message, true);
    }
...
async function loadCalls(){
    try{
        const c=await fetch('/api/calls').then(r=>r.json()).catch(()=>[]);
        
        // Update call stats
        const total = c.length;
        const answered = c.filter(x => x.status === 'answered' || x.status === 'completed' || x.status === 'user_hangup' || x.status === 'agent_hangup').length;
        const noAnswer = c.filter(x => x.status === 'no_answer' || x.status === 'voicemail' || x.status === 'voicemail_reached' || x.status === 'failed').length;
        const appts = c.filter(x => x.outcome === 'appointment' || x.appointment_set).length;
        
        if($('call-total')) $('call-total').textContent = total;
        if($('call-answered')) $('call-answered').textContent = answered;
        if($('call-noanswer')) $('call-noanswer').textContent = noAnswer;
        if($('call-appts')) $('call-appts').textContent = appts;
        
        // Populate table
        if($('calls-tb'))$('calls-tb').innerHTML=c.map(x=>`<tr><td style="font-size:12px">${new Date(x.created_at).toLocaleString()}</td><td>${x.phone}</td><td>${agents[x.agent_type]?.name||x.agent_type||'?'}</td><td>${x.is_inbound?'Inbound':'Outbound'}</td><td><span class="status ${x.status==='answered'||x.status==='completed'?'status-sold':'status-scheduled'}">${x.status||'Unknown'}</span></td><td>${x.duration||0}s</td></tr>`).join('');
...
if(service==='gcal'){
    ...
    openModal('facebook-setup-modal');
}else{
    toast('Coming soon!');
}
}
...
async function startGoogleAuth(){
    const clientId = $('gcal-client-id') ? $('gcal-client-id').value.trim() : '';
    if(!clientId){
        toast('Please enter your Google Client ID',true);
        return;
    }
    // Save client ID
    await fetch('/api/integrations/google_calendar', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({client_id: clientId, client_secret: $('gcal-client-secret') ? $('gcal-client-secret').value.trim() : ''})
    });
    ...
    toast('Complete authorization in the popup window');
}

// Test Google Calendar connection
async function testGoogleCalendar(){
    toast('Testing Google Calendar...');
    const result = await fetch('/api/integrations/google_calendar/test', {method: 'POST'}).then(r => r.json()).catch(() => ({success: false}));
    if(result.success){
        toast('✅ Google Calendar connected!');
        closeModal('gcal-setup-modal');
        updateGcalStatus(true);
    }else{
        toast('❌ Connection failed: ' + (result.error || 'Check credentials'), true);
    }
}

function updateGcalStatus(connected){
    const card = document.querySelector('[data-service="gcal"]');
    if(card){
        if(connected){
            card.classList.add('connected');
            card.querySelector('.int-conn-btn').textContent = '✓ Connected';
            card.querySelector('.int-conn-btn').classList.add('connected');
        }
    }
}

// Facebook Leads - Show webhook URL
function copyFacebookWebhook(){
    const url = window.location.origin + '/api/zapier/facebook-lead';
...
async function testFacebookWebhook(){
    toast('Sending test lead...');
    const result = await fetch('/api/zapier/facebook-lead', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            phone: '+17023240525',
    ...
    if(result.success){
        toast('✅ Test lead created! Check your phone for a call.');
        updateFacebookStatus(true);
    }else{
        toast('❌ Test failed: ' + (result.error || 'Unknown error'), true);
    }
}

function updateFacebookStatus(connected){
    const card = document.querySelector('[data-service="facebook"]');
    if(card){
        if(connected){
            card.classList.add('connected');
            card.querySelector('.int-conn-btn').textContent = '✓ Connected';
...
try{
const r=await fetch('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email,password:pw})}).then(r=>r.json()).catch(e=>{console.error('Login fetch error:',e);return {error:'Network error - please try again'}});
console.log('Login response:',r);
if(r.session_token){localStorage.setItem('voice_session',r.session_token);currentUser=r.user;document.body.classList.remove('logged-out');closeModal('login-modal');toast('Welcome back, '+(r.user.name||'User')+'!');localStorage.setItem('session',r.session_token);setTimeout(()=>location.reload(),500)}else{toast(r.error||'Login failed',true);btn.disabled=false;btn.textContent='Sign In'}
}catch(e){console.error('Login error:',e);toast('Login error - please try again',true);btn.disabled=false;btn.textContent='Sign In'}
}
...
<div class="nav-item" data-page="messages" onclick="navTo('messages')">💬 Messages <span class="nav-badge" id="unread-sms">0</span></div>
...
<div class="nav-item" data-page="compliance" onclick="navTo('compliance')">✅ Compliance</div>
...
<style>
/* ═══════════════════════════════════════════════════════════════════════════════
   CONVERSATION INTELLIGENCE HUB - VOICE 2.0 (10× Better Than GHL)
   This is not SMS. This is Conversation Intelligence.
   ═══════════════════════════════════════════════════════════════════════════════ */

/* Main Layout */
.convo-hub{display:grid;grid-template-columns:340px 1fr 360px;gap:0;height:calc(100vh - 120px);background:var(--black);border-radius:16px;overflow:hidden;border:1px solid var(--border)}
.convo-left{background:rgba(0,0,0,0.4);border-right:1px solid var(--border);display:flex;flex-direction:column}
.convo-center{display:flex;flex-direction:column;background:rgba(255,255,255,0.01)}
.convo-right{background:linear-gradient(180deg,rgba(139,92,246,0.03) 0%,rgba(0,0,0,0.4) 100%);border-left:1px solid var(--border);display:flex;flex-direction:column}

/* Left Panel Header */
.convo-left-header{padding:20px;border-bottom:1px solid var(--border)}
.convo-left-title{font-size:20px;font-weight:700;display:flex;align-items:center;gap:10px;margin-bottom:16px}
.convo-left-title span{background:linear-gradient(135deg,#14b8a6,#0d9488);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.convo-search{width:100%;padding:12px 16px;border-radius:12px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.03);color:#fff;font-size:14px}
.convo-search:focus{outline:none;border-color:#14b8a6;background:rgba(20,184,166,0.05)}
.convo-filters{display:flex;gap:6px;margin-top:14px;flex-wrap:wrap}
.convo-filter{padding:7px 14px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.5);border:1px solid transparent}
.convo-filter:hover{background:rgba(255,255,255,0.08);color:#fff}
.convo-filter.active{background:linear-gradient(135deg,#14b8a6,#0d9488);color:#000;border-color:transparent}
.convo-filter-count{background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:8px;margin-left:4px;font-size:10px}

/* Conversation List */
.convo-list{flex:1;overflow-y:auto}
.convo-list::-webkit-scrollbar{width:6px}
.convo-list::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:3px}
.convo-item{padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.04);cursor:pointer;transition:all 0.15s;position:relative}
.convo-item:hover{background:rgba(255,255,255,0.03)}
.convo-item.active{background:rgba(20,184,166,0.08);border-left:3px solid #14b8a6}
.convo-item.unread{background:rgba(59,130,246,0.05)}
.convo-item.unread::after{content:'';position:absolute;left:8px;top:50%;transform:translateY(-50%);width:8px;height:8px;background:#3b82f6;border-radius:50%}
.convo-item-top{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px}
.convo-item-name{font-weight:600;font-size:14px;color:#fff;display:flex;align-items:center;gap:8px}
.convo-item-source{font-size:10px;padding:2px 6px;border-radius:6px;background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.5)}
.convo-item-time{font-size:11px;color:rgba(255,255,255,0.4)}
.convo-item-preview{font-size:13px;color:rgba(255,255,255,0.5);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:10px;line-height:1.4}
.convo-item-meta{display:flex;gap:6px;align-items:center;flex-wrap:wrap}
.convo-status{padding:4px 10px;border-radius:12px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.3px}
.convo-status.new{background:rgba(59,130,246,0.15);color:#3b82f6}
.convo-status.replied{background:rgba(16,185,129,0.15);color:#10b981}
.convo-status.follow-up{background:rgba(245,158,11,0.15);color:#f59e0b}
.convo-status.booked{background:rgba(168,85,247,0.15);color:#a855f7}
.convo-status.stale{background:rgba(107,114,128,0.15);color:#6b7280}
.convo-status.hot{background:rgba(239,68,68,0.15);color:#ef4444}
.convo-intent{padding:4px 10px;border-radius:12px;font-size:10px;font-weight:500;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.6)}
.convo-sla{font-size:10px;font-weight:700;display:flex;align-items:center;gap:4px;padding:4px 8px;border-radius:8px}
.convo-sla.urgent{color:#ef4444;background:rgba(239,68,68,0.1)}
.convo-sla.warning{color:#f59e0b;background:rgba(245,158,11,0.1)}
.convo-sla.ok{color:#10b981;background:rgba(16,185,129,0.1)}
.convo-unread-badge{background:#14b8a6;color:#000;font-size:11px;font-weight:700;padding:3px 8px;border-radius:10px;min-width:20px;text-align:center}
.convo-score{font-size:10px;font-weight:600;padding:4px 8px;border-radius:8px}
.convo-score.high{color:#10b981;background:rgba(16,185,129,0.1)}
.convo-score.medium{color:#f59e0b;background:rgba(245,158,11,0.1)}
.convo-score.low{color:#6b7280;background:rgba(107,114,128,0.1)}

/* Center Panel - Thread */
.convo-thread-header{padding:20px;border-bottom:1px solid var(--border);background:rgba(0,0,0,0.3)}
.convo-thread-contact{display:flex;align-items:center;gap:16px}
.convo-thread-avatar{width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#14b8a6,#0d9488);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:#fff;position:relative}
.convo-thread-avatar.hot::after{content:'🔥';position:absolute;bottom:-2px;right:-2px;font-size:14px}
.convo-thread-info{flex:1}
.convo-thread-info h3{font-size:18px;font-weight:700;margin-bottom:4px;display:flex;align-items:center;gap:10px}
.convo-thread-info h3 .lead-badge{font-size:10px;padding:3px 8px;border-radius:8px;background:rgba(20,184,166,0.15);color:#14b8a6;font-weight:600}
.convo-thread-meta{font-size:13px;color:rgba(255,255,255,0.5);display:flex;align-items:center;gap:12px;flex-wrap:wrap}
.convo-thread-meta span{display:flex;align-items:center;gap:4px}
.convo-thread-actions{display:flex;gap:8px}
.convo-thread-btn{width:42px;height:42px;border-radius:12px;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.03);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:18px;transition:all 0.2s}
.convo-thread-btn:hover{background:rgba(20,184,166,0.15);border-color:#14b8a6;transform:translateY(-2px)}
.convo-thread-btn.primary{background:linear-gradient(135deg,#14b8a6,#0d9488);border:none}
.convo-thread-btn.primary:hover{transform:translateY(-2px);box-shadow:0 4px 15px rgba(20,184,166,0.3)}

/* Lead Context Bar */
.convo-lead-bar{padding:14px 20px;background:linear-gradient(90deg,rgba(20,184,166,0.08),rgba(139,92,246,0.05));border-bottom:1px solid rgba(20,184,166,0.15);display:flex;justify-content:space-between;align-items:center}
.convo-lead-info{display:flex;align-items:center;gap:16px;font-size:13px}
.convo-lead-tag{padding:5px 12px;border-radius:8px;font-size:11px;font-weight:600;display:flex;align-items:center;gap:6px}
.convo-lead-tag.intent-price{background:rgba(245,158,11,0.15);color:#f59e0b}
.convo-lead-tag.intent-timing{background:rgba(59,130,246,0.15);color:#3b82f6}
.convo-lead-tag.intent-ready{background:rgba(16,185,129,0.15);color:#10b981}
.convo-lead-tag.intent-objection{background:rgba(239,68,68,0.15);color:#ef4444}
.convo-lead-tag.intent-info{background:rgba(168,85,247,0.15);color:#a855f7}
.convo-lead-score{display:flex;align-items:center;gap:8px}
.convo-lead-score-bar{width:60px;height:6px;background:rgba(255,255,255,0.1);border-radius:3px;overflow:hidden}
.convo-lead-score-fill{height:100%;background:linear-gradient(90deg,#10b981,#14b8a6);transition:width 0.3s}

/* Messages Area */
.convo-messages{flex:1;overflow-y:auto;padding:20px;display:flex;flex-direction:column;gap:12px}
.convo-messages::-webkit-scrollbar{width:6px}
.convo-messages::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:3px}
.convo-msg{display:flex;max-width:75%}
.convo-msg.outbound{margin-left:auto;flex-direction:row-reverse}
.convo-msg.inbound{margin-right:auto}
.convo-msg-content{}
.convo-msg-bubble{padding:14px 18px;border-radius:20px;font-size:14px;line-height:1.6;position:relative}
.convo-msg.outbound .convo-msg-bubble{background:linear-gradient(135deg,#14b8a6,#0d9488);color:#fff;border-bottom-right-radius:6px}
.convo-msg.inbound .convo-msg-bubble{background:rgba(255,255,255,0.08);color:#fff;border-bottom-left-radius:6px}
.convo-msg-meta{display:flex;align-items:center;gap:8px;margin-top:6px;font-size:11px;color:rgba(255,255,255,0.4)}
.convo-msg.outbound .convo-msg-meta{justify-content:flex-end}
.convo-msg-status{display:flex;align-items:center;gap:3px}
.convo-msg-status.delivered{color:#10b981}
.convo-msg-status.sent{color:#3b82f6}
.convo-msg-status.failed{color:#ef4444}
.convo-msg-emotion{font-size:10px;padding:2px 8px;border-radius:10px;background:rgba(255,255,255,0.08)}

/* AI Analysis Badge on Messages */
.convo-msg-ai-tag{position:absolute;top:-8px;right:10px;font-size:9px;padding:2px 6px;border-radius:6px;background:rgba(139,92,246,0.2);color:#a855f7;font-weight:600}

/* Smart Composer */
.convo-composer{padding:16px 20px;border-top:1px solid var(--border);background:rgba(0,0,0,0.4)}
.convo-composer-top{display:flex;gap:8px;margin-bottom:12px;align-items:center}
.convo-composer-label{font-size:11px;color:rgba(255,255,255,0.4);font-weight:600;text-transform:uppercase;letter-spacing:0.5px}
.convo-tone-btns{display:flex;gap:6px;flex:1}
.convo-tone-btn{padding:6px 14px;border-radius:20px;font-size:11px;font-weight:600;cursor:pointer;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.08);transition:all 0.2s}
.convo-tone-btn:hover{background:rgba(255,255,255,0.08);color:#fff}
.convo-tone-btn.active{background:rgba(20,184,166,0.15);color:#14b8a6;border-color:rgba(20,184,166,0.3)}
.convo-goal-btns{display:flex;gap:6px}
.convo-goal-btn{padding:6px 12px;border-radius:20px;font-size:11px;font-weight:600;cursor:pointer;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.08);transition:all 0.2s}
.convo-goal-btn:hover{background:rgba(255,255,255,0.08);color:#fff}
.convo-goal-btn.active{background:rgba(139,92,246,0.15);color:#a855f7;border-color:rgba(139,92,246,0.3)}
.convo-input-wrap{display:flex;gap:12px;align-items:flex-end}
.convo-input{flex:1;padding:14px 18px;border-radius:24px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.03);color:#fff;font-size:14px;resize:none;min-height:50px;max-height:120px;font-family:inherit;line-height:1.5}
.convo-input:focus{outline:none;border-color:#14b8a6;background:rgba(20,184,166,0.03)}
.convo-input::placeholder{color:rgba(255,255,255,0.3)}
.convo-send-btn{width:50px;height:50px;border-radius:50%;border:none;background:linear-gradient(135deg,#14b8a6,#0d9488);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:20px;transition:all 0.2s;flex-shrink:0}
.convo-send-btn:hover{transform:scale(1.08);box-shadow:0 4px 20px rgba(20,184,166,0.4)}
.convo-send-btn:active{transform:scale(0.95)}
.convo-composer-preview{font-size:11px;color:rgba(255,255,255,0.4);margin-top:10px;display:flex;align-items:center;gap:6px}
.convo-composer-preview.good{color:#10b981}
.convo-composer-preview.warn{color:#f59e0b}

/* Right Panel - Aria Copilot */
.aria-copilot-header{padding:20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:14px;background:linear-gradient(180deg,rgba(139,92,246,0.08),transparent)}
.aria-copilot-avatar{width:44px;height:44px;border-radius:14px;background:linear-gradient(135deg,#8b5cf6,#6366f1);display:flex;align-items:center;justify-content:center;font-size:20px;position:relative}
.aria-copilot-avatar::after{content:'';position:absolute;bottom:2px;right:2px;width:10px;height:10px;background:#10b981;border-radius:50%;border:2px solid var(--black)}
.aria-copilot-title{font-weight:700;font-size:16px}
.aria-copilot-subtitle{font-size:11px;color:rgba(255,255,255,0.5);margin-top:2px}
.aria-copilot-badge{margin-left:auto;font-size:10px;padding:4px 10px;border-radius:12px;background:rgba(139,92,246,0.15);color:#a855f7;font-weight:600}

.aria-section{padding:18px 20px;border-bottom:1px solid rgba(255,255,255,0.05)}
.aria-section-title{font-size:11px;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;margin-bottom:14px;display:flex;align-items:center;gap:8px}
.aria-section-title span{font-size:14px}

/* AI Analysis Card */
.aria-analysis{padding:16px;background:linear-gradient(135deg,rgba(139,92,246,0.08),rgba(99,102,241,0.05));border:1px solid rgba(139,92,246,0.15);border-radius:14px;margin-bottom:12px}
.aria-analysis-header{display:flex;align-items:center;gap:10px;margin-bottom:12px}
.aria-analysis-icon{font-size:18px}
.aria-analysis-label{font-size:12px;font-weight:600;color:#a855f7}
.aria-analysis-text{font-size:13px;color:rgba(255,255,255,0.85);line-height:1.6}
.aria-analysis-tags{display:flex;flex-wrap:wrap;gap:6px;margin-top:12px}
.aria-analysis-tag{font-size:10px;padding:4px 10px;border-radius:10px;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.6)}

/* Suggested Replies */
.aria-reply{padding:14px 16px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:14px;cursor:pointer;transition:all 0.2s;margin-bottom:10px}
.aria-reply:hover{background:rgba(20,184,166,0.08);border-color:rgba(20,184,166,0.3);transform:translateX(4px)}
.aria-reply-text{font-size:13px;color:rgba(255,255,255,0.85);line-height:1.5;margin-bottom:10px}
.aria-reply-meta{display:flex;justify-content:space-between;align-items:center}
.aria-reply-type{font-size:10px;padding:3px 8px;border-radius:8px;font-weight:600}
.aria-reply-type.nepq{background:rgba(16,185,129,0.15);color:#10b981}
.aria-reply-type.soft{background:rgba(59,130,246,0.15);color:#3b82f6}
.aria-reply-type.direct{background:rgba(245,158,11,0.15);color:#f59e0b}
.aria-reply-use{font-size:11px;color:#14b8a6;font-weight:600;display:flex;align-items:center;gap:4px}

/* Quick Actions */
.aria-actions-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
.aria-action{display:flex;flex-direction:column;align-items:center;gap:8px;padding:14px 12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;cursor:pointer;transition:all 0.2s;text-align:center}
.aria-action:hover{background:rgba(20,184,166,0.08);border-color:rgba(20,184,166,0.2);transform:translateY(-2px)}
.aria-action-icon{width:36px;height:36px;border-radius:10px;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;font-size:16px}
.aria-action-text{font-size:11px;font-weight:600;color:rgba(255,255,255,0.7)}

/* Message Type Templates */
.aria-templates{display:flex;flex-wrap:wrap;gap:8px}
.aria-template{padding:8px 14px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.6);cursor:pointer;transition:all 0.2s;border:1px solid transparent;display:flex;align-items:center;gap:6px}
.aria-template:hover{background:rgba(20,184,166,0.1);color:#14b8a6;border-color:rgba(20,184,166,0.2)}

/* Empty States */
.convo-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:var(--gray-500);text-align:center;padding:40px}
.convo-empty-icon{font-size:72px;margin-bottom:24px;opacity:0.6}
.convo-empty-title{font-size:20px;font-weight:700;margin-bottom:10px;color:#fff}
.convo-empty-subtitle{font-size:14px;max-width:300px;line-height:1.6;color:rgba(255,255,255,0.5);margin-bottom:24px}
.convo-empty-btn{padding:12px 24px;border-radius:12px;background:linear-gradient(135deg,#14b8a6,#0d9488);color:#fff;font-weight:600;border:none;cursor:pointer;font-size:14px;transition:all 0.2s}
.convo-empty-btn:hover{transform:translateY(-2px);box-shadow:0 4px 20px rgba(20,184,166,0.3)}

/* List Empty */
.convo-list-empty{padding:40px 20px;text-align:center;color:rgba(255,255,255,0.4)}
.convo-list-empty-icon{font-size:48px;margin-bottom:16px;opacity:0.5}
.convo-list-empty-text{font-size:14px;margin-bottom:8px}
.convo-list-empty-sub{font-size:12px;color:rgba(255,255,255,0.3)}

/* Typing Indicator */
.convo-typing{display:flex;align-items:center;gap:8px;padding:12px 16px;background:rgba(255,255,255,0.05);border-radius:20px;margin-bottom:12px;max-width:100px}
.convo-typing-dot{width:8px;height:8px;background:rgba(255,255,255,0.4);border-radius:50%;animation:typing 1.4s infinite}
.convo-typing-dot:nth-child(2){animation-delay:0.2s}
.convo-typing-dot:nth-child(3){animation-delay:0.4s}
@keyframes typing{0%,100%{opacity:0.4;transform:scale(1)}50%{opacity:1;transform:scale(1.2)}}

/* Responsive */
@media (max-width:1200px){
.convo-hub{grid-template-columns:300px 1fr}
.convo-right{display:none}
}
@media (max-width:768px){
.convo-hub{grid-template-columns:1fr}
.convo-left{display:none}
.convo-left.mobile-show{display:flex;position:fixed;inset:0;z-index:1000;background:var(--black)}
}
</style>

<div class="convo-hub">
<!-- LEFT PANEL: Smart Conversation Feed -->
<div class="convo-left" id="convo-left-panel">
<div class="convo-left-header">
<div class="convo-left-title">
<span>💬</span> Conversations
<button onclick="openModal('new-sms-modal')" class="btn btn-sm btn-primary" style="margin-left:auto;font-size:11px;padding:8px 14px">+ New</button>
</div>
<input type="text" class="convo-search" placeholder="🔍 Search by name, phone, or message..." id="convo-search-input" oninput="filterConversations(this.value)">
<div class="convo-filters">
<button class="convo-filter active" data-filter="all" onclick="setConvoFilter('all')">All</button>
<button class="convo-filter" data-filter="unread" onclick="setConvoFilter('unread')">🔴 Unread</button>
<button class="convo-filter" data-filter="follow-up" onclick="setConvoFilter('follow-up')">⏰ Follow-up</button>
<button class="convo-filter" data-filter="hot" onclick="setConvoFilter('hot')">🔥 Hot</button>
<button class="convo-filter" data-filter="booked" onclick="setConvoFilter('booked')">✅ Booked</button>
</div>
</div>
<div class="convo-list" id="convo-list">
<div class="convo-list-empty">
<div class="convo-list-empty-icon">💬</div>
<div class="convo-list-empty-text">No conversations yet</div>
<div class="convo-list-empty-sub">Start a new conversation to begin</div>
</div>
</div>
</div>

<!-- CENTER PANEL: Thread -->
<div class="convo-center">
<div id="convo-thread-area">
<div class="convo-empty">
<div class="convo-empty-icon">💬</div>
<div class="convo-empty-title">Select a Conversation</div>
<div class="convo-empty-subtitle">Choose a conversation from the list or start a new one to begin messaging.</div>
<button class="convo-empty-btn" onclick="openModal('new-sms-modal')">+ Start New Conversation</button>
</div>
</div>
</div>

<!-- RIGHT PANEL: Aria Copilot -->
<div class="convo-right" id="aria-copilot-panel">
<div class="aria-copilot-header">
<div class="aria-copilot-avatar">🤖</div>
<div>
<div class="aria-copilot-title">Aria Copilot</div>
<div class="aria-copilot-subtitle">Conversation Intelligence</div>
</div>
<div class="aria-copilot-badge">AI Active</div>
</div>

<div id="aria-copilot-content" style="flex:1;overflow-y:auto">
<div class="aria-section">
<div class="aria-section-title"><span>🧠</span> Analysis</div>
<div class="aria-analysis">
<div class="aria-analysis-header">
<div class="aria-analysis-icon">💡</div>
<div class="aria-analysis-label">Waiting for conversation</div>
</div>
<div class="aria-analysis-text">Select a conversation to get AI-powered insights, suggested replies, and smart actions.</div>
</div>
</div>

<div class="aria-section">
<div class="aria-section-title"><span>⚡</span> Quick Actions</div>
<div class="aria-actions-grid">
<div class="aria-action" onclick="openModal('new-sms-modal')">
<div class="aria-action-icon">✉️</div>
<div class="aria-action-text">New Message</div>
</div>
<div class="aria-action" onclick="toast('Select a conversation first')">
<div class="aria-action-icon">📞</div>
<div class="aria-action-text">AI Call</div>
</div>
<div class="aria-action" onclick="toast('Select a conversation first')">
...
<div class="aria-action" onclick="toast('Select a conversation first')">
<div class="aria-action-icon">👤</div>
<div class="aria-action-text">View Lead</div>
</div>
</div>
</div>

<div class="aria-section">
<div class="aria-section-title"><span>📝</span> Message Templates</div>
<div class="aria-templates">
<button class="aria-template" onclick="useTemplate('opener')">👋 Opener</button>
<button class="aria-template" onclick="useTemplate('followup')">🔄 Follow-up</button>
<button class="aria-template" onclick="useTemplate('booking')">📅 Book Call</button>
<button class="aria-template" onclick="useTemplate('checkin')">💬 Check-in</button>
<button class="aria-template" onclick="useTemplate('reengage')">🔥 Re-engage</button>
</div>
</div>
...
<div class="header"><div><h1>✅ Compliance Audit Trail</h1><div class="header-sub">Visual execution tracking for every lead - 3 calls/day, 7 days/week</div></div>
<div style="display:flex;gap:8px">
<button class="btn btn-primary" onclick="openModal('test-lead-modal')">➕ Test Lead</button>
<button class="btn btn-secondary" onclick="loadCompliance()">🔄 Refresh</button>
</div></div>

<div class="stats-grid">
<div class="stat cyan"><div class="stat-value" id="comp-total">0</div><div class="stat-label">Total Leads</div></div>
<div class="stat green"><div class="stat-value" id="comp-contacted">0</div><div class="stat-label">Contacted</div></div>
<div class="stat orange"><div class="stat-value" id="comp-pending">0</div><div class="stat-label">Pending</div></div>
<div class="stat"><div class="stat-value" id="comp-complete">0</div><div class="stat-label">Sequence Complete</div></div>
</div>

<div class="card">
<div class="card-header"><h2>📋 Lead Compliance Timeline</h2>
<div style="display:flex;gap:8px;align-items:center">
<select id="comp-filter" onchange="loadCompliance()" style="padding:8px 12px;border-radius:6px;background:var(--gray-900);border:1px solid var(--border);color:#fff">
...
<div id="compliance-list" style="padding:16px">
<div style="text-align:center;padding:40px;color:var(--gray-500)">Loading compliance data...</div>
</div>
</div>
</div>
...
<div class="card">
<div class="card-header"><h2>📱 Unified Phone System</h2><div class="header-sub">ONE number for Voice + SMS</div></div>
<div style="padding:20px">

<!-- Primary Number - THE ONE -->
<div style="background:linear-gradient(135deg,rgba(20,184,166,0.1),rgba(59,130,246,0.05));border:2px solid rgba(20,184,166,0.3);border-radius:16px;padding:20px;margin-bottom:20px">
<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px">
<div style="display:flex;align-items:center;gap:16px">
<div style="width:56px;height:56px;background:linear-gradient(135deg,#14b8a6,#3b82f6);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px;box-shadow:0 4px 12px rgba(20,184,166,0.3)">⚡</div>
<div>
<div style="font-size:11px;color:#14b8a6;font-weight:600;letter-spacing:1px;margin-bottom:4px">PRIMARY NUMBER</div>
<div style="font-weight:700;font-size:22px;margin-bottom:2px">+1 (720) 818-9512</div>
<div style="font-size:13px;color:rgba(255,255,255,0.6)">Twilio → Retell SIP • Voice + SMS unified</div>
</div>
</div>
<div style="display:flex;gap:20px">
<div style="text-align:center">
<div style="font-size:24px">📞</div>
<div style="font-size:11px;color:#22c55e;font-weight:700">VOICE ✓</div>
<div style="font-size:10px;color:rgba(255,255,255,0.4)">via Retell AI</div>
</div>
<div style="text-align:center">
<div style="font-size:24px">💬</div>
<div style="font-size:11px;color:#22c55e;font-weight:700">SMS ✓</div>
<div style="font-size:10px;color:rgba(255,255,255,0.4)">via Twilio</div>
</div>
</div>
</div>
<div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap">
<button class="btn btn-primary" onclick="testSmsNumber()">📱 Test SMS</button>
<button class="btn btn-secondary" onclick="testVoiceCall()">📞 Test Call</button>
<button class="btn btn-secondary" onclick="doubleTap()">⚡ Double Tap (SMS + Call)</button>
</div>
</div>

<!-- How It Works -->
<div style="background:rgba(0,0,0,0.2);border-radius:12px;padding:16px;margin-bottom:20px">
<div style="font-weight:600;margin-bottom:12px;display:flex;align-items:center;gap:8px">🔧 How It Works</div>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;font-size:13px">
<div style="background:rgba(255,255,255,0.03);padding:12px;border-radius:8px">
<div style="color:#14b8a6;font-weight:600;margin-bottom:4px">1. Number Lives in Twilio</div>
<div style="color:rgba(255,255,255,0.6)">You own +17208189512 in Twilio. Full control.</div>
</div>
<div style="background:rgba(255,255,255,0.03);padding:12px;border-radius:8px">
<div style="color:#3b82f6;font-weight:600;margin-bottom:4px">2. Voice → Retell via SIP</div>
<div style="color:rgba(255,255,255,0.6)">Elastic SIP Trunk routes calls to Retell AI agents.</div>
</div>
<div style="background:rgba(255,255,255,0.03);padding:12px;border-radius:8px">
<div style="color:#22c55e;font-weight:600;margin-bottom:4px">3. SMS → Twilio API</div>
<div style="color:rgba(255,255,255,0.6)">Text messages sent directly via Twilio. Instant.</div>
</div>
<div style="background:rgba(255,255,255,0.03);padding:12px;border-radius:8px">
<div style="color:#f59e0b;font-weight:600;margin-bottom:4px">4. Same Number = Trust</div>
<div style="color:rgba(255,255,255,0.6)">Customer sees same number for call AND text.</div>
</div>
</div>
</div>

<!-- Setup Status -->
<div id="setup-status" style="background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:12px;padding:16px">
<div style="font-weight:600;margin-bottom:12px">⚙️ Setup Checklist</div>
<div id="setup-checklist" style="display:grid;gap:8px">
<div class="setup-item" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:rgba(34,197,94,0.1);border-radius:8px">
<span style="color:#22c55e">✓</span>
<span>Twilio Account Connected</span>
</div>
<div class="setup-item" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:rgba(34,197,94,0.1);border-radius:8px">
<span style="color:#22c55e">✓</span>
<span>Phone Number Purchased (+17208189512)</span>
</div>
<div class="setup-item" id="sip-trunk-status" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:rgba(251,191,36,0.1);border-radius:8px">
<span style="color:#fbbf24">○</span>
<span>Elastic SIP Trunk Configured</span>
<a href="https://console.twilio.com/us1/develop/sip-trunking/trunks" target="_blank" style="margin-left:auto;color:#3b82f6;font-size:12px">Setup →</a>
</div>
<div class="setup-item" id="retell-import-status" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:rgba(251,191,36,0.1);border-radius:8px">
<span style="color:#fbbf24">○</span>
<span>Number Imported to Retell</span>
<a href="https://dashboard.retellai.com/phone-numbers" target="_blank" style="margin-left:auto;color:#3b82f6;font-size:12px">Import →</a>
</div>
<div class="setup-item" id="a2p-status" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:rgba(251,191,36,0.1);border-radius:8px">
<span style="color:#fbbf24">○</span>
<span>A2P 10DLC Brand Registered (for SMS)</span>
<a href="https://console.twilio.com/us1/develop/sms/regulatory-compliance/overview" target="_blank" style="margin-left:auto;color:#3b82f6;font-size:12px">Register →</a>
</div>
</div>
<div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
<button class="btn btn-sm btn-primary" onclick="markSetupComplete('sip')">✓ SIP Done</button>
<button class="btn btn-sm btn-primary" onclick="markSetupComplete('retell')">✓ Retell Done</button>
<button class="btn btn-sm btn-primary" onclick="markSetupComplete('a2p')">✓ A2P Done</button>
</div>
</div>

<!-- Quick Setup Guide -->
<details style="margin-top:20px">
<summary style="cursor:pointer;font-weight:600;padding:12px;background:rgba(255,255,255,0.02);border-radius:8px">📖 Full Setup Guide (click to expand)</summary>
<div style="padding:16px;font-size:13px;line-height:1.8;color:rgba(255,255,255,0.8)">

<h4 style="color:#14b8a6;margin:16px 0 8px">Step 1: Create Elastic SIP Trunk in Twilio</h4>
<ol style="margin:0;padding-left:20px">
<li>Go to <a href="https://console.twilio.com/us1/develop/sip-trunking/trunks" target="_blank" style="color:#3b82f6">Twilio Console → Elastic SIP Trunking</a></li>
<li>Click "Create new SIP Trunk" → Name it "Retell"</li>
<li><strong>Termination (Outbound):</strong> Add URI like <code>voicelab.pstn.twilio.com</code></li>
<li><strong>Authentication:</strong> Create credentials (username/password) OR whitelist IPs: <code>0.0.0.0/1</code> and <code>128.0.0.0/1</code></li>
<li><strong>Origination (Inbound):</strong> Add URI <code>sip:sip.retellai.com</code></li>
</ol>

<h4 style="color:#3b82f6;margin:16px 0 8px">Step 2: Move Number to SIP Trunk</h4>
<ol style="margin:0;padding-left:20px">
<li>In Twilio Console → Phone Numbers → +17208189512</li>
<li>Under "Voice Configuration" → Select your SIP Trunk</li>
</ol>

<h4 style="color:#22c55e;margin:16px 0 8px">Step 3: Import to Retell</h4>
<ol style="margin:0;padding-left:20px">
<li>Go to <a href="https://dashboard.retellai.com/phone-numbers" target="_blank" style="color:#3b82f6">Retell Dashboard → Phone Numbers</a></li>
<li>Click "+" → "Connect via SIP trunking"</li>
<li>Enter: <code>+17208189512</code></li>
<li>Termination URI: Your Twilio termination URI</li>
<li>Username/Password: From Step 1</li>
<li>Bind to your AI agent</li>
</ol>

<h4 style="color:#f59e0b;margin:16px 0 8px">Step 4: A2P 10DLC Registration (for SMS)</h4>
<ol style="margin:0;padding-left:20px">
<li>Go to <a href="https://console.twilio.com/us1/develop/sms/regulatory-compliance/overview" target="_blank" style="color:#3b82f6">Twilio Console → Messaging → Regulatory Compliance</a></li>
<li>Register your Brand (business info) - ~$4 fee</li>
<li>Create a Campaign (describe your SMS use case) - ~$15 fee</li>
<li>Link your phone number to the campaign</li>
<li>Wait 1-7 days for approval (Twilio brand approval is faster than Retell's)</li>
</ol>

<div style="margin-top:16px;padding:12px;background:rgba(20,184,166,0.1);border-radius:8px;border-left:3px solid #14b8a6">
<strong>💡 Pro Tip:</strong> With Twilio, you register your BRAND once. Then ALL your numbers are covered. No 2-3 week wait per number like Retell.
</div>
...
<div class="modal-bg" id="gcal-setup-modal"><div class="modal" style="max-width:500px">
<h2>📅 Connect Google Calendar</h2>
<div style="background:rgba(20,184,166,0.1);border:1px solid rgba(20,184,166,0.3);border-radius:8px;padding:16px;margin:16px 0">
<div style="font-weight:600;color:#14b8a6;margin-bottom:8px">✨ What this does:</div>
<div style="font-size:13px;color:var(--gray-300);line-height:1.6">
• AI automatically creates calendar events when appointments are booked<br>
• Syncs appointment confirmations and reminders<br>
...
<ol style="font-size:13px;color:var(--gray-300);line-height:1.8;padding-left:20px;margin:0">
<li>Go to <a href="https://console.cloud.google.com/apis/credentials" target="_blank" style="color:var(--cyan)">Google Cloud Console</a></li>
<li>Create a new OAuth 2.0 Client ID (Web application)</li>
<li>Add redirect URI: <code style="background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px">${window.location.origin}/oauth/google/callback</code></li>
...
<input id="gcal-client-secret" type="password" placeholder="GOCSPX-xxxxx">
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('gcal-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="startGoogleAuth()">🔗 Connect Google Calendar</button>
</div>
</div></div>

<!-- Facebook Leads Setup Modal -->
<div class="modal-bg" id="facebook-setup-modal"><div class="modal" style="max-width:550px">
...
<div style="font-weight:600;color:#3b82f6;margin-bottom:8px">✨ What this does:</div>
<div style="font-size:13px;color:var(--gray-300);line-height:1.6">
• Auto-imports leads from your Facebook Lead Ads<br>
• AI calls each lead within 60 seconds<br>
• Tracks compliance for every lead
</div>
</div>

<div style="background:rgba(255,255,255,0.03);border-radius:8px;padding:16px;margin-bottom:16px">
<div style="font-weight:600;margin-bottom:12px">🔗 Your Webhook URL:</div>
<div style="display:flex;gap:8px;align-items:center">
<input id="fb-webhook-url" readonly value="" style="flex:1;font-family:monospace;font-size:12px;background:rgba(0,0,0,0.3)">
<button class="btn btn-sm btn-primary" onclick="copyFacebookWebhook()">📋 Copy</button>
</div>
</div>

<div style="background:rgba(255,255,255,0.03);border-radius:8px;padding:16px;margin-bottom:16px">
<div style="font-weight:600;margin-bottom:12px">📋 Connect via Zapier (Easiest):</div>
<ol style="font-size:13px;color:var(--gray-300);line-height:1.8;padding-left:20px;margin:0">
<li>Go to <a href="https://zapier.com/app/zaps" target="_blank" style="color:var(--cyan)">Zapier</a> and create a new Zap</li>
<li><strong>Trigger:</strong> Facebook Lead Ads → New Lead</li>
<li><strong>Action:</strong> Webhooks by Zapier → POST</li>
<li>Paste the webhook URL above</li>
<li>Map fields: phone, first_name, last_name, email</li>
...
<strong>💡 Pro Tip:</strong> Add <code style="background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px">"agent_type": "roofing"</code> to customize which AI agent calls your leads.
</div>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('facebook-setup-modal')">Close</button>
<button class="btn btn-primary" onclick="testFacebookWebhook()">🧪 Send Test Lead</button>
</div>
</div></div>

<!-- Test Lead Modal for Compliance Testing -->
<div class="modal-bg" id="test-lead-modal"><div class="modal" style="max-width:450px">
<h2>🧪 Add Test Lead</h2>
<div style="background:rgba(20,184,166,0.1);border:1px solid rgba(20,184,166,0.3);border-radius:8px;padding:12px;margin:16px 0;font-size:13px;color:#5eead4">
<strong>⚡ This simulates a Facebook lead:</strong> Lead will be saved, AI will call immediately, and you can track the full compliance flow.
</div>

<div class="form-group">
<label>Phone Number *</label>
<input id="test-lead-phone" type="tel" placeholder="(702) 555-1234">
</div>

<div class="form-row" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
<div class="form-group">
<label>First Name</label>
<input id="test-lead-fname" placeholder="John">
</div>
<div class="form-group">
<label>Last Name</label>
<input id="test-lead-lname" placeholder="Smith">
</div>
</div>

<div class="form-group">
<label>Email</label>
<input id="test-lead-email" type="email" placeholder="john@example.com">
</div>

<div class="form-group">
<label>Industry / Agent Type</label>
<select id="test-lead-agent">
<option value="roofing">🏠 Roofing</option>
<option value="solar">☀️ Solar</option>
<option value="hvac">❄️ HVAC</option>
<option value="medical">🏥 Medical</option>
...
<input type="checkbox" id="test-lead-call" checked style="width:18px;height:18px">
<span>📞 Call lead immediately after adding</span>
</label>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('test-lead-modal')">Cancel</button>
<button class="btn btn-primary" onclick="submitTestLead()">➕ Add Test Lead</button>
</div>
</div></div>

<script>
// Set webhook URL when modal opens
document.addEventListener('DOMContentLoaded', function(){
    var fbUrl = document.getElementById('fb-webhook-url');
    if(fbUrl) fbUrl.value = window.location.origin + '/api/zapier/facebook-lead';
});
</script>
...
<!-- New SMS Modal -->
<div class="modal-bg" id="new-sms-modal"><div class="modal" style="max-width:480px">
<h2>💬 Start New Conversation</h2>
<div class="form-group"><label>Phone Number *</label><input id="new-sms-phone" type="tel" placeholder="(555) 123-4567" style="font-size:16px"></div>
<div class="form-group"><label>Contact Name</label><input id="new-sms-name" placeholder="John Smith (optional)"></div>
<div class="form-group">
<label>Quick Templates</label>
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px">
<button type="button" class="btn btn-sm" style="background:rgba(255,255,255,0.05);font-size:11px" onclick="$('new-sms-msg').value='Hi! This is Hailey from our team. I saw you were interested in learning more. Is now a good time to chat?'">👋 Opener</button>
<button type="button" class="btn btn-sm" style="background:rgba(255,255,255,0.05);font-size:11px" onclick="$('new-sms-msg').value='Hey! Just following up on your inquiry. Would you have a few minutes for a quick call?'">🔄 Follow-up</button>
<button type="button" class="btn btn-sm" style="background:rgba(255,255,255,0.05);font-size:11px" onclick="$('new-sms-msg').value='Hi! I\\'d love to schedule a quick call to discuss how we can help. Would tomorrow at 10am or 2pm work better?'">📅 Book Call</button>
</div>
</div>
<div class="form-group"><label>Message</label><textarea id="new-sms-msg" rows="4" placeholder="Type your message..." style="font-size:14px;line-height:1.5"></textarea></div>
<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:16px">💡 Tip: A friendly, personal message gets 3x more responses than generic templates.</div>
<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('new-sms-modal')">Cancel</button>
<button class="btn btn-primary" onclick="startNewConvo()">📤 Send Message</button>
</div>
</div></div>
    ...
    def do_GET(self):
        ...
        # Compliance API
            ...
            try:
                ...
                # Get consent count
                c.execute("SELECT COUNT(*) FROM consent_log")
                consent_count = c.fetchone()[0]
                
                # Get DNC count
                c.execute("SELECT COUNT(*) FROM internal_dnc")
                dnc_count = c.fetchone()[0]
                
                # Get contacts today
                c.execute("SELECT COUNT(*) FROM contact_log WHERE created_at > datetime('now', '-24 hours')")
                contacts_today = c.fetchone()[0]
                
                # Get blocked today
                c.execute("SELECT COUNT(*) FROM contact_log WHERE status = 'blocked' AND created_at > datetime('now', '-24 hours')")
                ...
                self.send_json({
                    ...
                    'compliance_rate': round((1 - blocked_today / max(contacts_today, 1)) * 100, 1)
                })
            except Exception as e:
                self.send_json({'error': str(e)})
        elif path == '/api/compliance/dnc':
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                c.execute("SELECT * FROM internal_dnc ORDER BY created_at DESC LIMIT 100")
                dnc_list = [dict(row) for row in c.fetchall()]
                conn.close()
                self.send_json(dnc_list)
            except:
                self.send_json([])
        elif path == '/api/compliance/consent-log':
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                c.execute("SELECT * FROM consent_log ORDER BY created_at DESC LIMIT 100")
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
        elif path == '/api/compliance' or path.startswith('/api/compliance?'):
            try:
                ...
                if filter_type == 'today':
                    where_clause = "WHERE DATE(l.created_at) = DATE('now')"
                elif filter_type == 'pending':
                    where_clause = "WHERE l.status = 'new' OR l.status = 'no_answer'"
                elif filter_type == 'contacted':
                    where_clause = "WHERE l.status IN ('contacted', 'appointment_set', 'sold')"
                
                # Get leads with their compliance data
                c.execute(f'''
                    SELECT l.*, 
                           ls.started_at as text_sent_at,
                           ls.last_call_at as call_1_at,
                           ls.current_slot as sequence_day
                    FROM leads l
                    ...
                    LIMIT 100
                ''')
                leads_raw = c.fetchall()
                
                # Get call log for each lead to populate timeline
                leads = []
                for lead_row in leads_raw:
                    ...
                    c.execute('''
                        SELECT created_at, status FROM call_log 
                        WHERE phone LIKE ? 
                        ORDER BY created_at ASC
                    ''', (f"%{lead.get('phone', '')[-10:]}%",))
                    calls = c.fetchall()
                    
                    # Map calls to timeline slots
                    if len(calls) >= 1:
                        lead['call_1_at'] = calls[0]['created_at']
                    if len(calls) >= 2:
                        lead['call_2_at'] = calls[1]['created_at']
                    if len(calls) >= 3:
                        lead['call_1215_at'] = calls[2]['created_at']
                    if len(calls) >= 4:
                        lead['call_1215_double_at'] = calls[3]['created_at']
                    if len(calls) >= 5:
                        lead['call_530_at'] = calls[4]['created_at']
                    if len(calls) >= 6:
                        lead['call_530_double_at'] = calls[5]['created_at']
                    
                    # Calculate sequence day (based on days since created)
                    if not lead.get('sequence_day'):
                        try:
                            from datetime import datetime
                            created = datetime.strptime(lead['created_at'][:10], '%Y-%m-%d')
                            today = datetime.now()
                            lead['sequence_day'] = min(7, (today - created).days + 1)
                        except:
                            lead['sequence_day'] = 1
                ...
                total = c.fetchone()[0]
                c.execute("SELECT COUNT(*) FROM leads WHERE status IN ('contacted', 'appointment_set', 'sold')")
                contacted = c.fetchone()[0]
                c.execute("SELECT COUNT(*) FROM leads WHERE status = 'new' OR status = 'no_answer'")
                pending = c.fetchone()[0]
                c.execute("SELECT COUNT(*) FROM lead_sequences WHERE current_slot >= 21")  # 3 calls * 7 days
                ...
                import traceback
                traceback.print_exc()
                self.send_json({"leads": [], "stats": {}})
        
        elif path == '/api/phone-config':
            # Get phone number configurations
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                c.execute('SELECT * FROM phone_config ORDER BY id')
                phones = [dict(row) for row in c.fetchall()]
                conn.close()
                self.send_json(phones)
            except Exception as e:
                # Return defaults if table doesn't exist yet
                self.send_json([
                    {"id": 1, "phone": PRIMARY_PHONE, "name": "Primary (Unified)", "type": "primary", "provider": "twilio", "voice_enabled": True, "sms_enabled": True}
                ])
        else:
            self.send_error(404)
    def do_POST(self):
        try:
            ...
            b = self.rfile.read(l).decode('utf-8') if l > 0 else '{}'
            
            # Check content type - Twilio sends form-urlencoded
            content_type = self.headers.get('Content-Type', '')
            if 'application/x-www-form-urlencoded' in content_type:
                # Parse form data
                from urllib.parse import parse_qs
                parsed = parse_qs(b)
                d = {k: v[0] if len(v) == 1 else v for k, v in parsed.items()}
            else:
        ...
        # SMS API POST routes
            ...
            result = create_sms_conversation(d.get('phone'), d.get('contact_name'), d.get('message'))
            self.send_json(result)
        ...
        # Phone config update
            ...
            try:
                ...
                if sms_enabled:
                    c.execute("UPDATE phone_config SET sms_enabled = 1, sms_approved_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE phone = ?", (phone,))
                else:
                    c.execute("UPDATE phone_config SET sms_enabled = 0, updated_at = CURRENT_TIMESTAMP WHERE phone = ?", (phone,))
                
                conn.commit()
                conn.close()
                
                print(f"📱 Phone config updated: {phone} SMS={sms_enabled}")
                self.send_json({"success": True})
            except Exception as e:
                print(f"❌ Phone config error: {e}")
                self.send_json({"success": False, "error": str(e)})
        
        # Twilio Status Webhook - receives delivery updates
        elif path == '/api/twilio/status':
            try:
                # Twilio sends form-urlencoded data, parse it
                twilio_sid = d.get('MessageSid') or d.get('SmsSid', '')
                status = d.get('MessageStatus') or d.get('SmsStatus', '')
                ...
                if twilio_sid and status:
                    ...
                    c.execute("UPDATE sms_messages SET status = ? WHERE twilio_sid = ?", (status, twilio_sid))
                    updated = c.rowcount
                    
                    # Also update the sms_log table if it exists there
                    c.execute("UPDATE sms_log SET status = ? WHERE twilio_sid = ?", (status, twilio_sid))
                    
                    conn.commit()
                    conn.close()
                    
                    print(f"📱 SMS Status Update: {twilio_sid} → {status}")
        ...
        # Twilio Inbound SMS Webhook - receives incoming messages
        elif path == '/api/twilio/inbound':
            try:
                from_phone = d.get('From', '')
                body = d.get('Body', '')
                twilio_sid = d.get('MessageSid', '')
                
                if from_phone and body:
                    print(f"📨 Incoming SMS from {from_phone}: {body}")
                    
                    # Check for STOP/opt-out keywords
                    is_opt_out, reply = process_sms_keywords(from_phone, body)
                    ...
                    if not is_opt_out:
                        ...
                        c.execute("SELECT id FROM sms_conversations WHERE phone = ?", (from_phone,))
                        row = c.fetchone()
                        ...
                        c.execute("""INSERT INTO sms_messages 
                                    (conversation_id, direction, message, status, twilio_sid) 
                                    VALUES (?, 'inbound', ?, 'received', ?)""",
                                  (convo_id, body, twilio_sid))
                        
                        # Update conversation and increment unread
                        c.execute("""UPDATE sms_conversations 
                        ...
                        if OWNER_PHONE and OWNER_PHONE != from_phone:
                            notify_msg = f"📨 New SMS from {from_phone}:\n{body[:100]}{'...' if len(body) > 100 else ''}"
                            # Don't notify for now to avoid SMS loops
                            print(notify_msg)
                ...
                print(f"❌ Inbound SMS webhook error: {e}")
                import traceback
                traceback.print_exc()
                self.send_response(200)
                self.send_header('Content-Type', 'text/xml')
            ...
            try:
                result = authenticate_user(d.get('email', ''), d.get('password', ''))
                self.send_json(result)
            except Exception as e:
                print(f"Login error: {e}")
                self.send_json({"error": "Login failed"})
        ...
        # Test call from Test Lab - uses PRIMARY_PHONE (unified number)
        elif path == '/api/test-call':
            phone = d.get('phone', '')
            agent = d.get('agent', 'receptionist')
            print(f"🧪 TEST CALL: phone={phone}, agent={agent}")
            ...
            if not phone:
                ...
                try:
                    ...
                    # Use Retell API to make call from PRIMARY number
                    response = requests.post(
                        'https://api.retellai.com/v2/create-phone-call',
                        headers={
                            'Authorization': f'Bearer {RETELL_API_KEY}',
                        ...
                        json={
                            'from_number': PRIMARY_PHONE,  # Unified number
                            'to_number': formatted_phone,
                            'agent_id': 'agent_862cd6cf87f7b4d68a6986b3e9',  # Default receptionist
                            'retell_llm_dynamic_variables': {
                                'customer_name': 'there',
                                'company_name': COMPANY_NAME
                            }
                    ...
                    if response.status_code in [200, 201]:
                        data = response.json()
                        call_id = data.get('call_id', '')
                        print(f"   ✅ Test call initiated: {call_id}")
                        self.send_json({"success": True, "call_id": call_id})
                    else:
                        print(f"   ❌ Test call failed: {response.text}")
                        self.send_json({"success": False, "error": response.text[:200]})
        ...
        # Demo call from landing page
            ...
            print(f"📞 DEMO CALL: agent_type={agent_type}, phone={phone}, is_inbound={is_inbound}")
            if not phone:
                self.send_json({"success": False, "error": "Phone number required"})
            else:
                # Prefix with 'inbound_' if inbound mode selected
                if is_inbound and not agent_type.startswith('inbound_'):
                    agent_type = 'inbound_' + agent_type
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
        # ZAPIER/FACEBOOK WEBHOOK - Auto-calls leads immediately!
        # ═══════════════════════════════════════════════════════════════════════════════
        elif path == '/api/zapier/facebook-lead':
            try:
                phone = d.get('phone') or d.get('phone_number') or d.get('mobile') or ''
                first_name = d.get('first_name') or d.get('name') or d.get('full_name', '').split()[0] if d.get('full_name') else 'there'
                ...
                if not phone or len(phone_digits) < 10:
                    print(f"❌ Zapier webhook: Invalid phone number: {phone}")
                    self.send_json({"success": False, "error": "Valid phone number required"})
                    return
                
                agent_type = d.get('agent_type') or d.get('industry') or 'roofing'
                agent_type = agent_type.lower().replace(' ', '_')
                
                print(f"")
                print(f"🔥🔥🔥 FACEBOOK LEAD RECEIVED 🔥🔥🔥")
                ...
                # Save to leads table - use timeout-enabled connection
                conn = get_db_connection()
                c = conn.cursor()
                c.execute('''INSERT INTO leads (phone, first_name, last_name, email, source, status, agent_type, created_at)
                             VALUES (?, ?, ?, ?, 'facebook', 'new', ?, CURRENT_TIMESTAMP)''',
                          (phone, first_name, last_name, email, agent_type))
                lead_id = c.lastrowid
                
                # Also add to lead_sequences so it shows in pipeline
                c.execute('''INSERT OR IGNORE INTO lead_sequences (lead_id, phone, sequence_status, current_day, current_slot)
                             VALUES (?, ?, 'active', 1, 1)''', (lead_id, phone))
                conn.commit()
                conn.close()
                ...
                # Notify owner
                notify_owner_new_lead({
                    'first_name': first_name,
                    'last_name': last_name,
                    'phone': phone,
                    'email': email,
                    'industry': d.get('industry', 'Facebook Lead'),
                    'source': 'facebook'
                })
                
                # STEP 1: Send SMS FIRST
                print(f"   💬 SENDING INTRO SMS...")
                sms_result = send_lead_intro_sms(phone, first_name, agent_type)
                if sms_result.get('success'):
                    print(f"   ✅ SMS sent successfully")
                    update_compliance_step(lead_id, 'text_sent')
                else:
                    print(f"   ⚠️ SMS failed: {sms_result.get('error')}")
                
                # STEP 2: Brief delay before calling
                import time
                time.sleep(2)
                
                # STEP 3: AUTO-CALL
                print(f"   📞 AUTO-CALLING NOW...")
                call_result = make_call(phone, first_name, agent_type=agent_type, is_test=False)
                
                if call_result.get('success'):
                    call_id = call_result.get('call_id')
                    print(f"   ✅ Call initiated: {call_id}")
                    update_compliance_step(lead_id, 'call_1')
                    # Store for double-tap tracking
                    store_pending_call(lead_id, phone, first_name, agent_type, call_id, 1)
                else:
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                if lead:
                    result = make_call(lead['phone'], lead['first_name'] or 'there', agent_type='roofing', is_test=False)
                    self.send_json(result)
                else:
        ...
        elif path == '/api/compliance/mark-contacted':
            try:
                lead_id = d.get('lead_id')
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
        # RETELL CALL COMPLETION WEBHOOK - Handles double-tap logic
        # ═══════════════════════════════════════════════════════════════════════════════
        elif path == '/api/retell/call-completed' or path == '/api/webhook/retell':
            try:
                # Retell webhook payload format:
                # { "event": "call_ended", "call": { call_object } }
                event_type = d.get('event', '')
                call_data = d.get('call', {})
                
                # Get call details from the call object
                call_id = call_data.get('call_id', '') or d.get('call_id', '')
                disconnection_reason = call_data.get('disconnection_reason', '') or d.get('disconnection_reason', '')
                duration_ms = call_data.get('duration_ms', 0) or d.get('duration_ms', 0)
                duration = duration_ms / 1000 if duration_ms else 0
                
                # Get call analysis if available (from call_analyzed event)
                call_analysis = d.get('call_analysis', {}) or call_data.get('call_analysis', {})
                call_successful = call_analysis.get('call_successful', None)
                
                print(f"")
                print(f"📞 RETELL WEBHOOK RECEIVED")
                print(f"   Event: {event_type}")
                print(f"   Call ID: {call_id}")
                print(f"   Disconnection: {disconnection_reason}")
                print(f"   Duration: {duration}s")
                ...
                if event_type in ['call_ended', 'call_analyzed'] and call_id:
                    # Map Retell disconnection reasons to our outcomes
                    # From Retell docs: user_hangup, agent_hangup, voicemail_reached, 
                    # dial_failed, dial_no_answer, dial_busy, inactivity, machine_detected, etc.
                    no_answer_reasons = [
                        'voicemail_reached', 'dial_no_answer', 'dial_busy', 'dial_failed',
                        'no_answer', 'busy', 'failed', 'machine_detected', 'inactivity',
                        'voicemail', 'max_duration_reached'
                    ]
                    
                    answered_reasons = [
                        'user_hangup', 'agent_hangup', 'call_ended', 'call_transfer'
                    ]
                    
                    # Determine outcome
                    reason_lower = disconnection_reason.lower()
                    
                    if reason_lower in answered_reasons or duration > 30 or call_successful == True:
                        mapped_outcome = 'answered'
                    elif reason_lower in no_answer_reasons or call_successful == False:
                        mapped_outcome = 'voicemail' if 'voicemail' in reason_lower or 'machine' in reason_lower else 'no_answer'
                    else:
                        # Default: if duration > 15s, probably answered
                        mapped_outcome = 'answered' if duration > 15 else 'no_answer'
                    
                    print(f"   Mapped Outcome: {mapped_outcome}")
                    
                    # Handle the completion (may trigger double-tap if no answer)
                    handle_call_completion(call_id, mapped_outcome, duration)
                
                # Always return success to Retell
                self.send_json({"success": True, "received": True})
                ...
                self.send_json({"success": True, "error": str(e)})
        
        # ═══════════════════════════════════════════════════════════════════════════════
        # CALENDLY WEBHOOK - Track appointments booked via booking link
        # ═══════════════════════════════════════════════════════════════════════════════
        elif path == '/api/webhook/calendly' or path == '/api/calendly/webhook':
            try:
                event_type = d.get('event', '')
                payload = d.get('payload', {})
                
                print(f"")
                print(f"📅📅📅 CALENDLY WEBHOOK RECEIVED 📅📅📅")
                print(f"   Event: {event_type}")
                ...
                if event_type == 'invitee.created':
                    ...
                    event_name = scheduled_event.get('name') or event.get('name', 'Appointment')
                    ...
                    if start_time:
                        try:
                            ...
                            appt_date = start_time[:10] if len(start_time) > 10 else start_time
                            appt_time = start_time[11:16] if len(start_time) > 16 else ''
                    
                    print(f"   📅 NEW APPOINTMENT BOOKED!")
                    print(f"   Name: {invitee_name}")
                    print(f"   Email: {invitee_email}")
                    print(f"   Phone: {invitee_phone}")
                    print(f"   Date: {appt_date}")
                    print(f"   Time: {appt_time}")
                    
                    # Save to appointments table - use timeout-enabled connection
                    conn = get_db_connection()
                    c = conn.cursor()
                    ...
                    # Clean phone number
                    clean_phone = invitee_phone
                    if invitee_phone:
                        phone_digits = ''.join(filter(str.isdigit, invitee_phone))
                        if len(phone_digits) == 10:
                            clean_phone = '+1' + phone_digits
                        elif len(phone_digits) == 11 and phone_digits.startswith('1'):
                            clean_phone = '+' + phone_digits
                    
                    c.execute('''INSERT INTO appointments 
                                 (first_name, last_name, email, phone, appointment_date, appointment_time, 
                                  status, source, disposition_notes, created_at)
                                 VALUES (?, ?, ?, ?, ?, ?, 'scheduled', 'calendly', ?, CURRENT_TIMESTAMP)''',
                              (first_name, last_name, invitee_email, clean_phone, appt_date, appt_time, event_name))
                    appt_id = c.lastrowid
                    
                    # Try to link to existing lead by phone or email
                    lead_id = None
                    if invitee_phone:
                        phone_digits = ''.join(filter(str.isdigit, invitee_phone))[-10:]
                        c.execute("SELECT id FROM leads WHERE phone LIKE ?", (f"%{phone_digits}%",))
                        row = c.fetchone()
                        if row:
                            lead_id = row[0]
                    
                    if not lead_id and invitee_email:
                        c.execute("SELECT id FROM leads WHERE email = ?", (invitee_email,))
                        row = c.fetchone()
                        if row:
                            lead_id = row[0]
                    ...
                    if lead_id:
                        c.execute("UPDATE leads SET status = 'appointment_set', updated_at = CURRENT_TIMESTAMP WHERE id = ?", 
                                  (lead_id,))
                        # Update the appointment with lead_id
                        c.execute("UPDATE appointments SET lead_id = ? WHERE id = ?", (lead_id, appt_id))
                        # Update compliance tracking
                        update_compliance_step(lead_id, 'appointment_booked')
                        print(f"   ✅ Linked to lead ID: {lead_id}")
                    
                    conn.commit()
                    conn.close()
                    
                    print(f"   ✅ Appointment saved: ID {appt_id}")
                    
                    # Send confirmation SMS if phone available
                    if invitee_phone:
                        send_appointment_confirmation_sms(invitee_phone, first_name, appt_date, appt_time)
...
Check your calendar!"""
        ...
        # Test Lead - Creates lead and optionally auto-calls (same flow as Facebook leads)
        elif path == '/api/compliance/test-lead':
            try:
                phone = d.get('phone', '')
                first_name = d.get('first_name', 'Test')
                last_name = d.get('last_name', 'Lead')
                email = d.get('email', '')
                agent_type = d.get('agent_type', 'roofing')
                auto_call = d.get('auto_call', True)
                ...
                c.execute('''INSERT INTO leads (phone, first_name, last_name, email, source, status, agent_type, created_at)
                          ...
                          (phone, first_name, last_name, email, source, agent_type))
                lead_id = c.lastrowid
                ...
                # Log the lead submission event
                log_call(phone, agent_type, False, 'lead_created', 0, None, False)
                ...
                if auto_call:
                    ...
                    # STEP 2: Brief delay before calling (give them time to see the text)
                    import time
                    time.sleep(2)
                    ...
                    if call_result.get('success'):
                        ...
                        print(f"   ✅ Call initiated: {call_id}")
                        # Log the call
                        log_call(phone, agent_type, False, 'initiated', 0, call_id, True)
                        # Update compliance tracking
                        update_compliance_step(lead_id, 'call_1')
                        # Store for double-tap tracking
                        store_pending_call(lead_id, phone, first_name, agent_type, call_id, 1)
