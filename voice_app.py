...
if(r.session_token){localStorage.setItem('voice_session',r.session_token);currentUser=r.user;document.body.classList.remove('logged-out');closeModal('login-modal');toast('Welcome back, '+r.user.name+'!');localStorage.setItem('session',r.session_token);location.reload()}else toast(r.error||'Login failed',true);
}
