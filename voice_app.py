...
def ghl_get_contacts(limit=100, skip=0, query=None):
...
def get_next_call_window():
    """Get the next available call window"""
    now = datetime.now()
    hour = now.hour
...
def schedule_call_sequence(lead_id, phone, first_name, ghl_contact_id, agent_type='roofing'):
    ...
    try:
        ...
        c.execute('''INSERT INTO call_sequences 
            (lead_id, phone, first_name, ghl_contact_id, agent_type, status, calls_made, max_calls, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (lead_id, phone, first_name, ghl_contact_id, agent_type, 'active', 0, 3, datetime.now().isoformat()))
...
def schedule_next_call(sequence_id):
    ...
    try:
        ...
        if seq['calls_made'] >= seq['max_calls']:
            c.execute('UPDATE call_sequences SET status = ? WHERE id = ?', ('completed', sequence_id))
            conn.commit()
            conn.close()
            return {"success": True, "status": "sequence_completed"}
        
        # Check if appointment was set (check GHL tag)
        if seq['ghl_contact_id'] and GHL_API_KEY:
            ...
            if contact.get('contact', {}).get('tags', []):
                ...
                if 'appointment set' in tags or 'appointment-set' in tags:
        ...
        print(f"📅 Call scheduled for {next_time.strftime('%Y-%m-%d %H:%M')} ({window_name} window)")
        
        return {"success": True, "scheduled_time": next_time.isoformat(), "window": window_name}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def process_scheduled_calls():
    ...
    try:
        ...
        c.execute('''SELECT sc.*, cs.phone, cs.first_name, cs.address, cs.ghl_contact_id, cs.agent_type, cs.calls_made
            ...
            LIMIT 5''', ('pending', now.isoformat(), 'active'))
        ...
        for call in calls:
            ...
            print(f"📞 Processing scheduled call for {call['phone']}...")
            ...
            if result.get('success'):
                ...
                # Update sequence
                c.execute('UPDATE call_sequences SET calls_made = calls_made + 1, last_call_at = ? WHERE id = ?',
                    (now.isoformat(), call['sequence_id']))
                
                # Schedule next call (after a delay to allow this call to complete)
                # We'll schedule next call when we get the Retell webhook with results
...
def handle_call_completed(call_id, outcome, ghl_contact_id=None):
    ...
    try:
        ...
        double_tap_outcomes = ['no_answer', 'no-answer', 'busy', 'failed', 'voicemail']
        
        if outcome in double_tap_outcomes and not was_double_tap:
            print(f"📞📞 DOUBLE TAP - No answer, calling back immediately...")
            
            # Make immediate callback
            call_result = make_call(
                phone=seq['phone'],
                name=seq['first_name'] or 'there',
                agent_type=seq['agent_type'],
                is_test=False,
                ghl_contact_id=seq['ghl_contact_id'],
                address=seq.get('address', '')
            )
            
            if call_result.get('success'):
                # Log as double tap call
                c.execute('''INSERT INTO scheduled_calls 
                    (sequence_id, scheduled_time, window_name, status, call_id, is_double_tap, executed_at, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                    (seq['id'], datetime.now().isoformat(), 'double_tap', 'executed', 
                     call_result.get('call_id', ''), 1, datetime.now().isoformat(), datetime.now().isoformat()))
                
                conn.commit()
                conn.close()
                print(f"✅ Double tap call initiated: {call_result.get('call_id')}")
                return {"success": True, "action": "double_tap", "call_id": call_result.get('call_id')}
            else:
        ...
        if seq['calls_made'] >= seq['max_calls']:
            ...
            # Send final SMS via GHL if configured
            if ghl_contact_id and GHL_API_KEY:
                ghl_create_note(ghl_contact_id, "📱 Call sequence completed - 3 attempts made. Sending follow-up SMS.")
                ghl_add_tag(ghl_contact_id, 'Sequence Completed - No Answer')
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None, address=""):
    ...
    elif agent_type == 'solar' and ghl_contact_id:
        # SOLAR CLIENT - Use dedicated Hailey solar agent for GHL leads
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            # AUDIT TRAIL DASHBOARD - Visual view
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                ...
                rows_html = ""
                for seq in sequences:
                    status_color = {'active': '#10b981', 'completed': '#6b7280', 'appointment_set': '#00d1ff', 'not_interested': '#ef4444', 'max_calls_reached': '#f59e0b'}.get(seq.get('status', ''), '#6b7280')
                    
                    calls_html = ""
                    for call in seq.get('calls', []):
                        tap_badge = "📞📞 DOUBLE" if call.get('is_double_tap') else "📞"
                        call_status_color = {'pending': '#f59e0b', 'executed': '#10b981', 'failed': '#ef4444', 'cancelled': '#6b7280'}.get(call.get('status', ''), '#6b7280')
                        calls_html += f'''<div style="display:flex;gap:8px;align-items:center;padding:6px 0;border-bottom:1px solid #1e1e2e;">
                            <span>{tap_badge}</span>
                            <span style="color:{call_status_color};font-weight:600;">{call.get('status', 'unknown').upper()}</span>
                            <span style="color:#6b7280;">{call.get('window_name', '')} window</span>
                            <span style="color:#6b7280;font-size:12px;">{str(call.get('scheduled_time', ''))[:16]}</span>
                            {f"<span style='color:#00d1ff;font-size:11px;'>ID: {str(call.get('call_id', ''))[:20]}...</span>" if call.get('call_id') else ""}
                        </div>'''
                    
                    if not calls_html:
                        calls_html = "<div style='color:#6b7280;'>No calls yet</div>"
                    
                    rows_html += f'''<div style="background:#12121a;border:1px solid #1e1e2e;border-radius:12px;padding:20px;margin-bottom:16px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                            <div><span style="font-size:18px;font-weight:700;">{seq.get('first_name', 'Unknown')}</span>
                            <span style="color:#6b7280;margin-left:12px;">{seq.get('phone', '')}</span></div>
                            <span style="background:{status_color};color:#000;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;">{seq.get('status', 'unknown').upper().replace('_', ' ')}</span>
                        </div>
                        <div style="display:flex;gap:24px;margin-bottom:12px;color:#6b7280;font-size:13px;">
                            <span>📍 {seq.get('address', 'No address')}</span>
                            <span>🏷️ {seq.get('agent_type', 'solar')}</span>
                            <span>📊 {seq.get('calls_made', 0)}/{seq.get('max_calls', 3)} calls</span>
                            <span>🕐 {str(seq.get('created_at', ''))[:16]}</span>
                        </div>
                        <div style="background:#0a0a0f;border-radius:8px;padding:12px;margin-top:12px;">
                            <div style="font-size:12px;color:#6b7280;margin-bottom:8px;font-weight:600;">CALL HISTORY</div>
                            {calls_html}
                        </div>
                    </div>'''
                
                if not rows_html:
                    rows_html = "<div style='text-align:center;color:#6b7280;padding:40px;'>No sequences yet. Trigger a webhook with action=sequence to start.</div>"
                
                html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>VOICE - Sequence Audit Trail</title>
<style>:root{{--bg:#0a0a0f;--card:#12121a;--border:#1e1e2e;--cyan:#00d1ff;--green:#10b981;--text:#f5f5f5}}
*{{margin:0;padding:0;box-sizing:border-box}}body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:var(--bg);color:var(--text);padding:24px}}
.header{{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px}}.title{{font-size:28px;font-weight:700}}
.refresh{{background:var(--cyan);color:#000;border:none;padding:10px 20px;border-radius:8px;cursor:pointer;font-weight:600}}
.stats{{display:flex;gap:16px;margin-bottom:24px}}.stat{{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px 24px}}
.stat-value{{font-size:24px;font-weight:700;color:var(--cyan)}}.stat-label{{font-size:12px;color:#6b7280}}</style></head>
<body><div class="header"><div class="title">🔍 Sequence Audit Trail</div><button class="refresh" onclick="location.reload()">↻ Refresh</button></div>
<div class="stats"><div class="stat"><div class="stat-value">{len(sequences)}</div><div class="stat-label">Total Sequences</div></div>
<div class="stat"><div class="stat-value">{len([s for s in sequences if s.get('status') == 'active'])}</div><div class="stat-label">Active</div></div>
<div class="stat"><div class="stat-value">{len([s for s in sequences if s.get('status') == 'appointment_set'])}</div><div class="stat-label">Appointments Set</div></div>
<div class="stat"><div class="stat-value">{sum(len(s.get('calls', [])) for s in sequences)}</div><div class="stat-label">Total Calls</div></div></div>
{rows_html}<script>setTimeout(()=>location.reload(), 30000);</script></body></html>'''
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                if action == 'sequence' or action == 'start_sequence':
                    ...
                    # Create sequence
                    c.execute('''INSERT INTO call_sequences 
                        (lead_id, phone, first_name, address, ghl_contact_id, agent_type, status, calls_made, max_calls, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        (lead_id, phone, first_name, full_address, contact_id, agent_type, 'active', 0, 3, datetime.now().isoformat()))
                    sequence_id = c.lastrowid
