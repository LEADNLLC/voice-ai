    ...
    try:
        ...
        if existing.get('contacts') and len(existing['contacts']) > 0:
            ...
            result = ghl_create_contact(
                ...
                tags=['VoiceLab', 'Appointment Booked'],
                source='VoiceLab AI'
            )
        ...
        # Get calendars to find the right one
        calendars = ghl_get_calendars()
        if not calendars.get('calendars') or len(calendars['calendars']) == 0:
            return {"success": False, "error": "No calendars found in GHL"}
        
        calendar_id = calendars['calendars'][0]['id']  # Use first calendar
        ...
        if appt_date:
            ...
            result = ghl_create_appointment(
                ...
                end_time=end_time,
                title=f"Appointment - {appointment.get('first_name', 'Customer')}",
                notes=f"Booked via VoiceLab AI\nAddress: {appointment.get('address', 'N/A')}\nAgent: {appointment.get('agent_type', 'N/A')}"
            )
            
            if result.get('id') or result.get('event'):
                # Add tag to contact
                ghl_add_tag(contact_id, 'Appointment Booked')
...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            # Webhook endpoint for GHL to trigger AI calls
            # GHL workflow can POST to this endpoint to trigger a call
            try:
                print(f"📥 GHL Webhook received: {d}")
                
                action = d.get('action', 'call')
                ...
                if action == 'call':
                    # Trigger AI call
                    result = test_agent_with_phone(agent_type, phone, is_live=True)
                    ...
                    c.execute('''INSERT INTO webhook_logs (source, event_type, payload, created_at)
                                VALUES (?, ?, ?, ?)''',
                             ('ghl', 'call_trigger', json.dumps(d), datetime.now().isoformat()))
                    conn.commit()
                    conn.close()
                    
                    # Update GHL contact with note
                    if contact_id:
                        ghl_create_note(contact_id, f"VoiceLab AI call triggered at {datetime.now().strftime('%Y-%m-%d %H:%M')}")
                    
                    self.send_json({"success": True, "result": result})
                    
                elif action == 'add_lead':
                    # Add lead to VoiceLab from GHL
                    conn = sqlite3.connect(DB_PATH)
                    c = conn.cursor()
                    c.execute('''INSERT INTO leads (first_name, last_name, phone, email, status, source, created_at, ghl_contact_id)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                             (first_name,
                              d.get('last_name', d.get('contact', {}).get('lastName', '')),
                              format_phone(phone),
                              d.get('email', d.get('contact', {}).get('email', '')),
                              'new',
                              'GoHighLevel Webhook',
                              datetime.now().isoformat(),
                              contact_id))
                    ...
                    # Start calling sequence for a contact
                    conn = sqlite3.connect(DB_PATH)
                    c = conn.cursor()
                    ...
                    # Update lead to start sequence
                    c.execute('UPDATE leads SET status = ?, sequence_started = 1 WHERE id = ?',
                             ('active', lead_id))
                    conn.commit()
                    conn.close()
                    
                    self.send_json({"success": True, "lead_id": lead_id, "message": "Sequence started"})
