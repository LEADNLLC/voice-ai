...
def get_timezone_from_phone(phone):
    ...
    if len(phone_clean) >= 10:
        ...
        # Simplified timezone mapping by area code ranges
        eastern = ['201','202','203','207','212','215','216','217','219','223','224','225','227','228','229','231','234','239','240','248','251','252','254','256','260','262','267','269','270','272','276','279','281','301','302','303','304','305','307','308','309','310','312','313','314','315','316','317','318','319','320','321','323','325','330','331','332','334','336','337','339','340','341','346','347','351','352','360','361','364','380','385','386','401','402','404','405','406','407','408','409','410','412','413','414','415','417','419','423','424','425','430','432','434','435','440','442','443','445','447','448','458','463','464','469','470','475','478','479','480','484','501','502','503','504','505','507','508','509','510','512','513','515','516','517','518','520','530','531','534','539','540','541','551','559','561','562','563','567','570','571','572','573','574','575','580','585','586']
...
def is_valid_call_time(phone):
    ...
    try:
        import pytz
        tz = get_timezone_from_phone(phone)
        if tz:
        ...
        pass
    # Default: assume Eastern, be conservative
    from datetime import datetime
    hour = datetime.now().hour
    return 9 <= hour < 20  # 9am-8pm Eastern = safe for all US
...
def send_sms_to_conversation(conversation_id, phone, message):
    ...
    try:
        ...
        log_contact_attempt(phone, 'sms', 'sent')
...
function renderConvoList(convos) {
    ...
    list.innerHTML = convos.map(c => 
        '<div onclick="openConvo(' + c.id + ')" style="padding:12px 16px;border-bottom:1px solid var(--border);cursor:pointer">' +
        '<div style="font-weight:500">' + (c.contact_name || c.phone) + '</div>' +
        '<div style="font-size:12px;color:var(--gray-500);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + (c.last_message || 'No messages') + '</div>' +
        '</div>'
    ).join('');
}
...
async function openConvo(id) {
    ...
    area.innerHTML = 
        ...
        '<div style="padding:16px;border-bottom:1px solid var(--border)"><strong>' + (convo.contact_name || convo.phone) + '</strong></div>' +
        '<div id="sms-messages" style="flex:1;overflow-y:auto;padding:16px">' + 
        (convo.messages || []).map(m => 
            '<div style="margin-bottom:12px;text-align:' + (m.direction === 'outbound' ? 'right' : 'left') + '">' +
            '<div style="display:inline-block;padding:8px 12px;border-radius:12px;max-width:70%;background:' + (m.direction === 'outbound' ? 'var(--cyan)' : 'rgba(255,255,255,0.1)') + '">' + m.message + '</div>' +
            '</div>'
        ).join('') +
        '</div>' +
        '<div style="padding:16px;border-top:1px solid var(--border);display:flex;gap:8px">' +
        '<input id="sms-input" style="flex:1;padding:10px;border-radius:8px;border:1px solid var(--border);background:rgba(255,255,255,0.05);color:#fff" placeholder="Type a message...">' +
        '<button class="btn btn-primary" onclick="sendSmsMsg()">Send</button>' +
        '</div></div>';
}
