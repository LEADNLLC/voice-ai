...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            # AUDIT TRAIL - Get all sequences and their call history
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                
                # Get all sequences with their scheduled calls
                c.execute('''SELECT 
                    cs.id, cs.phone, cs.first_name, cs.address, cs.agent_type,
                    cs.status, cs.calls_made, cs.max_calls, cs.last_call_at,
                    cs.created_at, cs.ghl_contact_id
                    FROM call_sequences cs
                    ORDER BY cs.created_at DESC
                    LIMIT 50''')
                sequences = [dict(row) for row in c.fetchall()]
                
                # Get scheduled calls for each sequence
                for seq in sequences:
                    c.execute('''SELECT 
                        id, scheduled_time, window_name, status, call_id, 
                        is_double_tap, executed_at, error, created_at
                        FROM scheduled_calls 
                        WHERE sequence_id = ?
                        ORDER BY created_at ASC''', (seq['id'],))
                    seq['calls'] = [dict(row) for row in c.fetchall()]
                
                # Get recent call log entries
                c.execute('''SELECT 
                    call_id, phone, agent_type, duration, status, 
                    recording_url, is_live, created_at
                    FROM call_log 
            ...
            # AUDIT TRAIL DASHBOARD - Visual view of all sequences
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                ...
                for seq in sequences:
                    status_color = {
                        'active': '#10b981',
                        'completed': '#6b7280',
                        'appointment_set': '#00d1ff',
                        'not_interested': '#ef4444',
                        'max_calls_reached': '#f59e0b'
                    }.get(seq['status'], '#6b7280')
                    ...
                    for call in seq.get('calls', []):
                        ...
                        call_status_color = {
                            ...
                            'failed': '#ef4444',
                            'cancelled': '#6b7280'
                        }.get(call.get('status'), '#6b7280')
                        
                        calls_html += f'''
                        <div style="display:flex;gap:8px;align-items:center;padding:6px 0;border-bottom:1px solid #1e1e2e;">
                            <span>{tap_badge}</span>
                            ...
                            <span style="color:#6b7280;font-size:12px;">{call.get('scheduled_time', '')[:16] if call.get('scheduled_time') else ''}</span>
                            {f"<span style='color:#00d1ff;font-size:11px;'>ID: {call.get('call_id', '')[:20]}...</span>" if call.get('call_id') else ""}
                        </div>
                        '''
                    ...
                    rows_html += f'''
                    <div style="background:#12121a;border:1px solid #1e1e2e;border-radius:12px;padding:20px;margin-bottom:16px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                            <div>
                                <span style="font-size:18px;font-weight:700;">{seq.get('first_name', 'Unknown')}</span>
                                <span style="color:#6b7280;margin-left:12px;">{seq.get('phone', '')}</span>
                            </div>
                            <span style="background:{status_color};color:#000;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;">
                                {seq.get('status', 'unknown').upper().replace('_', ' ')}
                            </span>
                        </div>
                        <div style="display:flex;gap:24px;margin-bottom:12px;color:#6b7280;font-size:13px;">
                            ...
                            <span>🕐 Created: {seq.get('created_at', '')[:16] if seq.get('created_at') else 'N/A'}</span>
                        </div>
...
.stat{{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px 24px;}}
.stat-value{{font-size:24px;font-weight:700;color:var(--cyan);}}
.stat-label{{font-size:12px;color:#6b7280;}}
</style>
</head><body>
<div class="header">
    <div class="title">🔍 Sequence Audit Trail</div>
    <button class="refresh" onclick="location.reload()">↻ Refresh</button>
</div>
<div class="stats">
    <div class="stat">
        <div class="stat-value">{len(sequences)}</div>
        <div class="stat-label">Total Sequences</div>
    </div>
    <div class="stat">
        <div class="stat-value">{len([s for s in sequences if s.get('status') == 'active'])}</div>
        <div class="stat-label">Active</div>
    </div>
    <div class="stat">
        <div class="stat-value">{len([s for s in sequences if s.get('status') == 'appointment_set'])}</div>
        <div class="stat-label">Appointments Set</div>
    </div>
    <div class="stat">
        <div class="stat-value">{sum(len(s.get('calls', [])) for s in sequences)}</div>
