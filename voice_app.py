...
def create_sms_conversation(phone, contact_name=None, initial_message=None):
    ...
    try:
        ...
        if initial_message:
            c.execute("INSERT INTO sms_messages (conversation_id, direction, message, status) VALUES (?, 'outbound', ?, 'sent')",
                      (convo_id, initial_message))
            c.execute("UPDATE sms_conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?", (convo_id,))
