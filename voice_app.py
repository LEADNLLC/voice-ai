...
async function loadMessages() {
    try {
        const convos = await fetch('/api/sms/conversations').then(r => r.json()).catch(() => []);
        smsConvos = convos;
...
function renderConvoList(convos) {
    ...
    if (!list) return;
    if (!convos.length) {
        list.innerHTML = '<div style="padding:40px;text-align:center;color:var(--gray-500)">No conversations yet</div>';
        return;
    }
...
async function openConvo(id) {
    const convo = await fetch('/api/sms/conversations/' + id).then(r => r.json()).catch(() => null);
    if (!convo) return;
    currentConvo = convo;
    ...
    function getStatusIndicator(msg) {
        ...
        if (!indicator) return '';
        return '<div style="font-size:11px;color:' + color + ';margin-top:4px;text-align:right">' + indicator + '</div>';
    }
    
    area.innerHTML = 
        '<div style="flex:1;display:flex;flex-direction:column">' +
        '<div style="padding:16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center"><strong>' + (convo.contact_name || convo.phone) + '</strong><span style="font-size:12px;color:var(--gray-500)">' + convo.phone + '</span></div>' +
        '<div id="sms-messages" style="flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:4px">' + 
        (convo.messages || []).map((m, i, arr) => {
            const isOutbound = m.direction === 'outbound';
            const isLast = i === arr.length - 1 || arr[i + 1].direction !== m.direction;
            const showStatus = isOutbound && isLast;
            return '<div style="display:flex;flex-direction:column;align-items:' + (isOutbound ? 'flex-end' : 'flex-start') + '">' +
            '<div style="display:inline-block;padding:10px 14px;border-radius:18px;max-width:70%;background:' + (isOutbound ? 'linear-gradient(135deg, #14b8a6, #0d9488)' : 'rgba(255,255,255,0.1)') + ';color:#fff;font-size:14px;line-height:1.4">' + m.message + '</div>' +
            (showStatus ? getStatusIndicator(m) : '') +
            '</div>';
        }).join('') +
        '</div>' +
        '<div style="padding:16px;border-top:1px solid var(--border);display:flex;gap:8px">' +
        '<input id="sms-input" style="flex:1;padding:12px 16px;border-radius:20px;border:1px solid var(--border);background:rgba(255,255,255,0.05);color:#fff;font-size:14px" placeholder="iMessage" onkeypress="if(event.key===\\'Enter\\')sendSmsMsg()">' +
        '<button class="btn btn-primary" style="border-radius:50%;width:44px;height:44px;padding:0;display:flex;align-items:center;justify-content:center" onclick="sendSmsMsg()">↑</button>' +
        '</div></div>';
