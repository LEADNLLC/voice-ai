...
def process_scheduled_calls():
    ...
    try:
        ...
        c.execute('''SELECT sc.*, cs.phone, cs.first_name, cs.address, cs.ghl_contact_id, cs.agent_type, 
                    cs.calls_made, cs.current_day, cs.calls_today, cs.max_days, cs.last_call_date
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            # BULK IMPORT - Fetch leads from GHL pipeline and start sequences
            try:
                if not GHL_API_KEY:
                    self.send_json({"success": False, "error": "GHL_API_KEY not configured"})
                    return
                
                stage_name = q.get('stage', ['Sameday'])[0]  # Default to Sameday stage
                ...
                # Get opportunities in that stage
                ...
                for opp in opportunities:
                    ...
                    contact_name = opp.get('contactName', opp.get('name', 'Unknown'))
                    ...
                    phone = contact.get('phone', '')
                    first_name = contact.get('firstName', contact_name.split()[0] if contact_name else 'there')
                    address = contact.get('address1', '')
                    city = contact.get('city', '')
                    
                    if not phone:
                        errors.append({"name": contact_name, "reason": "No phone number"})
                        continue
                    ...
                    if lead_row:
                        ...
                        c.execute('''INSERT INTO leads (first_name, phone, status, source, ghl_contact_id, created_at)
                                    VALUES (?, ?, ?, ?, ?, ?)''',
                                 (first_name, phone, 'active', f'GHL Import - {stage_name}', contact_id, datetime.now().isoformat()))
                        lead_id = c.lastrowid
                    
                    # Create sequence - 7 days, 3 calls per day
                    c.execute('''INSERT INTO call_sequences 
                    ...
                    if contact_id:
                        ...
                        ghl_create_note(contact_id, f"🚀 AI Call Sequence Started (Bulk Import)\n• 7 days, 3 calls/day with double-tap\n• First call: {window_name} window")
                ...
                for seq in sequences:
                    ...
                    rows_html += f'''<div style="background:#12121a;border:1px solid #1e1e2e;border-radius:12px;padding:20px;margin-bottom:16px;">
                        ...
                        <div style="display:flex;gap:24px;margin-bottom:12px;color:#6b7280;font-size:13px;">
                            ...
                            <span>📅 Day {seq.get('current_day', 1)}/{seq.get('max_days', 7)}</span>
                            <span>📞 Today: {seq.get('calls_today', 0)}/3</span>
                            <span>📊 Total: {seq.get('calls_made', 0)}/{seq.get('max_calls', 21)}</span>
