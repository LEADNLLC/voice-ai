...
📞 INSTANT RESPONSE - Every call answered, 24/7
📅 APPOINTMENT BOOKING - From conversation to calendar
📱 AUTO SMS - Confirmation & reminder texts
💰 COST TRACKING - Real-time budget monitoring
🔄 FOLLOW-UP - Relentless, professional persistence
👁️ FULL VISIBILITY - Complete audit trail
"""
...
CLOSE: "I've got two spots... tomorrow at 10 or Thursday at 2... which works?" ALWAYS BOOK."""},
    ...
    "life_insurance": {"name": "Life Insurance", "industry": "Life Insurance", "icon": "💼", "color": "#1e40af", "bilingual": False,
        ...
        "prompt": """You are Sarah, a caring life insurance specialist. Sound warm, natural, and conversational. You help families protect their future.
...
- "Already have coverage" → "Perfect, that's smart... but when's the last time you looked at it? Rates have actually dropped a lot. Let me just do a quick comparison... worst case you find out you're already getting a great deal. Tomorrow at 10 or Thursday at 2?"

CLOSE (Always push for appointment): "Perfect... so I've got two spots open for a quick 15-minute call with our advisor... tomorrow at 10 or Thursday at 2... which one works better for you?"

NEVER say "I understand" - say "I hear you" or "Got it" instead.
...
- Get: Name, phone, type of insurance, current provider"""
    ...
    "inbound_life_insurance": {
        "name": "Life Insurance", "industry": "Life Insurance Agency", "icon": "💼", "color": "#1e40af", "bilingual": False,
        "company": "Guardian Life Solutions",
        "first_message": "Thank you for calling Guardian Life Solutions, this is Amanda, how can I help protect your family today?",
        "prompt": """You are Amanda, the warm and caring receptionist at Guardian Life Solutions, a life insurance agency.

""" + RECEPTIONIST_CORE.replace("{company_name}", "Guardian Life Solutions").replace("{agent_name}", "Amanda") + """

SPECIFIC TO LIFE INSURANCE:
- Products: Term life, whole life, universal life, final expense, mortgage protection
- Quotes: "I'd be happy to have one of our advisors prepare a free quote for you! It only takes about 15 minutes."
- Questions about coverage: "Great question! Our advisors can walk you through all the options. When would be a good time for a quick call?"
- Existing clients: "Let me pull up your policy... can I get your name and date of birth?"
- Claims: "I'm so sorry for your loss. Let me connect you with our claims team right away. They'll guide you through everything."
- Hours: Monday-Friday 8am-7pm, Saturday 9am-3pm
- ALWAYS try to schedule a callback or appointment with an advisor
- Get: Name, phone, what type of coverage they're interested in, family situation, best time to call"""
    },
...
def ghl_get_opportunities(pipeline_id=None, stage_id=None, contact_id=None):
    ...
    return ghl_request('GET', '/opportunities/', params=params)
...
def ghl_create_appointment(contact_id, calendar_id, start_time, end_time, title=None, notes=None):
    ...
    data = {
        ...
        'startTime': start_time,  # ISO format: 2024-01-15T10:00:00-05:00
        'endTime': end_time,
    ...
    if notes:
        data['notes'] = notes
    return ghl_request('POST', '/calendars/events/appointments', data)
...
def process_scheduled_calls():
    ...
    try:
        ...
        c.execute('''SELECT sc.*, cs.phone, cs.first_name, cs.ghl_contact_id, cs.agent_type, cs.calls_made
...
def handle_call_completed(call_id, outcome, ghl_contact_id=None):
    """Handle when a call completes - schedule next call or end sequence"""
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        c = conn.cursor()
        
        # Find the sequence for this call
        c.execute('''SELECT cs.* FROM call_sequences cs
        ...
        # Check outcome - should we continue?
...
def ghl_update_after_call(contact_id, agent_type, duration_sec, call_status, call_cost, recording_url='', outcome='completed', notes=''):
    ...
    try:
        ...
        if existing.get('contact'):
            ...
            # Find total calls and total cost fields by name
...
INDUSTRY_COMPANIES = {
    ...
    'inbound_life_insurance': 'Guardian Life Solutions',
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None):
    phone = format_phone(phone)
    ...
    if is_inbound:
        ...
        # SOLAR CLIENT - Use dedicated solar agent for GHL leads
        retell_agent_id = 'agent_a722737690485bd3cc013c9d3a'  # Solar Client Agent (Hailey)
        from_number = '+18083005141'  # Solar client phone number
        call_type = "outbound"
        call_purpose = "appointment_setting"
        greeting_style = "sales"
        company_name = "your solar company"  # Generic - agent prompt handles details
        opening = ""  # Solar agent has its own opening in the prompt
...
def test_agent_with_phone(agent_type, phone=None, is_live=False, ghl_contact_id=None, customer_name="there"):
    ...
    result = make_call(phone, customer_name, agent_type, is_test=not is_live, ghl_contact_id=ghl_contact_id)
...
<section class="hero">
<div class="hero-badge"><span></span> 24/7 Voice Platform</div>
...
function switchDemoTab(type) {
    ...
    if (type === 'outbound') {
        ...
        select.innerHTML = `
            ...
            <option value="inbound_life_insurance">💼 Life Insurance</option>
...
function switchHeroTab(type) {
    ...
    if (type === 'outbound') {
        ...
        select.innerHTML = `
            ...
            <option value="inbound_insurance">🛡️ Insurance Agency</option>
            <option value="inbound_life_insurance">💼 Life Insurance</option>
...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                last_name = d.get('last_name', d.get('contact', {}).get('lastName', ''))
                ...
                if action == 'sequence' or action == 'start_sequence':
                    ...
                    c.execute('''INSERT INTO call_sequences 
                        (lead_id, phone, first_name, ghl_contact_id, agent_type, status, calls_made, max_calls, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        (lead_id, phone, first_name, contact_id, agent_type, 'active', 0, 3, datetime.now().isoformat()))
                    sequence_id = c.lastrowid
                    ...
                    if contact_id and GHL_API_KEY:
                        ghl_create_note(contact_id, f"🤖 AI call triggered\nAgent: {agent_type}\nTime: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
                        ghl_add_tag(contact_id, 'AI Call Initiated')
            ...
            try:
                ...
                if ghl_contact_id and GHL_API_KEY:
                    ...
                    old_ai_tags = ['AI - No Answer', 'AI - Voicemail', 'AI - Busy', 'AI - Connected',
                                   ...
                                   'AI - Not Interested', 'AI - Already Solar']
                        ...
                        try:
                            ...
                            if target_stage_id:
                                ...
                                if opps.get('opportunities') and len(opps['opportunities']) > 0:
                                    ...
                                    ghl_update_opportunity(opp_id, {'pipelineStageId': target_stage_id, 'status': 'open'})
                                    print(f"✅ Updated opportunity {opp_id} → {pipeline_stage}")
                                else:
