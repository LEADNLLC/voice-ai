...
# +17254258024 has SMS enabled in Retell - use for EVERYTHING!
# ═══════════════════════════════════════════════════════════════════════════════
HAILEY_PHONE_NUMBER = os.environ.get('HAILEY_PHONE_NUMBER', '+17254258024')  # Las Vegas - Retell SMS enabled!
RETELL_PHONE_NUMBER = os.environ.get('RETELL_PHONE_NUMBER', '+17254258024')  # Same number!
SMS_PHONE_NUMBER = os.environ.get('SMS_PHONE_NUMBER', '+17254258024')  # Same number for SMS via Retell!
...
RETELL_PHONE_POOL = {
    # Nevada (Las Vegas, Henderson, Reno) - Using SMS-enabled number!
    '702': '+17254258024',  # Las Vegas NV - SMS enabled!
    '725': '+17254258024',  # Las Vegas NV - SMS enabled!
    '775': '+17254258024',  # Reno NV (fallback to Vegas)
...
RETELL_STATE_FALLBACK = {
    'NV': '+17254258024',  # Nevada → Las Vegas (SMS enabled!)
...
RETELL_DEFAULT_NUMBER = os.environ.get('RETELL_PHONE_NUMBER', '+17254258024')
...
# SMS now via Retell! Using HAILEY_PHONE_NUMBER (+17254258024) - Same number for calls + texts!
# ====================================================
...
def send_security_alert(reason, details=""):
    ...
    try:
        ...
        if TWILIO_SID and TWILIO_TOKEN and OWNER_PHONE:
            ...
            if not from_number:
                from_number = '+17253348817'  # SMS number (Las Vegas - GHL)
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None, address="", text_first=None):
    ...
    if is_inbound:
        ...
        # 🔥 HAILEY LAS VEGAS - SAME NUMBER FOR CALLS + TEXTS!
        from_number = HAILEY_PHONE_NUMBER  # +17254258024 - Las Vegas Retell (SMS enabled!)
        ...
        from_number = HAILEY_PHONE_NUMBER  # +17252523878 - Las Vegas Retell
...
def send_twilio_sms(to_number, message):
    ...
    from_number = os.environ.get('OUTBOUND_NUMBER', '+17253348817')
                ...
                try:
                    ...
                    if is_inbound:
                        ...
                        if USE_VERIFIED_CALLER_ID:
                            from_number = VERIFIED_CALLER_ID  # +17253348817 - Same as GHL SMS!
                        else:
...
# Customer sees +17253348817 for CALLS and TEXTS!
# ═══════════════════════════════════════════════════════════════════════════════
...
def make_call_with_verified_caller_id(phone, name="there", agent_type="solar", ghl_contact_id=None, address=""):
    ...
    4. Customer sees +17253348817 for everything!
    """
    import base64
...
def text_then_call(phone, name="there", agent_type="solar", ghl_contact_id=None, address="", delay_seconds=30):
    ...
    Customer sees SAME number (+17254258024) for both!
    """
    import threading
    import time
