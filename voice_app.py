...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                ghl_contact_id = custom_vars.get('ghl_contact_id', '')
                agent_type = custom_vars.get('agent_type', 'unknown')
                ...
                if ghl_contact_id and GHL_API_KEY:
                    ...
                    # Determine outcome based on transcript keywords
                    outcome = 'completed'
                    ...
                    if transcript:
                        ...
                        # Check for appointment booked
                        if 'appointment' in transcript_lower and ('book' in transcript_lower or 'schedule' in transcript_lower or 'set' in transcript_lower or 'confirm' in transcript_lower):
                            ...
                            # Look for "tomorrow", "monday", etc.
                            if 'tomorrow' in transcript_lower:
                                appointment_date = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
                            
                            # Look for time patterns like "10 am", "2:30 pm"
                            time_match = re.search(r'(\d{1,2})(?::(\d{2}))?\s*(am|pm)', transcript_lower)
                            if time_match:
                                hour = int(time_match.group(1))
                        ...
                        elif 'not interested' in transcript_lower or 'no thank' in transcript_lower or 'not right now' in transcript_lower:
                            outcome = 'not_interested'
                        elif 'call back' in transcript_lower or 'callback' in transcript_lower or 'call me back' in transcript_lower:
                            outcome = 'callback'
                        elif 'voicemail' in call_status.lower() or duration_sec < 30:
                            outcome = 'no_answer'
                    ...
                    # If appointment was set, create it in GHL calendar!
                    if outcome == 'appointment_set':
                        print(f"🎉 Appointment detected! Creating in GHL calendar...")
                        
                        # Use extracted date/time or defaults
                        if not appointment_date:
                            # Default to tomorrow if we couldn't extract
                            appointment_date = (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d')
                        ...
                        if appt_result.get('success'):
                            print(f"✅ Appointment created in GHL calendar!")
                            # The ghl_sync_appointment_to_ghl function adds "Appointment Set" tag
                            # which triggers the notification workflow!
                        else:
                            print(f"⚠️ Failed to create appointment: {appt_result.get('error')}")
                            # Still add the tag so you get notified
                            ghl_add_tag(ghl_contact_id, 'Appointment Set')
                    
                    print(f"✅ GHL contact {ghl_contact_id} updated with call results (outcome: {outcome})")
                
                self.send_json({"success": True, "processed": True})
