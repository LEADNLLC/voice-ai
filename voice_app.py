        ...
        if path == '/':
            ...
            try:
                ...
                for contact in leads_to_restore:
                    ...
                    # Create sequence with restored progress
                    c.execute('''INSERT INTO call_sequences 
                        (phone, ghl_contact_id, first_name, address, agent_type, status,
                         current_day, calls_today, calls_made, max_calls, max_days,
                         last_outcome, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                        (phone, contact_id, first_name, address, 'solar', status,
                         current_day, calls_today, calls_made, 21, 7,
                         last_outcome, datetime.now().isoformat()))
            ...
            # SIMPLE LEADS TABLE - Clean, scannable view
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                ...
                for seq in sequences:
                    ...
                    if status == 'appointment_set':
                        badge = '<span style="background:#00d1ff;color:#000;padding:4px 12px;border-radius:12px;font-weight:700;">✅ BOOKED</span>'
                    elif status == 'active':
                        badge = '<span style="background:#10b981;color:#000;padding:4px 12px;border-radius:12px;font-weight:700;">🔥 ACTIVE</span>'
                    elif status in ['max_calls_reached', 'max_days_reached']:
                        badge = '<span style="background:#f59e0b;color:#000;padding:4px 12px;border-radius:12px;font-weight:700;">⏸️ EXHAUSTED</span>'
                    elif status == 'not_interested':
                        badge = '<span style="background:#ef4444;color:#fff;padding:4px 12px;border-radius:12px;font-weight:700;">❌ NOT INT</span>'
                    elif status == 'stopped':
                        badge = '<span style="background:#6b7280;color:#fff;padding:4px 12px;border-radius:12px;font-weight:700;">⏹️ STOPPED</span>'
                    else:
                        badge = f'<span style="background:#6b7280;color:#fff;padding:4px 12px;border-radius:12px;">{status}</span>'
                    
                    rows_html += f'''<tr>
                        <td style="padding:14px;font-weight:600;">{seq.get('first_name', 'Unknown')}</td>
                        <td style="padding:14px;color:#00d1ff;">{seq.get('phone', '')}</td>
                        <td style="padding:14px;text-align:center;font-weight:700;">{current_day}</td>
                        <td style="padding:14px;text-align:center;font-weight:700;">{calls_made}</td>
                        <td style="padding:14px;">{badge}</td>
                        <td style="padding:14px;color:#6b7280;font-size:13px;">{seq.get('last_outcome', '-')}</td>
                    </tr>'''
...
th{{text-align:left;padding:16px;background:#1e1e2e;font-weight:700;font-size:12px;text-transform:uppercase;color:#6b7280}}
tr:hover{{background:#1a1a2e}}
.btn{{background:#00d1ff;color:#000;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700;margin-left:8px}}
.btn-red{{background:#ef4444}}
</style></head>
<body>
<div class="header">
    <div class="title">📋 Leads Overview</div>
    <div>
        <a href="/sequences" class="btn">📊 Full Dashboard</a>
...
<table>
    ...
    <tbody>
        {rows_html if rows_html else '<tr><td colspan="6" style="text-align:center;padding:40px;color:#6b7280;">No leads yet. Hit /restore to import from GHL.</td></tr>'}
    </tbody>
</table>
