...
# Database path - use /app for Railway, current dir for local
DB_PATH = os.environ.get('DB_PATH', './voice.db')
if os.environ.get('RAILWAY_ENVIRONMENT'):
    DB_PATH = '/app/voice.db' 
...
def request_password_reset(email):
    ...
    c.execute('SELECT id, name FROM users WHERE email = ? AND is_active = 1', (email.lower(),))
    user = c.fetchone()
    ...
    c.execute("DELETE FROM sessions WHERE user_id = ? AND session_token LIKE 'reset_%'", (user['id'],))
    c.execute("INSERT INTO sessions (user_id, session_token, expires_at) VALUES (?, ?, ?)",
              (user['id'], f'reset_{reset_token}', expires_at.isoformat()))
    conn.commit()
    conn.close()
...
def reset_password(token, new_password):
    ...
    c.execute("""SELECT s.user_id, s.expires_at FROM sessions s 
    ...
    # Delete reset token
    c.execute("DELETE FROM sessions WHERE session_token = ?", (f'reset_{token}',))
...
<div class="modal-bg" id="facebook-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">📘</div>
<h2 style="margin:0 0 8px 0">Connect Facebook Lead Ads</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Auto-import leads from your Facebook & Instagram ad campaigns</p>
</div>

<div class="form-group">
<label>Facebook App ID</label>
<input id="fb-app-id" placeholder="Enter your Facebook App ID">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From developers.facebook.com → Your App → Settings → Basic</p>
</div>
...
<input id="fb-account-id" placeholder="act_123456789">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From Business Settings → Ad Accounts</p>
</div>

<div style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:16px;margin:20px 0">
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://developers.facebook.com" target="_blank" style="color:var(--cyan)">developers.facebook.com</a></li>
<li>Create App → Business → Lead Ads</li>
<li>Add Facebook Login & Leads Retrieval products</li>
<li>Set OAuth redirect: <code style="background:rgba(255,255,255,0.1);padding:2px 6px;border-radius:4px;font-size:11px">https://your-domain.com/oauth/facebook/callback</code></li>
<li>Copy App ID & Secret above</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('facebook-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveFacebookSetup()">Connect Facebook</button>
</div>
</div>
</div>

<!-- Google Setup Modal -->
<div class="modal-bg" id="google-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">📅</div>
<h2 style="margin:0 0 8px 0">Connect Google Calendar</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Sync appointments automatically with your Google Calendar</p>
</div>

<div class="form-group">
<label>Google Client ID</label>
<input id="google-client-id" placeholder="xxxxx.apps.googleusercontent.com">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From Google Cloud Console → APIs & Services → Credentials</p>
</div>
...
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://console.cloud.google.com" target="_blank" style="color:var(--cyan)">console.cloud.google.com</a></li>
<li>Create Project → Enable Google Calendar API</li>
<li>Create OAuth 2.0 Credentials (Web Application)</li>
<li>Add redirect URI: <code style="background:rgba(255,255,255,0.1);padding:2px 6px;border-radius:4px;font-size:11px">https://your-domain.com/oauth/google/callback</code></li>
<li>Copy Client ID & Secret above</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('google-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveGoogleSetup()">Connect Google</button>
</div>
</div>
</div>

<!-- Stripe Setup Modal -->
<div class="modal-bg" id="stripe-setup-modal">
<div class="modal" style="max-width:480px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">💳</div>
<h2 style="margin:0 0 8px 0">Connect Stripe</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Track revenue and process payments</p>
</div>

<div class="form-group">
<label>Stripe Secret Key</label>
<input id="stripe-secret" type="password" placeholder="sk_live_...">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From Stripe Dashboard → Developers → API Keys</p>
</div>
...
<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('stripe-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveStripeSetup()">Connect Stripe</button>
</div>
</div>
</div>

<!-- Outlook Setup Modal -->
<div class="modal-bg" id="outlook-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">📧</div>
<h2 style="margin:0 0 8px 0">Connect Microsoft Outlook</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Sync with Outlook Calendar & Email</p>
</div>

<div class="form-group">
<label>Azure Application (Client) ID</label>
<input id="outlook-client-id" placeholder="Enter your Application ID">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From Azure Portal → App Registrations</p>
</div>

<div class="form-group">
<label>Client Secret</label>
<input id="outlook-client-secret" type="password" placeholder="Enter your Client Secret">
</div>

<div style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:16px;margin:20px 0">
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://portal.azure.com" target="_blank" style="color:var(--cyan)">portal.azure.com</a></li>
<li>Azure Active Directory → App Registrations → New</li>
<li>Add API permissions: Calendars.ReadWrite, Mail.Read</li>
<li>Create Client Secret under Certificates & Secrets</li>
<li>Add redirect URI to Authentication</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('outlook-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveOutlookSetup()">Connect Outlook</button>
</div>
</div>
</div>
...
<!-- Retell Setup Modal -->
<div class="modal-bg" id="retell-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">🔊</div>
<h2 style="margin:0 0 8px 0">Connect Retell AI</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Configure your Retell voice agents for inbound & outbound calls</p>
</div>

<div class="form-group">
<label>Retell API Key</label>
<input id="retell-api-key" type="password" placeholder="key_xxxxxxxxxxxxxxxx">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From retellai.com → Settings → API Keys</p>
</div>

<div class="form-group">
<label>Outbound Agent ID</label>
<input id="retell-outbound-agent" placeholder="agent_xxxxxxxxxxxxxxxx">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">Agent used for outbound sales calls</p>
</div>

<div class="form-group">
<label>Inbound Agent ID</label>
<input id="retell-inbound-agent" placeholder="agent_xxxxxxxxxxxxxxxx">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">Agent used for inbound reception calls</p>
</div>

<div class="form-group">
<label>Outbound Phone Number</label>
<input id="retell-outbound-phone" placeholder="+17201234567">
</div>

<div class="form-group">
<label>Inbound Phone Number</label>
<input id="retell-inbound-phone" placeholder="+17207654321">
</div>

<div style="background:rgba(168,85,247,0.1);border:1px solid rgba(168,85,247,0.2);border-radius:8px;padding:16px;margin:20px 0">
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://retellai.com" target="_blank" style="color:var(--cyan)">retellai.com</a> and sign in</li>
<li>Create agents for Outbound (sales) and Inbound (reception)</li>
<li>Get phone numbers from Retell or bring your own via Twilio</li>
<li>Copy your API Key from Settings</li>
<li>Copy Agent IDs from each agent's settings</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('retell-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveRetellSetup()">Connect Retell</button>
</div>
</div>
</div>

<!-- VAPI Setup Modal -->
<div class="modal-bg" id="vapi-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">🤖</div>
<h2 style="margin:0 0 8px 0">Connect VAPI</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Configure VAPI voice agents</p>
</div>

<div class="form-group">
<label>VAPI API Key</label>
<input id="vapi-api-key" type="password" placeholder="vapi_xxxxxxxxxxxxxxxx">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From vapi.ai → Settings → API Keys</p>
</div>

<div class="form-group">
<label>Phone ID</label>
<input id="vapi-phone-id" placeholder="Phone ID from VAPI">
</div>

<div class="form-group">
<label>Assistant ID (Optional)</label>
<input id="vapi-assistant-id" placeholder="asst_xxxxxxxx">
</div>

<div style="background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:16px;margin:20px 0">
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://vapi.ai" target="_blank" style="color:var(--cyan)">vapi.ai</a> and sign in</li>
<li>Create an assistant with your voice configuration</li>
<li>Get a phone number from Phone Numbers section</li>
<li>Copy API Key from Settings → API Keys</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('vapi-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveVapiSetup()">Connect VAPI</button>
</div>
</div>
</div>

<!-- Twilio Setup Modal -->
<div class="modal-bg" id="twilio-setup-modal">
<div class="modal" style="max-width:520px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">📱</div>
<h2 style="margin:0 0 8px 0">Connect Twilio</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">SMS messaging and phone number management</p>
</div>

<div class="form-group">
<label>Account SID</label>
<input id="twilio-sid" placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">From twilio.com → Console Dashboard</p>
</div>

<div class="form-group">
<label>Auth Token</label>
<input id="twilio-token" type="password" placeholder="Your Auth Token">
</div>

<div class="form-group">
<label>Phone Number</label>
<input id="twilio-phone" placeholder="+17201234567">
<p style="font-size:11px;color:var(--gray-500);margin-top:4px">Your Twilio phone number for SMS</p>
</div>

<div style="background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.2);border-radius:8px;padding:16px;margin:20px 0">
<div style="font-weight:600;font-size:13px;margin-bottom:8px">📋 Setup Instructions</div>
<ol style="font-size:12px;color:var(--gray-300);margin:0;padding-left:18px;line-height:1.8">
<li>Go to <a href="https://twilio.com/console" target="_blank" style="color:var(--cyan)">twilio.com/console</a></li>
<li>Copy Account SID and Auth Token from dashboard</li>
<li>Buy a phone number or use existing one</li>
<li>Configure webhooks for SMS/Voice if needed</li>
</ol>
</div>

<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('twilio-setup-modal')">Cancel</button>
<button class="btn btn-primary" onclick="saveTwilioSetup()">Connect Twilio</button>
</div>
</div>
</div>
...
body.logged-out #aria-btn { display: none !important; }
body.logged-out .modal-bg#login-modal { display: flex !important; }
</style>
</head>
<body>
...
body.logged-out .sidebar,body.logged-out .main,body.logged-out #aria-btn{display:none !important}body.logged-out .modal-bg#login-modal{display:flex !important}body.logged-out{background:#0a0a0a}
...
async function loadPipeline(){try{await refreshPipeline()}catch(e){console.error('loadPipeline error:',e)}}

async function refreshPipeline() {
    try {
        const agentFilter = $('pp-agent-filter')?.value || 'all';
        const industryFilter = $('pp-industry-filter')?.value || 'all';
        
        const [leads, stats, stages] = await Promise.all([
        ...
        const agentSelect = $('pp-agent-filter');
        ...
        const indSelect = $('pp-industry-filter');
        ...
        // Filter leads
        let filtered = leads;
        if(agentFilter !== 'all') filtered = filtered.filter(l => l.assigned_agent === agentFilter);
        if(industryFilter !== 'all') filtered = filtered.filter(l => l.industry === industryFilter);
        
        // Update stats
        if($('pp-total')) $('pp-total').textContent = filtered.length;
        if($('pp-active')) $('pp-active').textContent = filtered.filter(l => !['sold','lost_dead','not_interested'].includes(l.pipeline_stage)).length;
        if($('pp-qualified')) $('pp-qualified').textContent = filtered.filter(l => l.pipeline_stage === 'qualified').length;
        const sold = filtered.filter(l => l.pipeline_stage === 'sold').length;
        const convRate = filtered.length > 0 ? Math.round(sold / filtered.length * 100) : 0;
        if($('pp-conversion')) $('pp-conversion').textContent = convRate + '%';
        if($('pp-revenue')) $('pp-revenue').textContent = '$' + (stats.pipeline_value || 0).toLocaleString();
        
        // Build Kanban board
        const stageConfig = [
            {id:'new_lead',name:'New Lead',icon:'🆕'},
            {id:'contacted',name:'Contacted',icon:'📞'},
            {id:'no_answer',name:'No Answer',icon:'📵'},
            {id:'callback_scheduled',name:'Callback',icon:'📅'},
            {id:'qualified',name:'Qualified',icon:'✅'},
            {id:'appointment_set',name:'Appt Set',icon:'🗓️'},
            {id:'appt_completed',name:'Completed',icon:'✔️'},
            {id:'proposal_sent',name:'Proposal',icon:'📄'},
            {id:'negotiation',name:'Negotiation',icon:'🤝'},
            {id:'sold',name:'Sold',icon:'💰'},
            {id:'lost_dead',name:'Lost',icon:'❌'}
        ];
        
        const board = $('pp-board');
        if(board) {
            board.innerHTML = stageConfig.map(stage => {
                const stageLeads = filtered.filter(l => l.pipeline_stage === stage.id);
                return `<div class="pp-column">
                    <div class="pp-column-header">
                        <div class="pp-column-title"><span class="icon">${stage.icon}</span>${stage.name}</div>
                        <div class="pp-column-count">${stageLeads.length}</div>
                    </div>
                    <div class="pp-column-body">
                        ${stageLeads.length ? stageLeads.map(l => `
                            <div class="pp-card" onclick="openLeadDetail(${l.id})">
                                <div class="pp-card-name">${l.first_name || ''} ${l.last_name || ''}</div>
                                <div class="pp-card-phone">${l.phone || 'No phone'}</div>
                                <div class="pp-card-meta">
                                    ${l.assigned_agent ? `<span class="pp-card-tag agent">${l.assigned_agent}</span>` : ''}
                                    ${l.industry ? `<span class="pp-card-tag">${l.industry}</span>` : ''}
                                    <span class="pp-card-tag time">${l.created_date ? new Date(l.created_date).toLocaleDateString() : ''}</span>
                                </div>
                                <div class="pp-card-actions">
                                    <button class="pp-card-btn" onclick="event.stopPropagation();callLead(${l.id})">📞 Call</button>
                                    <button class="pp-card-btn success" onclick="event.stopPropagation();moveLead(${l.id},'next')">→ Next</button>
                                </div>
                            </div>
                        `).join('') : '<div class="pp-empty"><div class="pp-empty-icon">📭</div><div class="pp-empty-text">No leads</div></div>'}
                    </div>
                </div>`;
            }).join('');
        }
        ...
        // Build funnel
        const funnelData = [
            {label:'Total Leads', count: filtered.length, color:'blue'},
            {label:'Contacted', count: filtered.filter(l=>['contacted','no_answer','callback_scheduled','qualified','appointment_set','appt_completed','proposal_sent','negotiation','sold'].includes(l.pipeline_stage)).length, color:'purple'},
            {label:'Qualified', count: filtered.filter(l=>['qualified','appointment_set','appt_completed','proposal_sent','negotiation','sold'].includes(l.pipeline_stage)).length, color:'yellow'},
            {label:'Sold', count: sold, color:'green'}
        ];
        const maxCount = Math.max(...funnelData.map(f=>f.count), 1);
        const funnelEl = $('pp-funnel-bars');
        if(funnelEl) {
            funnelEl.innerHTML = funnelData.map(f => `
                <div class="pp-funnel-bar">
                    <div class="pp-funnel-label">${f.label}</div>
                    <div class="pp-funnel-track">
                        <div class="pp-funnel-fill ${f.color}" style="width:${(f.count/maxCount)*100}%">
                            <span class="pp-funnel-value">${f.count}</span>
                        </div>
                    </div>
                    <div class="pp-funnel-pct">${filtered.length?Math.round(f.count/filtered.length*100):0}%</div>
                </div>
            `).join('');
        }
        
        // Recent activity (mock for now - would come from audit log)
        const activityEl = $('pp-activity');
        if(activityEl) {
            const recentLeads = filtered.slice(0, 5);
            activityEl.innerHTML = recentLeads.length ? recentLeads.map(l => `
                <div class="pp-activity-item">
                    <div class="pp-activity-icon ${l.pipeline_stage === 'sold' ? 'sale' : 'move'}">
                        ${l.pipeline_stage === 'sold' ? '💰' : '📋'}
                    </div>
                    <div class="pp-activity-content">
                        <div class="pp-activity-text"><strong>${l.first_name || 'Lead'}</strong> moved to ${l.pipeline_stage?.replace(/_/g,' ')}</div>
                        <div class="pp-activity-time">${l.updated_at ? new Date(l.updated_at).toLocaleString() : 'Recently'}</div>
                    </div>
                </div>
            `).join('') : '<div style="padding:40px;text-align:center;color:rgba(255,255,255,0.4)">No recent activity</div>';
        }
...
function filterPipeline() { refreshPipeline(); }

async function moveLead(leadId, direction) {
    const stages = ['new_lead','contacted','no_answer','callback_scheduled','qualified','appointment_set','appt_completed','proposal_sent','negotiation','sold','lost_dead'];
    try {
        const lead = await fetch('/api/leads/' + leadId).then(r => r.json());
        const currentIdx = stages.indexOf(lead.pipeline_stage || 'new_lead');
        const nextIdx = direction === 'next' ? Math.min(currentIdx + 1, stages.length - 1) : Math.max(currentIdx - 1, 0);
        await fetch('/api/leads/' + leadId, {
            method: 'PUT',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({pipeline_stage: stages[nextIdx]})
        });
        toast('Lead moved to ' + stages[nextIdx].replace(/_/g, ' '));
        refreshPipeline();
    } catch(e) {
        toast('Error moving lead', true);
    }
}
...
if(service==='gcal'||service==='google'){
    openModal('google-setup-modal');
} else if(service==='facebook'){
    openModal('facebook-setup-modal');
} else if(service==='stripe'){
    openModal('stripe-setup-modal');
} else if(service==='outlook'){
    openModal('outlook-setup-modal');
} else if(service==='retell'){
    openModal('retell-setup-modal');
} else if(service==='vapi'){
    openModal('vapi-setup-modal');
} else if(service==='twilio'){
    openModal('twilio-setup-modal');
} else {
    toast('Setup for '+service+' coming soon!');
}
}

// Integration Save Functions
async function saveFacebookSetup(){
    const appId = $('fb-app-id').value.trim();
    const appSecret = $('fb-app-secret').value.trim();
    const accountId = $('fb-account-id').value.trim();
    if(!appId || !appSecret){toast('Please enter App ID and Secret',true);return}
    try {
        const r = await fetch('/api/integrations/facebook', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({app_id: appId, app_secret: appSecret, account_id: accountId})
        }).then(r=>r.json());
        if(r.success){
            toast('✅ Facebook connected successfully!');
            closeModal('facebook-setup-modal');
            // Update UI
            const card = document.querySelector('[data-service="facebook"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
                card.querySelector('.int-conn-btn').innerHTML = '✓ Connected';
                card.querySelector('.int-conn-btn').className = 'int-conn-btn connected';
            }
        } else {
            toast(r.error || 'Connection failed', true);
        }
    } catch(e) {
...
async function saveRetellSetup(){
    const apiKey = $('retell-api-key').value.trim();
    const outboundAgent = $('retell-outbound-agent').value.trim();
    const inboundAgent = $('retell-inbound-agent').value.trim();
    const outboundPhone = $('retell-outbound-phone').value.trim();
    const inboundPhone = $('retell-inbound-phone').value.trim();
    if(!apiKey){toast('Please enter Retell API Key',true);return}
    try {
        const r = await fetch('/api/integrations/retell', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                api_key: apiKey, 
        ...
        if(r.success){
            toast('✅ Retell connected successfully!');
            closeModal('retell-setup-modal');
            const card = document.querySelector('[data-service="retell"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
...
async function saveVapiSetup(){
    const apiKey = $('vapi-api-key').value.trim();
    const phoneId = $('vapi-phone-id').value.trim();
    const assistantId = $('vapi-assistant-id').value.trim();
    if(!apiKey){toast('Please enter VAPI API Key',true);return}
    try {
        const r = await fetch('/api/integrations/vapi', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({api_key: apiKey, phone_id: phoneId, assistant_id: assistantId})
        }).then(r=>r.json());
        if(r.success){
            toast('✅ VAPI connected successfully!');
            closeModal('vapi-setup-modal');
            const card = document.querySelector('[data-service="vapi"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
...
async function saveTwilioSetup(){
    const sid = $('twilio-sid').value.trim();
    const token = $('twilio-token').value.trim();
    const phone = $('twilio-phone').value.trim();
    if(!sid || !token){toast('Please enter Account SID and Auth Token',true);return}
    try {
        const r = await fetch('/api/integrations/twilio', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({account_sid: sid, auth_token: token, phone_number: phone})
        }).then(r=>r.json());
        if(r.success){
            toast('✅ Twilio connected successfully!');
            closeModal('twilio-setup-modal');
            const card = document.querySelector('[data-service="twilio"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
...
async function saveGoogleSetup(){
    ...
    if(!clientId || !clientSecret){toast('Please enter Client ID and Secret',true);return}
    try {
...
async function saveStripeSetup(){
    ...
    const webhook = $('stripe-webhook').value.trim();
    if(!secret){toast('Please enter Stripe Secret Key',true);return}
    try {
        const r = await fetch('/api/integrations/stripe', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({secret_key: secret, webhook_secret: webhook})
        }).then(r=>r.json());
        if(r.success){
            toast('✅ Stripe connected!');
            closeModal('stripe-setup-modal');
            const card = document.querySelector('[data-service="stripe"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
...
async function saveOutlookSetup(){
    const clientId = $('outlook-client-id').value.trim();
    const clientSecret = $('outlook-client-secret').value.trim();
    if(!clientId || !clientSecret){toast('Please enter Client ID and Secret',true);return}
    try {
        const r = await fetch('/api/integrations/outlook', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({client_id: clientId, client_secret: clientSecret})
        }).then(r=>r.json());
        if(r.success){
            ...
            closeModal('outlook-setup-modal');
            const card = document.querySelector('[data-service="outlook"]');
            if(card){
                card.classList.add('connected');
                card.querySelector('.int-conn-badge').textContent = 'Connected';
                card.querySelector('.int-conn-badge').className = 'int-conn-badge connected';
...
async function sendResetLink(){
    ...
    if(!email){toast('Please enter your email',true);return}
    try {
        ...
        toast('Error sending reset link',true);
    }
}
...
async function doResetPassword(){
    ...
    try {
        ...
        if(r.success){
            ...
            toast(r.error||'Reset failed',true);
        }
    } catch(e) {
        toast('Error resetting password',true);
    }
}
...
<div class="card"><div class="card-header"><h2>Pending Dispositions</h2></div><div id="pend-list" style="padding:16px;display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px"></div></div></div>
...
<style>
/* Pipeline Pro - Multi-Agent CRM */
#page-pipeline{background:linear-gradient(180deg,#080b12 0%,#0a0e17 100%)}
.pipeline-pro{max-width:100%;padding:0}

/* Header */
.pp-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px;flex-wrap:wrap;gap:20px}
.pp-title-area h1{font-size:28px;font-weight:700;color:#fff;margin:0 0 6px 0;letter-spacing:-0.5px}
.pp-title-area p{font-size:13px;color:rgba(255,255,255,0.4);margin:0}
.pp-controls{display:flex;gap:12px;align-items:center}
.pp-select{padding:10px 16px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:8px;color:#fff;font-size:13px;cursor:pointer;min-width:180px}
.pp-select:focus{outline:none;border-color:rgba(59,130,246,0.5)}
.pp-select option{background:#0a0e17}
.pp-btn{padding:10px 20px;background:#3b82f6;border:none;border-radius:8px;color:#fff;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;gap:8px}
.pp-btn:hover{background:#2563eb;transform:translateY(-1px)}
.pp-btn.secondary{background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1)}
.pp-btn.secondary:hover{background:rgba(255,255,255,0.1)}

/* Pipeline Stats Bar */
.pp-stats{display:grid;grid-template-columns:repeat(5,1fr);gap:16px;margin-bottom:32px}
@media(max-width:1000px){.pp-stats{grid-template-columns:repeat(3,1fr)}}
@media(max-width:600px){.pp-stats{grid-template-columns:repeat(2,1fr)}}
.pp-stat{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:20px;position:relative;overflow:hidden}
.pp-stat::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--accent,#3b82f6)}
.pp-stat-value{font-size:32px;font-weight:700;color:#fff}
.pp-stat-label{font-size:11px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;margin-top:6px}
.pp-stat.green::before{background:#22c55e}.pp-stat.green .pp-stat-value{color:#22c55e}
.pp-stat.yellow::before{background:#eab308}.pp-stat.yellow .pp-stat-value{color:#eab308}
.pp-stat.red::before{background:#ef4444}.pp-stat.red .pp-stat-value{color:#ef4444}
.pp-stat.cyan::before{background:#00D1FF}.pp-stat.cyan .pp-stat-value{color:#00D1FF}

/* Kanban Board */
.pp-board{display:flex;gap:12px;overflow-x:auto;padding-bottom:20px;margin-bottom:32px}
.pp-board::-webkit-scrollbar{height:6px}
.pp-board::-webkit-scrollbar-track{background:rgba(255,255,255,0.02);border-radius:3px}
.pp-board::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:3px}
.pp-column{min-width:220px;max-width:220px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:12px;display:flex;flex-direction:column}
.pp-column-header{padding:16px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-between;align-items:center}
.pp-column-title{font-size:12px;font-weight:600;color:rgba(255,255,255,0.7);text-transform:uppercase;letter-spacing:0.5px;display:flex;align-items:center;gap:8px}
.pp-column-title .icon{font-size:14px}
.pp-column-count{background:rgba(255,255,255,0.1);padding:4px 10px;border-radius:12px;font-size:13px;font-weight:700;color:#fff}
.pp-column-body{padding:12px;flex:1;min-height:200px;max-height:400px;overflow-y:auto}
.pp-column-body::-webkit-scrollbar{width:4px}
.pp-column-body::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.1);border-radius:2px}

/* Lead Cards */
.pp-card{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:14px;margin-bottom:10px;cursor:pointer;transition:all 0.2s}
.pp-card:hover{background:rgba(255,255,255,0.05);border-color:rgba(59,130,246,0.3);transform:translateY(-2px)}
.pp-card:last-child{margin-bottom:0}
.pp-card-name{font-size:14px;font-weight:600;color:#fff;margin-bottom:4px}
.pp-card-phone{font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:10px}
.pp-card-meta{display:flex;gap:8px;flex-wrap:wrap}
.pp-card-tag{font-size:10px;padding:3px 8px;border-radius:4px;background:rgba(59,130,246,0.15);color:#3b82f6;font-weight:500}
.pp-card-tag.agent{background:rgba(168,85,247,0.15);color:#a855f7}
.pp-card-tag.time{background:rgba(255,255,255,0.08);color:rgba(255,255,255,0.6)}
.pp-card-actions{display:flex;gap:6px;margin-top:12px;padding-top:10px;border-top:1px solid rgba(255,255,255,0.06)}
.pp-card-btn{flex:1;padding:8px;background:rgba(255,255,255,0.05);border:none;border-radius:6px;color:rgba(255,255,255,0.7);font-size:11px;cursor:pointer;transition:all 0.15s}
.pp-card-btn:hover{background:rgba(59,130,246,0.2);color:#3b82f6}
.pp-card-btn.danger:hover{background:rgba(239,68,68,0.2);color:#ef4444}
.pp-card-btn.success:hover{background:rgba(34,197,94,0.2);color:#22c55e}

/* Conversion Funnel */
.pp-funnel{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:14px;padding:24px;margin-bottom:32px}
.pp-funnel-title{font-size:14px;font-weight:600;color:#fff;margin-bottom:20px;display:flex;align-items:center;gap:10px}
.pp-funnel-title::after{content:'';flex:1;height:1px;background:rgba(255,255,255,0.06)}
.pp-funnel-bar{display:flex;align-items:center;margin-bottom:16px}
.pp-funnel-bar:last-child{margin-bottom:0}
.pp-funnel-label{width:140px;font-size:12px;color:rgba(255,255,255,0.6)}
.pp-funnel-track{flex:1;height:32px;background:rgba(255,255,255,0.04);border-radius:6px;overflow:hidden;position:relative}
.pp-funnel-fill{height:100%;border-radius:6px;transition:width 0.6s ease;display:flex;align-items:center;justify-content:flex-end;padding-right:12px}
.pp-funnel-fill.blue{background:linear-gradient(90deg,#3b82f6,#2563eb)}
.pp-funnel-fill.purple{background:linear-gradient(90deg,#8b5cf6,#7c3aed)}
.pp-funnel-fill.green{background:linear-gradient(90deg,#22c55e,#16a34a)}
.pp-funnel-fill.yellow{background:linear-gradient(90deg,#eab308,#ca8a04)}
.pp-funnel-value{font-size:13px;font-weight:700;color:#fff}
.pp-funnel-pct{width:60px;text-align:right;font-size:12px;color:rgba(255,255,255,0.4);margin-left:12px}

/* Recent Activity */
.pp-activity{background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:14px;overflow:hidden}
.pp-activity-header{padding:18px 22px;border-bottom:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-between;align-items:center}
.pp-activity-title{font-size:14px;font-weight:600;color:#fff}
.pp-activity-list{max-height:300px;overflow-y:auto}
.pp-activity-item{padding:14px 22px;border-bottom:1px solid rgba(255,255,255,0.04);display:flex;align-items:center;gap:14px}
.pp-activity-item:last-child{border-bottom:none}
.pp-activity-icon{width:36px;height:36px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0}
.pp-activity-icon.call{background:rgba(59,130,246,0.15)}
.pp-activity-icon.move{background:rgba(168,85,247,0.15)}
.pp-activity-icon.sale{background:rgba(34,197,94,0.15)}
.pp-activity-icon.lost{background:rgba(239,68,68,0.15)}
.pp-activity-content{flex:1;min-width:0}
.pp-activity-text{font-size:13px;color:rgba(255,255,255,0.85)}
.pp-activity-text strong{color:#fff}
.pp-activity-time{font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px}

/* Empty State */
.pp-empty{text-align:center;padding:60px 20px;color:rgba(255,255,255,0.4)}
.pp-empty-icon{font-size:48px;margin-bottom:16px;opacity:0.5}
.pp-empty-text{font-size:14px}
</style>

<div class="pipeline-pro">
    <div class="pp-header">
        <div class="pp-title-area">
            <h1>🔄 Pipeline</h1>
            <p>Track leads across multiple agents and industries</p>
        </div>
        <div class="pp-controls">
            <select class="pp-select" id="pp-agent-filter" onchange="filterPipeline()">
                <option value="all">All Agents</option>
            </select>
            <select class="pp-select" id="pp-industry-filter" onchange="filterPipeline()">
                <option value="all">All Industries</option>
            </select>
            <button class="pp-btn secondary" onclick="refreshPipeline()">↻ Refresh</button>
            <button class="pp-btn" onclick="openModal('lead-modal')">+ Add Lead</button>
        </div>
    </div>

    <div class="pp-stats">
        <div class="pp-stat cyan"><div class="pp-stat-value" id="pp-total">0</div><div class="pp-stat-label">Total Leads</div></div>
        <div class="pp-stat"><div class="pp-stat-value" id="pp-active">0</div><div class="pp-stat-label">Active</div></div>
        <div class="pp-stat green"><div class="pp-stat-value" id="pp-qualified">0</div><div class="pp-stat-label">Qualified</div></div>
        <div class="pp-stat yellow"><div class="pp-stat-value" id="pp-conversion">0%</div><div class="pp-stat-label">Conversion</div></div>
        <div class="pp-stat"><div class="pp-stat-value" id="pp-revenue">$0</div><div class="pp-stat-label">Pipeline Value</div></div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px">
        <div class="pp-funnel">
            <div class="pp-funnel-title">📊 Conversion Funnel</div>
            <div id="pp-funnel-bars"></div>
        </div>
        <div class="pp-activity">
            <div class="pp-activity-header">
                <div class="pp-activity-title">⚡ Recent Activity</div>
            </div>
            <div class="pp-activity-list" id="pp-activity"></div>
        </div>
    </div>
</div>
</div>
...
<button class="int-conn-btn connected" onclick="aiConnect('vapi')">⚙️ Manage</button>
</div>
<!-- Retell -->
<div class="int-conn-card" data-status="setup" data-service="retell">
<div class="int-conn-top">
<div class="int-conn-logo">🔊</div>
<span class="int-conn-badge ready">Voice AI</span>
</div>
<div class="int-conn-name">Retell AI</div>
<div class="int-conn-desc">Advanced voice agents</div>
<div class="int-conn-health">
<div class="int-conn-health-item"><div class="int-conn-health-value" id="retell-agents">—</div><div class="int-conn-health-label">Agents</div></div>
<div class="int-conn-health-item"><div class="int-conn-health-value" id="retell-calls">—</div><div class="int-conn-health-label">Calls</div></div>
</div>
<button class="int-conn-btn primary" onclick="aiConnect('retell')">🔗 Connect Retell</button>
...
<button class="int-conn-btn connected" onclick="aiConnect('twilio')">⚙️ Manage</button>
</div>
<!-- Google Calendar -->
...
<div class="modal-bg" id="forgot-modal"><div class="modal" style="max-width:400px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">🔐</div>
<h2 style="margin:0 0 8px 0">Reset Password</h2>
<p style="color:var(--gray-500);font-size:13px;margin:0">Enter your email to receive a reset link</p>
</div>
<div class="form-group"><label>Email</label><input id="forgot-email" type="email" placeholder="you@company.com"></div>
<div class="modal-btns" style="flex-direction:column;gap:8px">
<button class="btn btn-primary" style="width:100%" onclick="sendResetLink()">Send Reset Link</button>
<button class="btn btn-secondary" style="width:100%" onclick="closeModal('forgot-modal');openModal('login-modal')">Back to Login</button>
</div>
</div></div>

<!-- Reset Password Modal (for when user clicks reset link) -->
<div class="modal-bg" id="reset-modal"><div class="modal" style="max-width:400px">
<div style="text-align:center;margin-bottom:24px">
<div style="font-size:48px;margin-bottom:12px">🔑</div>
<h2 style="margin:0 0 8px 0">Set New Password</h2>
</div>
<div class="form-group"><label>New Password</label><input id="reset-pw" type="password" placeholder="••••••••"></div>
<div class="form-group"><label>Confirm Password</label><input id="reset-pw2" type="password" placeholder="••••••••"></div>
<input type="hidden" id="reset-token">
<div class="modal-btns">
<button class="btn btn-primary" style="width:100%" onclick="doResetPassword()">Update Password</button>
</div>
</div></div>
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == '/api/integrations/facebook':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                c.execute("""INSERT OR REPLACE INTO integrations (service, api_key, api_secret, status, config) 
                             VALUES ('facebook_ads', ?, ?, 'active', ?)""",
                          (data.get('app_id',''), data.get('app_secret',''), 
                           json.dumps({'account_id': data.get('account_id','')})))
                conn.commit()
                return json.dumps({'success': True})
            except Exception as e:
                return json.dumps({'error': str(e)})
                
        elif path == '/api/integrations/google':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                c.execute("""INSERT OR REPLACE INTO integrations (service, api_key, api_secret, status) 
        ...
        elif path == '/api/integrations/stripe':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                c.execute("""INSERT OR REPLACE INTO integrations (service, api_key, api_secret, status) 
                          ...
                          (data.get('secret_key',''), data.get('webhook_secret','')))
                conn.commit()
                return json.dumps({'success': True})
            except Exception as e:
                return json.dumps({'error': str(e)})
                
        elif path == '/api/integrations/outlook':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                ...
                return json.dumps({'error': str(e)})
        elif path == '/api/integrations/retell':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                config = json.dumps({
                    'outbound_agent': data.get('outbound_agent',''),
                    'inbound_agent': data.get('inbound_agent',''),
                    'outbound_phone': data.get('outbound_phone',''),
                    'inbound_phone': data.get('inbound_phone','')
                })
                c.execute("""INSERT OR REPLACE INTO integrations (service, api_key, status, config) 
                             VALUES ('retell', ?, 'active', ?)""",
                          (data.get('api_key',''), config))
                conn.commit()
                return json.dumps({'success': True})
            except Exception as e:
                return json.dumps({'error': str(e)})
                
        elif path == '/api/integrations/vapi':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
        ...
        elif path == '/api/integrations/twilio':
            data = json.loads(body) if body else {}
            try:
                conn = get_db()
                c = conn.cursor()
                config = json.dumps({'phone_number': data.get('phone_number','')})
                c.execute("""INSERT OR REPLACE INTO integrations (service, api_key, api_secret, status, config) 
                             VALUES ('twilio', ?, ?, 'active', ?)""",
                          (data.get('account_sid',''), data.get('auth_token',''), config))
                conn.commit()
                return json.dumps({'success': True})
            except Exception as e:
                return json.dumps({'error': str(e)})
    ...
    def do_POST(self):
        ...
        # Auth routes
            ...
            self.send_json(request_password_reset(d.get('email', '')))
        elif path == '/api/reset-password':
            self.send_json(reset_password(d.get('token', ''), d.get('password', '')))
        # Integration routes
...
def main():
    ...
    ''')
    # Only open browser locally, not on Railway/cloud
    import sys
    if 'RAILWAY_ENVIRONMENT' not in os.environ and 'RENDER' not in os.environ:
