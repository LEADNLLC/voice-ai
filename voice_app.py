...
def get_sms_conversations():
    ...
    try:
        ...
        """)
        convos = [dict(row) for row in c.fetchall()]
        conn.close()
        return convos
    except Exception as e:
...
def handle_call_completion(call_id, outcome, duration=0):
    ...
    try:
        ...
        if not row:
            conn.close()
            print(f"   ⚠️ No pending call found for: {call_id}")
            return
...
async function loadMessages() {
    ...
    try {
        const convos = await fetch('/api/sms/conversations').then(r => r.json()).catch(e => { console.error('fetch error:', e); return []; });
        console.log('Conversations loaded:', convos.length, convos);
        smsConvos = convos;
...
function renderConvoList(convos) {
    console.log('renderConvoList called with', convos.length, 'convos');
    const list = $('sms-convo-list');
    if (!list) { console.error('sms-convo-list element not found'); return; }
    if (!convos.length) {
        list.innerHTML = '<div style="padding:40px;text-align:center;color:var(--gray-500)"><div style="font-size:48px;margin-bottom:16px">💬</div><div style="font-weight:600;margin-bottom:8px">No conversations yet</div><div style="font-size:13px">Send your first message to get started</div></div>';
        return;
    }
    
    // Update unread badge in nav
    const totalUnread = convos.reduce((sum, c) => sum + (c.unread_count || 0), 0);
    const badge = $('unread-sms');
    if (badge) {
    ...
    list.innerHTML = convos.map(c => {
        ...
        const timeAgo = c.updated_at ? formatTimeAgo(c.updated_at) : '';
        return '<div onclick="openConvo(' + c.id + ')" style="padding:14px 16px;border-bottom:1px solid var(--border);cursor:pointer;background:' + (isSelected ? 'rgba(20,184,166,0.1)' : 'transparent') + ';transition:background 0.15s" onmouseover="this.style.background=\\'' + (isSelected ? 'rgba(20,184,166,0.15)' : 'rgba(255,255,255,0.03)') + '\\'" onmouseout="this.style.background=\\'' + (isSelected ? 'rgba(20,184,166,0.1)' : 'transparent') + '\\'">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">' +
        '<span style="font-weight:' + (hasUnread ? '600' : '500') + ';color:' + (hasUnread ? '#fff' : 'rgba(255,255,255,0.9)') + '">' + (c.contact_name || formatPhoneDisplay(c.phone)) + '</span>' +
        (hasUnread ? '<span style="background:#14b8a6;color:#000;font-size:11px;font-weight:700;padding:2px 6px;border-radius:10px;min-width:18px;text-align:center">' + c.unread_count + '</span>' : '<span style="font-size:11px;color:var(--gray-500)">' + timeAgo + '</span>') +
        '</div>' +
        '<div style="font-size:13px;color:' + (hasUnread ? 'rgba(255,255,255,0.7)' : 'var(--gray-500)') + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:' + (hasUnread ? '500' : '400') + '">' + (c.last_message || 'No messages yet') + '</div>' +
        '</div>';
    }).join('');
}
...
async function openConvo(id) {
    ...
    const convo = await fetch('/api/sms/conversations/' + id).then(r => r.json()).catch(e => { console.error('openConvo fetch error:', e); return null; });
    if (!convo) { console.error('No convo data'); return; }
    console.log('Conversation data:', convo);
    currentConvo = convo;
    const area = $('sms-chat-area');
    if (!area) { console.error('sms-chat-area not found'); return; }
    ...
    function getStatusIndicator(msg) {
        ...
        if (status === 'delivered') {
            ...
            text = 'Read';
            color = '#3b82f6';
        } else if (status === 'sent' || status === 'queued' || status === 'accepted') {
            icon = '✓';
            text = 'Sent';
        ...
        return '<span style="color:' + color + ';margin-left:6px" title="' + text + '">' + icon + '</span>';
    }
    ...
    messages.forEach((m, i) => {
        ...
        if (msgDate && msgDate !== lastDate) {
            ...
            messagesHtml += '<div style="text-align:center;padding:16px 0 8px"><span style="background:rgba(255,255,255,0.1);padding:4px 12px;border-radius:12px;font-size:11px;color:rgba(255,255,255,0.5)">' + dateLabel + '</span></div>';
            lastDate = msgDate;
        }
        ...
        messagesHtml += '<div style="display:flex;flex-direction:column;align-items:' + (isOutbound ? 'flex-end' : 'flex-start') + ';margin-bottom:4px">' +
            '<div style="max-width:75%;display:flex;flex-direction:column;align-items:' + (isOutbound ? 'flex-end' : 'flex-start') + '">' +
            '<div style="padding:10px 14px;border-radius:18px;background:' + (isOutbound ? 'linear-gradient(135deg, #14b8a6, #0d9488)' : 'rgba(255,255,255,0.1)') + ';color:#fff;font-size:14px;line-height:1.5;word-wrap:break-word">' + (m.message || '').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>' +
            '<div style="display:flex;align-items:center;margin-top:4px;font-size:11px;color:rgba(255,255,255,0.4)">' +
            '<span>' + time + '</span>' +
    ...
    area.innerHTML = 
        ...
        '<div>' +
        '<div style="font-weight:600;font-size:16px">' + (convo.contact_name || formatPhoneDisplay(convo.phone)) + '</div>' +
        '<div style="font-size:12px;color:var(--gray-500);margin-top:2px">' + convo.phone + '</div>' +
        '</div>' +
        ...
        '<input id="sms-input" style="flex:1;padding:12px 18px;border-radius:24px;border:1px solid var(--border);background:rgba(255,255,255,0.05);color:#fff;font-size:14px" placeholder="Type a message..." onkeypress="if(event.key===\\'Enter\\')sendSmsMsg()">' +
        '<button class="btn btn-primary" style="border-radius:50%;width:44px;height:44px;padding:0;display:flex;align-items:center;justify-content:center;font-size:18px" onclick="sendSmsMsg()">➤</button>' +
        '</div>' +
        '</div>' +
        '</div>';
    
    // Scroll to bottom
    const msgContainer = $('sms-messages');
    if (msgContainer) {
        setTimeout(() => { msgContainer.scrollTop = msgContainer.scrollHeight; }, 50);
    }
    
    // Refresh conversation list to update read status
    loadMessages();
}
...
async function sendSmsMsg() {
    ...
    // Optimistic UI update - show message immediately as "Sending..."
    const msgContainer = $('sms-messages');
    if (msgContainer) {
        const tempMsg = document.createElement('div');
        tempMsg.style.cssText = 'display:flex;flex-direction:column;align-items:flex-end';
        tempMsg.innerHTML = '<div style="display:inline-block;padding:10px 14px;border-radius:18px;max-width:70%;background:linear-gradient(135deg, #14b8a6, #0d9488);color:#fff;font-size:14px;line-height:1.4">' + msg.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</div>' +
        '<div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;text-align:right">Sending...</div>';
    ...
    // Refresh the conversation to get actual status
...
<div class="page-header"><h1>💬 Messages</h1><p>SMS conversations with leads</p></div>
<div style="display:grid;grid-template-columns:300px 1fr;gap:20px;height:calc(100vh - 200px)">
<div style="background:rgba(255,255,255,0.02);border-radius:12px;border:1px solid var(--border);overflow:hidden">
<div style="padding:16px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
<span style="font-weight:600">Conversations</span>
<button class="btn btn-sm btn-primary" onclick="openModal('new-sms-modal')">+ New</button>
</div>
<div id="sms-convo-list" style="overflow-y:auto;max-height:calc(100vh - 280px)"></div>
</div>
<div style="background:rgba(255,255,255,0.02);border-radius:12px;border:1px solid var(--border);display:flex;flex-direction:column">
<div id="sms-chat-area" style="flex:1;display:flex;align-items:center;justify-content:center;color:var(--gray-500)">
...
<!-- New SMS Modal -->
<div class="modal-bg" id="new-sms-modal"><div class="modal" style="max-width:400px">
<h2>💬 New Message</h2>
<div class="form-group"><label>Phone Number</label><input id="new-sms-phone" type="tel" placeholder="+1 (555) 123-4567"></div>
<div class="form-group"><label>Contact Name</label><input id="new-sms-name" placeholder="Optional"></div>
<div class="form-group"><label>Message</label><textarea id="new-sms-msg" rows="3" placeholder="Type your message..."></textarea></div>
<div class="modal-btns">
<button class="btn btn-secondary" onclick="closeModal('new-sms-modal')">Cancel</button>
<button class="btn btn-primary" onclick="startNewConvo()">Send</button>
</div>
</div></div>
    ...
    def do_POST(self):
        ...
        # ═══════════════════════════════════════════════════════════════════════════════
            ...
            try:
                ...
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute('''INSERT INTO leads (phone, first_name, last_name, email, source, status, created_at)
            ...
            try:
                ...
                if event_type == 'invitee.created':
                    ...
                    c.execute('''INSERT INTO appointments 
                                 ...
                                 VALUES (?, ?, ?, ?, ?, ?, 'confirmed', 'calendly', ?, CURRENT_TIMESTAMP)''',
                              (first_name, last_name, invitee_email, invitee_phone, appt_date, appt_time, event_name))
                    appt_id = c.lastrowid
                    ...
                    if lead_id:
                        c.execute("UPDATE leads SET status = 'appointment_set', appointment_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", 
                                  (appt_id, lead_id))
                        # Update compliance tracking
        ...
        # Test Lead - Creates lead and optionally auto-calls (same flow as Facebook leads)
            ...
            try:
                ...
                c.execute('''INSERT INTO leads (phone, first_name, last_name, email, source, status, created_at)
                          ...
                          (phone, first_name, last_name, email, source))
                lead_id = c.lastrowid
                conn.commit()
                conn.close()
