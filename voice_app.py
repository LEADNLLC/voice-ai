...
let html='';
for(let i=0;i<firstDay;i++){const d=new Date(y,m,1-firstDay+i);html+='<div class="cal-day-cell other-month" onclick="selectCalDay(\\''+d.toISOString().split('T')[0]+'\\')">';html+='<div class="cal-day-num">'+d.getDate()+'</div></div>'}
...
html+='<div class="cal-day-cell'+(isToday?' today':'')+(isSelected?' selected':'')+'" onclick="selectCalDay(\\''+dt+'\\')">';
...
bHtml+='<div class="cal-week-slot" onclick="quickBookSlot(\\''+dateStr+'\\',\\''+timeKey+'\\')">';
if(appt){
...
html+='<div class="cal-daily-empty" onclick="quickBookSlot(\\''+dateStr+'\\',\\''+timeKey+'\\')">+ Add appointment</div>';
}
html+='</div></div>';
}
...
for(let d=1;d<=daysInMonth;d++){
const dt=y+'-'+String(m+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');
html+='<div class="cal-mini-day'+(dt===today?' today':'')+(dt===selected?' selected':'')+'" onclick="selectCalDay(\\''+dt+'\\')">'+d+'</div>';
}
grid.innerHTML=html;
}
