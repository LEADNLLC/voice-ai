...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            # AUDIT TRAIL DASHBOARD - Visual view of ALL activity
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                ...
                # Get ALL recent calls from call_log (today's activity)
                c.execute('''SELECT * FROM call_log ORDER BY created_at DESC LIMIT 100''')
                all_calls = [dict(row) for row in c.fetchall()]
                
                conn.close()
                
                # Build sequences HTML
                rows_html = ""
                for seq in sequences:
                    status_color = {'active': '#10b981', 'completed': '#6b7280', 'stopped': '#ef4444', 'appointment_set': '#00d1ff', 'not_interested': '#ef4444', 'max_calls_reached': '#f59e0b'}.get(seq.get('status', ''), '#6b7280')
                    
                    calls_html = ""
                    for call in seq.get('calls', []):
                        tap_badge = "📞📞 DOUBLE" if call.get('is_double_tap') else "📞"
                        call_status_color = {'pending': '#f59e0b', 'executed': '#10b981', 'failed': '#ef4444', 'cancelled': '#6b7280', 'skipped': '#6b7280'}.get(call.get('status', ''), '#6b7280')
                ...
                if not rows_html:
                    rows_html = "<div style='text-align:center;color:#6b7280;padding:40px;'>No sequences yet.</div>"
                
                # Build ALL CALLS section
                calls_section = ""
                for call in all_calls:
                    status_color = {'completed': '#10b981', 'no_answer': '#f59e0b', 'short_call': '#f59e0b', 'failed': '#ef4444', 'appointment_set': '#00d1ff'}.get(call.get('status', ''), '#6b7280')
                    duration = call.get('duration', 0) or 0
                    calls_section += f'''<div style="display:flex;gap:16px;align-items:center;padding:12px;border-bottom:1px solid #1e1e2e;">
                        <span style="font-weight:600;">{call.get('phone', 'Unknown')}</span>
                        <span style="color:{status_color};font-weight:600;">{call.get('status', 'unknown').upper()}</span>
                        <span style="color:#6b7280;">{duration:.1f}s</span>
                        <span style="color:#6b7280;">{call.get('agent_type', '')}</span>
                        <span style="color:#6b7280;font-size:12px;">{str(call.get('created_at', ''))[:16]}</span>
                    </div>'''
                
                if not calls_section:
                    calls_section = "<div style='color:#6b7280;padding:20px;'>No calls yet today.</div>"
                
                html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>VOICE - Full Audit Trail</title>
...
.stats{{display:flex;gap:16px;margin-bottom:24px;flex-wrap:wrap}}.stat{{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:16px 24px}}
.stat-value{{font-size:24px;font-weight:700;color:var(--cyan)}}.stat-label{{font-size:12px;color:#6b7280}}
.section-title{{font-size:20px;font-weight:700;margin:32px 0 16px;padding-bottom:8px;border-bottom:1px solid var(--border)}}</style></head>
<body><div class="header"><div class="title">🔍 Full Audit Trail</div><button class="refresh" onclick="location.reload()">↻ Refresh</button></div>
<div class="stats">
<div class="stat"><div class="stat-value">{len(sequences)}</div><div class="stat-label">Total Sequences</div></div>
...
<div class="stat"><div class="stat-value">{len([s for s in sequences if s.get('status') == 'appointment_set'])}</div><div class="stat-label">Appointments</div></div>
<div class="stat"><div class="stat-value">{len(all_calls)}</div><div class="stat-label">Total Calls Today</div></div>
<div class="stat"><div class="stat-value">{len([c for c in all_calls if c.get('status') == 'completed'])}</div><div class="stat-label">Completed</div></div>
<div class="stat"><div class="stat-value">{len([c for c in all_calls if c.get('status') in ['no_answer', 'short_call']])}</div><div class="stat-label">No Answer</div></div>
</div>

<div class="section-title">📋 Active Sequences</div>
{rows_html}

<div class="section-title">📞 All Calls (Recent 100)</div>
