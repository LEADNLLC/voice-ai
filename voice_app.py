...
def send_sms(phone, message, message_type="general"):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            data = response.json()
            print(f"✅ Retell SMS sent successfully!")
            ...
            try:
                ...
                c.execute('INSERT INTO sms_log (phone, message, message_type, status, twilio_sid) VALUES (?, ?, ?, ?, ?)',
                          (phone, message, message_type, 'sent', data.get('id', 'retell')))
                conn.commit()
                conn.close()
            ...
            return {"success": True, "from": HAILEY_PHONE_NUMBER}
        else:
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None, address="", text_first=None):
    ...
    if text_first and agent_type == 'solar':
        ...
        # Send the intro text
        intro_message = f"Hey {name}! This is Hailey with the energy program. I'm about to give you a quick call - just wanted to give you a heads up! Talk in a sec 📞"
        
        sms_result = send_retell_sms(phone, intro_message)
...
def send_retell_sms(to_number, message, from_number=None):
    """Send SMS via Retell API - same number as calls!"""
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            data = response.json()
            print(f"✅ Retell SMS sent successfully!")
            return {'success': True, 'from': from_number, 'data': data}
        else:
            print(f"❌ Retell SMS failed: {response.status_code}")
            print(f"   Response: {response.text}")
            return {'success': False, 'error': response.text}
...
def text_then_call_immediate(phone, name="there", agent_type="solar", ghl_contact_id=None, address=""):
    ...
    # Send quick intro text
    intro_message = f"Hey {name}! Hailey here - calling you now about the energy program! 📞"
