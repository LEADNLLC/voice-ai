...
async function markSmsEnabled(type) {
    ...
    try {
        await fetch('/api/phone-config', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({phone: phone, sms_enabled: true})
        });
        ...
        toast('✅ ' + type.charAt(0).toUpperCase() + type.slice(1) + ' SMS marked as enabled!');
    } catch(e) {
...
async function loadSmsStatus() {
    try {
        ...
        phones.forEach(p => {
            if (p.sms_enabled) {
                let type = '';
                if (p.phone === '+17208640910') type = 'outbound';
                else if (p.phone === '+17207345479') type = 'inbound';
