...
let html='';
for(let i=0;i<firstDay;i++){const d=new Date(y,m,1-firstDay+i);html+='<div class="cal-day-cell other-month" onclick="selectCalDay(\\x27'+d.toISOString().split('T')[0]+'\\x27)"><div class="cal-day-num">'+d.getDate()+'</div></div>'}
...
html+='<div class="cal-day-cell'+(isToday?' today':'')+(isSelected?' selected':'')+'" onclick="selectCalDay(\\x27'+dt+'\\x27)">';
...
bHtml+='<div class="cal-week-slot" onclick="quickBookSlot(\\x27'+dateStr+'\\x27,\\x27'+timeKey+'\\x27)">';
if(appt){
...
html+='<div class="cal-daily-empty" onclick="quickBookSlot(\\x27'+dateStr+'\\x27,\\x27'+timeKey+'\\x27)">+ Add appointment</div>';
}
html+='</div></div>';
}
...
for(let d=1;d<=daysInMonth;d++){
const dt=y+'-'+String(m+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');
html+='<div class="cal-mini-day'+(dt===today?' today':'')+(dt===selected?' selected':'')+'" onclick="selectCalDay(\\x27'+dt+'\\x27)">'+d+'</div>';
}
grid.innerHTML=html;
}
...
async function refreshPipeline() {
    try {
        var data = await fetch('/api/lead-pipeline').then(function(r) { return r.json(); }).catch(function() { return {leads: [], stats: {}, pipeline: {}}; });
        pipelineData = data;
        
        // Update stats
        if ($('lp-total')) $('lp-total').textContent = data.stats.total || 0;
        if ($('lp-active')) $('lp-active').textContent = data.stats.active || 0;
        if ($('lp-contacted')) $('lp-contacted').textContent = data.stats.contacted || 0;
        if ($('lp-no-answer')) $('lp-no-answer').textContent = data.stats.no_answer || 0;
        if ($('lp-appointments')) $('lp-appointments').textContent = data.stats.appointments || 0;
        if ($('lp-not-interested')) $('lp-not-interested').textContent = data.stats.not_interested || 0;
        
        // Update stage counts
        if ($('stage-all')) $('stage-all').textContent = data.stats.total || 0;
        if ($('stage-new')) $('stage-new').textContent = (data.pipeline.new || []).length;
        if ($('stage-active')) $('stage-active').textContent = data.stats.active || 0;
        if ($('stage-contacted')) $('stage-contacted').textContent = (data.pipeline.contacted || []).length;
        if ($('stage-appointment')) $('stage-appointment').textContent = (data.pipeline.appointment || []).length;
        
        // Update slot counts
        if ($('slot-initial-count')) $('slot-initial-count').textContent = (data.pipeline.new || []).length;
        if ($('slot1-count')) $('slot1-count').textContent = (data.pipeline.slot1_pending || []).length;
        if ($('slot2-count')) $('slot2-count').textContent = (data.pipeline.slot2_pending || []).length;
        if ($('slot3-count')) $('slot3-count').textContent = (data.pipeline.slot3_pending || []).length;
...
function viewLeadDetail(leadId) {
    ...
    '</div>' +
    '<div style="display:flex;gap:12px">' +
        '<button class="btn btn-primary" style="flex:1" onclick="callLeadNow(' + leadId + ');closeModal(\\'lead-detail-modal\\')">📞 Call Now</button>' +
        '<button class="btn btn-secondary" onclick="closeModal(\\'lead-detail-modal\\')">Close</button>' +
    '</div>';
...
async function saveLead(){
    var p=$('l-phone').value;
    if(!p){toast('Phone required',true);return}
    
    // Create lead first
    var leadData = {
        phone: p,
        name: $('l-name').value || 'there',
        email: $('l-email') ? $('l-email').value : '',
        source: $('l-source') ? $('l-source').value : 'manual',
        agent_type: $('l-agent').value || 'roofing'
    };
    
    // Start the cycle which creates the lead
    ...
    if (r.lead_id) {
        // Start the sequence for this lead
        await fetch('/api/lead-sequence/start', {
            method: 'POST',
            headers: {'Content-Type':'application/json'},
            body: JSON.stringify({lead_id: r.lead_id, phone: p})
        });
    }
    
    closeModal('lead-modal');
    toast('✅ Lead added & sequence started!');
    refreshPipeline();
}
...
.lp-title{font-size:24px;font-weight:700}
.lp-sub{font-size:12px;color:var(--gray-500);margin-top:4px}
.lp-actions{display:flex;gap:12px}
.lp-stats{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-bottom:24px}
.lp-stat{background:var(--gray-900);border:1px solid var(--border);border-radius:12px;padding:16px;text-align:center}
.lp-stat-value{font-size:28px;font-weight:700}
.lp-stat-value.cyan{color:var(--cyan)}
.lp-stat-value.green{color:var(--success)}
.lp-stat-value.orange{color:var(--warning)}
.lp-stat-value.red{color:var(--danger)}
.lp-stat-label{font-size:10px;color:var(--gray-500);text-transform:uppercase;margin-top:4px}
.lp-sequence-panel{background:linear-gradient(135deg,rgba(0,209,255,0.05),rgba(168,85,247,0.05));border:1px solid rgba(0,209,255,0.2);border-radius:16px;padding:24px;margin-bottom:24px}
.lp-seq-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
.lp-seq-title{font-size:18px;font-weight:700;display:flex;align-items:center;gap:10px}
.lp-seq-badge{background:var(--cyan);color:var(--black);padding:4px 12px;border-radius:12px;font-size:11px;font-weight:600}
.lp-slots{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}
.lp-slot{background:var(--gray-900);border:1px solid var(--border);border-radius:12px;padding:20px;position:relative;overflow:hidden}
.lp-slot.active{border-color:var(--cyan);box-shadow:0 0 20px rgba(0,209,255,0.15)}
.lp-slot.completed{border-color:var(--success);opacity:0.7}
.lp-slot-time{font-size:24px;font-weight:700;margin-bottom:4px}
.lp-slot-label{font-size:11px;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.5px}
.lp-slot-count{position:absolute;top:12px;right:12px;background:var(--cyan);color:var(--black);padding:4px 10px;border-radius:10px;font-size:12px;font-weight:600}
.lp-slot-desc{font-size:12px;color:var(--gray-400);margin-top:8px;line-height:1.5}
.lp-slot-actions{margin-top:12px;display:flex;gap:8px}
.lp-slot-btn{flex:1;padding:8px;background:rgba(0,209,255,0.1);border:1px solid rgba(0,209,255,0.3);border-radius:6px;color:var(--cyan);font-size:11px;font-weight:600;cursor:pointer;transition:all .2s}
.lp-slot-btn:hover{background:rgba(0,209,255,0.2)}
.lp-pipeline{display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:24px}
.lp-stage{background:var(--gray-900);border:1px solid var(--border);border-radius:12px;padding:16px;cursor:pointer;transition:all .2s}
.lp-stage:hover{border-color:var(--cyan);transform:translateY(-2px)}
.lp-stage.selected{border-color:var(--cyan);background:rgba(0,209,255,0.05)}
.lp-stage-icon{font-size:24px;margin-bottom:8px}
.lp-stage-name{font-size:12px;font-weight:600;margin-bottom:4px}
.lp-stage-count{font-size:24px;font-weight:700}
.lp-leads-panel{background:var(--gray-900);border:1px solid var(--border);border-radius:12px;overflow:hidden}
.lp-leads-header{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center}
.lp-leads-title{font-size:14px;font-weight:600}
.lp-leads-filter{display:flex;gap:8px}
.lp-filter-btn{padding:6px 12px;background:transparent;border:1px solid var(--border);border-radius:6px;color:var(--gray-400);font-size:11px;cursor:pointer;transition:all .2s}
.lp-filter-btn:hover,.lp-filter-btn.active{background:var(--cyan);color:var(--black);border-color:var(--cyan)}
.lp-lead-row{display:flex;align-items:center;padding:14px 20px;border-bottom:1px solid var(--border);transition:background .2s}
.lp-lead-row:hover{background:rgba(255,255,255,0.02)}
.lp-lead-info{flex:1;min-width:0}
.lp-lead-name{font-weight:600;font-size:14px;margin-bottom:2px}
.lp-lead-phone{font-size:12px;color:var(--gray-500);font-family:monospace}
.lp-lead-meta{display:flex;gap:16px;margin-top:6px;font-size:11px;color:var(--gray-400)}
.lp-lead-status{display:flex;align-items:center;gap:8px}
.lp-status-badge{padding:4px 10px;border-radius:10px;font-size:10px;font-weight:600;text-transform:uppercase}
.lp-status-badge.new{background:rgba(59,130,246,0.15);color:#3b82f6}
.lp-status-badge.active{background:rgba(0,209,255,0.15);color:var(--cyan)}
.lp-status-badge.contacted{background:rgba(16,185,129,0.15);color:var(--success)}
.lp-status-badge.no-answer{background:rgba(245,158,11,0.15);color:var(--warning)}
.lp-status-badge.appointment{background:rgba(168,85,247,0.15);color:#a855f7}
.lp-status-badge.not-interested{background:rgba(239,68,68,0.15);color:var(--danger)}
.lp-lead-actions{display:flex;gap:8px}
.lp-action-btn{padding:8px 14px;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;transition:all .2s;border:none}
.lp-action-btn.primary{background:var(--cyan);color:var(--black)}
.lp-action-btn.secondary{background:rgba(255,255,255,0.05);color:var(--gray-300);border:1px solid var(--border)}
.lp-action-btn.danger{background:rgba(239,68,68,0.15);color:var(--danger)}
.lp-action-btn:hover{transform:translateY(-1px)}
.lp-empty{padding:60px;text-align:center;color:var(--gray-500)}
.lp-sequence-indicator{display:flex;align-items:center;gap:6px;font-size:11px;color:var(--gray-400)}
.lp-seq-dot{width:8px;height:8px;border-radius:50%;background:var(--gray-600)}
.lp-seq-dot.done{background:var(--success)}
.lp-seq-dot.active{background:var(--cyan);animation:pulse 1s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}
</style>

<div class="lp-header">
    <div>
        <div class="lp-title">👥 Lead Pipeline</div>
        <div class="lp-sub">Patented 3-Calls-Per-Day System with Double-Tap Technology</div>
    </div>
    <div class="lp-actions">
        <button class="btn btn-secondary" onclick="openModal('import-modal')">📤 Import CSV</button>
        <button class="btn btn-primary" onclick="openModal('lead-modal')">+ Add Lead</button>
    </div>
</div>

<div class="lp-stats">
    <div class="lp-stat"><div class="lp-stat-value cyan" id="lp-total">0</div><div class="lp-stat-label">Total Leads</div></div>
    <div class="lp-stat"><div class="lp-stat-value" id="lp-active">0</div><div class="lp-stat-label">In Sequence</div></div>
    <div class="lp-stat"><div class="lp-stat-value green" id="lp-contacted">0</div><div class="lp-stat-label">Contacted</div></div>
    <div class="lp-stat"><div class="lp-stat-value orange" id="lp-no-answer">0</div><div class="lp-stat-label">No Answer</div></div>
    <div class="lp-stat"><div class="lp-stat-value" style="color:#a855f7" id="lp-appointments">0</div><div class="lp-stat-label">Appointments</div></div>
    <div class="lp-stat"><div class="lp-stat-value red" id="lp-not-interested">0</div><div class="lp-stat-label">Not Interested</div></div>
</div>

<div class="lp-sequence-panel">
    <div class="lp-seq-header">
        <div class="lp-seq-title">⚡ Daily Calling Sequence <span class="lp-seq-badge">LIVE</span></div>
        <button class="btn btn-sm btn-primary" onclick="refreshPipeline()">🔄 Refresh</button>
    </div>
    <div class="lp-slots">
        <div class="lp-slot" id="slot-initial">
            <div class="lp-slot-count" id="slot-initial-count">0</div>
            <div class="lp-slot-time">🆕 New</div>
            <div class="lp-slot-label">Initial Contact</div>
            <div class="lp-slot-desc">Text + immediate call when lead arrives</div>
            <div class="lp-slot-actions">
                <button class="lp-slot-btn" onclick="runSlot(0)">▶️ Run Now</button>
            </div>
        </div>
        <div class="lp-slot" id="slot-1">
            <div class="lp-slot-count" id="slot1-count">0</div>
            <div class="lp-slot-time">8:30 AM</div>
            <div class="lp-slot-label">Morning Slot</div>
            <div class="lp-slot-desc">Double-tap: Call twice per lead</div>
            <div class="lp-slot-actions">
                <button class="lp-slot-btn" onclick="runSlot(1)">▶️ Run Slot 1</button>
            </div>
        </div>
        <div class="lp-slot" id="slot-2">
            <div class="lp-slot-count" id="slot2-count">0</div>
            <div class="lp-slot-time">12:15 PM</div>
            <div class="lp-slot-label">Midday Slot</div>
            <div class="lp-slot-desc">Double-tap: Call twice per lead</div>
            <div class="lp-slot-actions">
                <button class="lp-slot-btn" onclick="runSlot(2)">▶️ Run Slot 2</button>
            </div>
        </div>
        <div class="lp-slot" id="slot-3">
            <div class="lp-slot-count" id="slot3-count">0</div>
            <div class="lp-slot-time">5:30 PM</div>
            <div class="lp-slot-label">Evening Slot</div>
            <div class="lp-slot-desc">📱 Auto-text before call: "We've been trying to reach you..."</div>
            <div class="lp-slot-actions">
                <button class="lp-slot-btn" onclick="runSlot(3)">▶️ Run Slot 3</button>
            </div>
        </div>
    </div>
</div>
...
<div class="lp-pipeline">
    <div class="lp-stage selected" onclick="filterLeads('all')" data-stage="all">
        <div class="lp-stage-icon">📋</div>
        <div class="lp-stage-name">All Leads</div>
        <div class="lp-stage-count" id="stage-all">0</div>
    </div>
    <div class="lp-stage" onclick="filterLeads('new')" data-stage="new">
        <div class="lp-stage-icon">🆕</div>
        <div class="lp-stage-name">New</div>
        <div class="lp-stage-count" id="stage-new">0</div>
    </div>
    <div class="lp-stage" onclick="filterLeads('active')" data-stage="active">
        <div class="lp-stage-icon">📞</div>
        <div class="lp-stage-name">In Sequence</div>
        <div class="lp-stage-count" id="stage-active">0</div>
    </div>
    <div class="lp-stage" onclick="filterLeads('contacted')" data-stage="contacted">
        <div class="lp-stage-icon">✅</div>
        <div class="lp-stage-name">Contacted</div>
        <div class="lp-stage-count" id="stage-contacted">0</div>
    </div>
    <div class="lp-stage" onclick="filterLeads('appointment')" data-stage="appointment">
        <div class="lp-stage-icon">📅</div>
        <div class="lp-stage-name">Appointment</div>
        <div class="lp-stage-count" id="stage-appointment">0</div>
    </div>
</div>

<div class="lp-leads-panel">
    <div class="lp-leads-header">
        <div class="lp-leads-title">Leads <span id="leads-showing" style="color:var(--gray-500);font-weight:400">(showing all)</span></div>
        <div class="lp-leads-filter">
            <input type="text" id="lead-search" placeholder="Search phone or name..." style="padding:6px 12px;background:var(--black);border:1px solid var(--border);border-radius:6px;color:var(--white);font-size:12px" oninput="searchLeads(this.value)">
        </div>
    </div>
    <div id="leads-list">
        <div class="lp-empty">Loading leads...</div>
    </div>
</div>
</div>
