...
GHL_CALENDAR_ID = os.environ.get('GHL_CALENDAR_ID', 'qjH7iCqDP8t3dCQ3TlJx')  # Calendar for AI appointments
...
def get_hawaii_time():
    """Get current time in Hawaii (UTC-10, no daylight saving)"""
    from datetime import timezone
    utc_now = datetime.now(timezone.utc)
    hawaii_offset = timedelta(hours=-10)
    return utc_now + hawaii_offset

def is_in_call_window():
    """Check if current time is within allowed calling hours (Hawaii time)"""
    now = get_hawaii_time()
    hour = now.hour
    
    print(f"🕐 Hawaii time: {now.strftime('%I:%M %p')} (hour={hour})")
...
def get_next_call_window():
    """Get the next available call window (Hawaii time)"""
    now = get_hawaii_time()
    hour = now.hour
...
def process_scheduled_calls():
    ...
    try:
        ...
        now = get_hawaii_time().replace(tzinfo=None)  # Hawaii time, no timezone info for DB
...
def make_call(phone, name="there", agent_type="roofing", is_test=False, use_spanish=False, ghl_contact_id=None, address=""):
    ...
    if is_inbound:
        ...
        from_number = '+18083005141'  # Solar client phone number (HI)
        call_type = "outbound"
        ...
        print(f"✅ USING SOLAR CLIENT NUMBER: {from_number}")
    else:
...
def record_call_analytics(call_id, sequence_id, phone, duration, outcome, window_name=None):
    ...
    outbound_number = os.environ.get('OUTBOUND_NUMBER', '+18083005141')
...
def send_warmup_sms(phone, first_name, contact_id=None):
    """Send warm-up SMS before calling"""
    ...
    return {'success': False, 'error': 'No contact_id or GHL not configured'}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            # COMPREHENSIVE LEADS TABLE - All contact info + ways to reach them
            try:
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                c = conn.cursor()
                ...
                for seq in sequences:
                    ...
                    phone = seq.get('phone', '')
                    email = seq.get('email', '') or ''
                    first_name = seq.get('first_name', 'Unknown')
                    last_name = seq.get('last_name', '') or ''
                    full_name = f"{first_name} {last_name}".strip()
                    address = seq.get('address', '') or ''
                    city = seq.get('city', '') or ''
                    state = seq.get('state', '') or ''
                    zip_code = seq.get('zip_code', '') or ''
                    source = seq.get('source', '') or 'Unknown'
                    lead_created = seq.get('lead_created_at', '') or seq.get('created_at', '')
                    last_outcome = seq.get('last_outcome', '-') or '-'
                    
                    # Format lead created date
                    if lead_created:
                        try:
                            lead_date = lead_created[:10]
                        except:
                            lead_date = '-'
                    else:
                        lead_date = '-'
                    ...
                    if status == 'appointment_set':
                        badge = '<span style="background:#00d1ff;color:#000;padding:6px 14px;border-radius:12px;font-weight:700;font-size:12px;">✅ BOOKED</span>'
                        row_border = 'border-left:4px solid #00d1ff;'
                    elif status == 'active':
                        badge = '<span style="background:#10b981;color:#000;padding:6px 14px;border-radius:12px;font-weight:700;font-size:12px;">🔥 ACTIVE</span>'
                        row_border = 'border-left:4px solid #10b981;'
                    elif status in ['max_calls_reached', 'max_days_reached']:
                        badge = '<span style="background:#f59e0b;color:#000;padding:6px 14px;border-radius:12px;font-weight:700;font-size:12px;">⏸️ EXHAUSTED</span>'
                        row_border = 'border-left:4px solid #f59e0b;'
                    elif status == 'not_interested':
                        badge = '<span style="background:#ef4444;color:#fff;padding:6px 14px;border-radius:12px;font-weight:700;font-size:12px;">❌ NOT INT</span>'
                        row_border = 'border-left:4px solid #ef4444;'
                    else:
                        badge = f'<span style="background:#6b7280;color:#fff;padding:6px 14px;border-radius:12px;font-size:12px;">{status.upper()}</span>'
                        row_border = 'border-left:4px solid #6b7280;'
                    
                    # Contact action buttons
                    phone_clean = phone.replace('+', '').replace('-', '').replace(' ', '')
                    call_btn = f'<a href="tel:{phone}" style="background:#10b981;color:#000;padding:6px 12px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:600;">📞 Call</a>' if phone else ''
                    sms_btn = f'<a href="sms:{phone}" style="background:#3b82f6;color:#fff;padding:6px 12px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:600;">💬 SMS</a>' if phone else ''
                    email_btn = f'<a href="mailto:{email}" style="background:#8b5cf6;color:#fff;padding:6px 12px;border-radius:6px;text-decoration:none;font-size:12px;font-weight:600;">📧 Email</a>' if email else ''
                    
                    # Days since lead came in
                    days_old = '-'
                    if lead_created:
                        try:
                            lead_dt = datetime.fromisoformat(lead_created.replace('Z', '+00:00').replace('.000', ''))
                            days_old = (datetime.now(lead_dt.tzinfo) - lead_dt).days if lead_dt.tzinfo else (datetime.now() - lead_dt.replace(tzinfo=None)).days
                            days_old = f"{days_old}d ago"
                        except:
                            days_old = '-'
                    
                    cards_html += f'''
                    <div style="background:#12121a;border-radius:12px;padding:20px;margin-bottom:16px;{row_border}">
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
                            <div>
                                <div style="font-size:20px;font-weight:700;">{full_name}</div>
                                <div style="color:#6b7280;font-size:13px;margin-top:4px;">Source: {source} • Lead age: {days_old}</div>
                            </div>
                            <div style="text-align:right;">
                                {badge}
                                <div style="margin-top:8px;font-size:13px;color:#6b7280;">Day {current_day}/7 • {calls_made} calls</div>
                            </div>
                        </div>
                        
                        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:16px;">
                            <div style="background:#0a0a0f;padding:12px;border-radius:8px;">
                                <div style="font-size:11px;color:#6b7280;text-transform:uppercase;margin-bottom:4px;">📞 Phone</div>
                                <div style="font-size:15px;color:#00d1ff;font-weight:600;">{phone or '-'}</div>
                            </div>
                            <div style="background:#0a0a0f;padding:12px;border-radius:8px;">
                                <div style="font-size:11px;color:#6b7280;text-transform:uppercase;margin-bottom:4px;">📧 Email</div>
                                <div style="font-size:14px;color:#f5f5f5;word-break:break-all;">{email or '-'}</div>
                            </div>
                            <div style="background:#0a0a0f;padding:12px;border-radius:8px;">
                                <div style="font-size:11px;color:#6b7280;text-transform:uppercase;margin-bottom:4px;">📍 Location</div>
                                <div style="font-size:14px;">{location}</div>
                            </div>
                            <div style="background:#0a0a0f;padding:12px;border-radius:8px;">
                                <div style="font-size:11px;color:#6b7280;text-transform:uppercase;margin-bottom:4px;">🏠 Address</div>
                                <div style="font-size:14px;">{address or '-'}</div>
                            </div>
                        </div>
                        
                        <div style="display:flex;justify-content:space-between;align-items:center;padding-top:12px;border-top:1px solid #1e1e2e;">
                            <div style="font-size:13px;color:#6b7280;">
                                Last: <span style="color:#f5f5f5;">{last_outcome}</span> • 
                                Added: <span style="color:#f5f5f5;">{lead_date}</span>
                            </div>
                            <div style="display:flex;gap:8px;">
                                {call_btn} {sms_btn} {email_btn}
                            </div>
                        </div>
                    </div>'''
                ...
                exhausted = len([s for s in sequences if s.get('status') in ['max_calls_reached', 'max_days_reached']])
                with_email = len([s for s in sequences if s.get('email')])
...
<style>
*{{margin:0;padding:0;box-sizing:border-box}}
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#0a0a0f;color:#f5f5f5;padding:24px}}
...
.stat-value{{font-size:32px;font-weight:800;color:#00d1ff}}
.stat-label{{font-size:11px;color:#6b7280;text-transform:uppercase;margin-top:4px}}
.btn{{background:#00d1ff;color:#000;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px}}
.btn-outline{{background:transparent;border:1px solid #00d1ff;color:#00d1ff}}
.btn-red{{background:#ef4444}}
.actions{{display:flex;gap:8px;flex-wrap:wrap}}
</style></head>
<body>
<div class="header">
    <div class="title">📋 Lead Contact Center</div>
    <div class="actions">
        <a href="/sequences" class="btn btn-outline">📊 Full Dashboard</a>
        <a href="/analytics" class="btn btn-outline">🧠 Intelligence</a>
        <a href="/restore" class="btn">🔄 Restore</a>
        <a href="/stop" class="btn btn-red">🛑 Stop</a>
    </div>
</div>
...
<div class="stats">
    ...
    <div class="stat"><div class="stat-value">{booked}</div><div class="stat-label">Booked</div></div>
    <div class="stat"><div class="stat-value">{exhausted}</div><div class="stat-label">Exhausted</div></div>
    <div class="stat"><div class="stat-value">{with_email}</div><div class="stat-label">Have Email</div></div>
</div>

{cards_html if cards_html else '<div style="text-align:center;padding:60px;background:#12121a;border-radius:12px;"><div style="font-size:48px;margin-bottom:16px;">📭</div><div style="font-size:20px;margin-bottom:8px;">No leads yet</div><div style="color:#6b7280;"><a href="/restore" style="color:#00d1ff;">Click here to restore from GHL</a></div></div>'}
...
<script>setTimeout(()=>location.reload(), 30000);</script>
            ...
            try:
                ...
                c.execute('''SELECT * FROM call_analytics ORDER BY call_time DESC LIMIT 50''')
                recent_calls = [dict(row) for row in c.fetchall()]
                
                # Get phone health
                c.execute('SELECT * FROM phone_health ORDER BY updated_at DESC')
                phone_health = [dict(row) for row in c.fetchall()]
                
                # Get best time predictions
                c.execute('SELECT * FROM best_time_learning WHERE total_attempts >= 2 ORDER BY answer_rate DESC LIMIT 20')
                ...
                # Build hour heatmap
                hour_data = {h.get('hour', 0): h for h in insights.get('best_hours', [])}
                hour_heatmap = ""
                for h in range(24):
                    data = hour_data.get(h, {'attempts': 0, 'answer_rate': 0})
                    rate = data.get('answer_rate', 0)
                    if rate > 25:
                        color = '#10b981'
                    elif rate > 15:
                        color = '#f59e0b'
                        ...
                        color = '#1e1e2e'
                    hour_heatmap += f'<div style="width:30px;height:40px;background:{color};border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:10px;" title="{rate}% answer rate at {h}:00">{h}</div>'
                
                # Build phone health cards
                phone_cards = ""
                for ph in phone_health:
                    health = ph.get('health_score', 100)
                    if health > 80:
                        health_color = '#10b981'
                        health_icon = '🟢'
                    elif health > 50:
                    ...
                    phone_cards += f'''
                    <div style="background:#12121a;border-radius:12px;padding:16px;border-left:4px solid {health_color};">
                        <div style="display:flex;justify-content:space-between;align-items:center;">
                            <div>
                                <div style="font-size:11px;color:#6b7280;">OUTBOUND NUMBER</div>
                                <div style="font-size:18px;font-weight:700;">{ph.get('phone_number', 'Unknown')}</div>
                            </div>
                            <div style="text-align:right;">
                                <div style="font-size:24px;">{health_icon}</div>
                                <div style="font-size:12px;color:{health_color};">{health}% Health</div>
                            </div>
                        </div>
                        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:12px;">
                            <div style="text-align:center;background:#0a0a0f;padding:8px;border-radius:6px;">
                                <div style="font-size:20px;font-weight:700;">{ph.get('calls_today', 0)}</div>
                                <div style="font-size:10px;color:#6b7280;">Today</div>
                            </div>
                            <div style="text-align:center;background:#0a0a0f;padding:8px;border-radius:6px;">
                                <div style="font-size:20px;font-weight:700;">{ph.get('answers_today', 0)}</div>
                                <div style="font-size:10px;color:#6b7280;">Answered</div>
                            </div>
                            <div style="text-align:center;background:#0a0a0f;padding:8px;border-radius:6px;">
                                <div style="font-size:20px;font-weight:700;">{round((ph.get('answer_rate_today') or 0) * 100)}%</div>
                                <div style="font-size:10px;color:#6b7280;">Rate</div>
                            </div>
                            <div style="text-align:center;background:#0a0a0f;padding:8px;border-radius:6px;">
                                <div style="font-size:20px;font-weight:700;">{ph.get('spam_risk_score', 0)}</div>
                                <div style="font-size:10px;color:#6b7280;">Spam Risk</div>
                            </div>
                        </div>
                        {f'<div style="margin-top:8px;padding:8px;background:#ef4444;border-radius:6px;font-size:12px;">⚠️ {ph.get("rotation_reason")}</div>' if ph.get('needs_rotation') else ''}
                    </div>'''
                
                # Build best windows
                window_cards = ""
                for w in insights.get('best_windows', []):
                    window_cards += f'''
                    <div style="background:#0a0a0f;padding:12px;border-radius:8px;text-align:center;">
                        <div style="font-size:14px;font-weight:600;text-transform:capitalize;">{w.get('window', 'Unknown')}</div>
                        <div style="font-size:24px;font-weight:800;color:#00d1ff;">{w.get('answer_rate', 0)}%</div>
                        <div style="font-size:11px;color:#6b7280;">{w.get('appointments', 0)} appointments</div>
                    </div>'''
                
                # Recent calls table
                calls_rows = ""
                for call in recent_calls[:20]:
                    quality = call.get('quality_score', 0)
                    if quality >= 75:
                        quality_badge = f'<span style="background:#10b981;color:#000;padding:2px 8px;border-radius:4px;">{quality}</span>'
                    elif quality >= 50:
                        quality_badge = f'<span style="background:#f59e0b;color:#000;padding:2px 8px;border-radius:4px;">{quality}</span>'
                    else:
                        quality_badge = f'<span style="background:#ef4444;color:#fff;padding:2px 8px;border-radius:4px;">{quality}</span>'
                    
                    calls_rows += f'''<tr>
                        <td style="padding:10px;">{call.get('phone', '')}</td>
                        <td style="padding:10px;">{call.get('outcome', '-')}</td>
                        <td style="padding:10px;">{round(call.get('duration_seconds') or 0)}s</td>
                        <td style="padding:10px;">{quality_badge}</td>
                        <td style="padding:10px;">{call.get('sentiment_score', 0)}/10</td>
                        <td style="padding:10px;">{call.get('window_name', '-')}</td>
                        <td style="padding:10px;color:#6b7280;font-size:12px;">{(call.get('call_time') or '')[:16]}</td>
                    </tr>'''
                
                # Best time predictions
                predictions_rows = ""
                for bt in best_times:
                    predictions_rows += f'''<tr>
                        <td style="padding:10px;">{bt.get('phone', '')}</td>
                        <td style="padding:10px;font-weight:700;">{bt.get('best_window', '-').title()}</td>
                        <td style="padding:10px;">{bt.get('best_hour', '-')}:00</td>
                        <td style="padding:10px;color:#10b981;">{round((bt.get('answer_rate') or 0) * 100)}%</td>
                        <td style="padding:10px;">{bt.get('total_attempts', 0)}</td>
                        <td style="padding:10px;">{round((bt.get('confidence_score') or 0) * 100)}%</td>
                    </tr>'''
                
                total_calls = summary.get('total') or 0
                total_answered = summary.get('answered') or 0
                avg_quality = summary.get('avg_quality') or 0
                appointments = summary.get('appointments') or 0
                answer_rate = round((total_answered / total_calls * 100) if total_calls > 0 else 0, 1)
                
                html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>🧠 Call Intelligence Dashboard</title>
<style>
*{{margin:0;padding:0;box-sizing:border-box}}
body{{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#0a0a0f;color:#f5f5f5;padding:24px}}
.header{{display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:16px}}
...
.stat{{background:#12121a;border-radius:12px;padding:20px;text-align:center}}
.stat-value{{font-size:36px;font-weight:800;color:#00d1ff}}
.stat-label{{font-size:11px;color:#6b7280;text-transform:uppercase;margin-top:4px}}
.section{{background:#12121a;border-radius:12px;padding:20px;margin-bottom:24px}}
.section-title{{font-size:16px;font-weight:700;margin-bottom:16px;display:flex;align-items:center;gap:8px}}
table{{width:100%;border-collapse:collapse}}
th{{text-align:left;padding:12px;background:#1e1e2e;font-size:11px;text-transform:uppercase;color:#6b7280}}
...
<body>
<div class="header">
    <div class="title">🧠 Call Intelligence Dashboard</div>
    <div style="display:flex;gap:8px;">
        <a href="/leads" class="btn" style="background:transparent;border:1px solid #00d1ff;color:#00d1ff;">📋 Leads</a>
        <a href="/sequences" class="btn">📊 Sequences</a>
    </div>
</div>

<div class="stats">
    <div class="stat"><div class="stat-value">{total_calls}</div><div class="stat-label">Total Calls</div></div>
    <div class="stat"><div class="stat-value">{total_answered}</div><div class="stat-label">Answered</div></div>
    <div class="stat"><div class="stat-value">{answer_rate}%</div><div class="stat-label">Answer Rate</div></div>
    <div class="stat"><div class="stat-value">{round(avg_quality)}</div><div class="stat-label">Avg Quality</div></div>
    <div class="stat"><div class="stat-value">{appointments}</div><div class="stat-label">Appointments</div></div>
</div>

<div class="section">
    <div class="section-title">📞 Phone Number Health</div>
    <div class="grid-3">{phone_cards if phone_cards else '<div style="color:#6b7280;">No phone data yet</div>'}</div>
</div>

<div class="section">
    <div class="section-title">⏰ Best Call Windows</div>
    <div class="grid-3">{window_cards if window_cards else '<div style="color:#6b7280;">No window data yet</div>'}</div>
</div>

<div class="section">
    <div class="section-title">🔥 Hour Heatmap (Answer Rate)</div>
    <div class="heatmap">{hour_heatmap}</div>
    <div style="margin-top:8px;font-size:11px;color:#6b7280;">🟢 >25% | 🟡 15-25% | 🔴 5-15% | ⚫ <5%</div>
</div>

<div class="section">
    <div class="section-title">🎯 Best Time Predictions (Per Lead)</div>
    <table>
        <thead><tr><th>Phone</th><th>Best Window</th><th>Best Hour</th><th>Answer Rate</th><th>Attempts</th><th>Confidence</th></tr></thead>
        <tbody>{predictions_rows if predictions_rows else '<tr><td colspan="6" style="text-align:center;padding:20px;color:#6b7280;">Not enough data yet</td></tr>'}</tbody>
    </table>
</div>

<div class="section">
    <div class="section-title">📊 Recent Call Quality</div>
    <table>
        <thead><tr><th>Phone</th><th>Outcome</th><th>Duration</th><th>Quality</th><th>Sentiment</th><th>Window</th><th>Time</th></tr></thead>
        <tbody>{calls_rows if calls_rows else '<tr><td colspan="7" style="text-align:center;padding:20px;color:#6b7280;">No calls recorded yet</td></tr>'}</tbody>
    </table>
</div>
...
<div class="container">
    ...
    <div class="phone-panel">
        ...
        <div style="margin-top:12px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;font-size:13px;color:#6b7280;">
            ...
            <div>⏰ Last updated: {get_hawaii_time().strftime('%I:%M %p')} HI</div>
        </div>
    </div>
...
Let's close some deals! 🚀"""
        ...
        # ═══════════════════════════════════════════════════════════════════
            ...
            # UPDATED: Now moves contacts to correct pipeline stage + better appointment detection
            try:
